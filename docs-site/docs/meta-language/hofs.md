...
Phase B of the meta-language ships three built-in **higher-order functions (HOFs)**: `map`, `filter`, and `reduce`. HOFs operate on `List<T>` values at compile time and produce new `List<U>` values or reduced fragments that splice into SQL.
