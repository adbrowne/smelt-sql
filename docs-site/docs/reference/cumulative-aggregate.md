...
Multiple candidates are also admitted, applied in the order written: `COALESCE(MAX(a), MAX(b), 'unknown')` keeps a `(value, written)` state pair per candidate and prefers `a` over `b` over the fallback on every read, so the cross-window merge no longer needs to preserve a preference order itself — each candidate's own state is independently first-non-null. Every candidate still needs its own declared functional dependency (or must itself be key-derived) — except a candidate that is itself one of the model's own `GROUP BY` key columns, `MAX`/`MIN` wrapper or not: key membership already proves the per-key constancy a declaration would otherwise assert, so `COALESCE(MAX(device_id), 0)` needs no declaration when `device_id` is the model's key. The first candidate whose provenance cannot be proven (and is not itself a key column) refuses the whole column, naming that candidate.
...
A window-forward model expects both `--event-time-start` and `--event-time-end`; they address the *driving source's* partition column. Running it without them (or with only one) **refuses** — supply both flags whenever you mean to maintain the table incrementally. To intentionally rebuild the whole target from the SELECT over the entire source instead, pass `--full-refresh`, which drops and recreates the target and skips the window requirement.
