...
- **Fail-loud discipline** — Every path that can encounter unrecognisable user input must emit a diagnostic rather than silently falling back to a default or `Unknown` value. Three CI gates enforce this; never lower them without a reviewer sign-off note in the commit:
