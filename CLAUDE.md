...
- **Function-registry single ownership** — a built-in SQL function's name, classification (aggregate/window/scalar), registry-driven type, **and per-dialect emission** all derive from one table (`BuiltinRegistry` in `crates/smelt-types/src/signatures.rs`), never lowered without a reviewer sign-off note:
  ...
  - **Emission-ownership gate** (`cargo test -p smelt-dialect --test emission_ownership`) — `printer.rs` holds no name-matched function spelling and no branch on a concrete `SqlDialect` variant. A per-dialect spelling is `Signature::emission` data; a capability-shaped difference is a `BackendCapabilities` flag. Every `RewriteId` variant (parsed out of `signatures.rs`, not restated) must be dispatched.
