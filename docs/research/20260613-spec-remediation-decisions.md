...
**Decision:** Other - see notes.   **Notes:** This might be a change. I don't want dedicated scan roots per type. We want to support large projects where items are grouped by domain/subproject - and items of different types can be kept together. Therefore things should appear under smelt.* directly. This might be a wider change but a model in the root of the project (same dir as smelt.yml) should show up as smelt.model_name. If paths are configured then the paths should not be added to the path (for example if models as added as a path then ./models/foo.sql model should be smelt.foo). Models/sources/seeds etc should be found by searching all subdirectories under the project that are not excluded.

**Resolved 2026-06-13 (after review back-and-forth):**
- **No dedicated scan roots.** `functions/`/`sources/`/`tests/` are NOT special; discovery scans **every non-excluded subdirectory** under the project root and the *file content/extension* determines kind (not location). Reconciles **D-05** (kept: "scan everywhere, kind by content"; dropped: "dedicated roots retain prefix").
- **`paths:` = strip-only, not a discovery gate.** Confirmed semantics: `paths:` is a list of directory prefixes **stripped from the address**; discovery is always project-wide. A file's address = its path relative to project root, minus any configured `paths:` prefix. `models/foo.sql` with `paths:["models"]` → `smelt.foo`; `foo.sql` at root → `smelt.foo`; `sources/raw/events.yml` (not in `paths:`) → `smelt.sources.raw.events`.
- **Rationale to capture in the spec:** the strip-list supports the "everything under a generic container folder (`src/`, `pipeline/`)" layout — put `src` in `paths:` and addresses stay clean (`smelt.staging.orders`, not `smelt.src.staging.orders`) without forcing one directory per kind.
- **Ripples:** C1 example table rows become `smelt.sources.raw.events` / `smelt.seeds.raw.users` (directory retained because not stripped); seeds.md "under `paths:`" reworded; `paths:` redefinition lands in smelt_yml.md; D-resolve code obligation (discovery = scan-all, `paths:` = strip-only). architecture.md is `stable` — extra care.
...
**Decision:** A____________   **Notes:** ____________
...
**Decision:** A - it's real already   **Notes:** ____________
...
**Decision:** A ____________   **Notes:** ____________
...
**Decision:** B ____________   **Notes:** ____________
...
**Decision:** A ____________   **Notes:** Although smelt.source() should no longer exist - ensure there are not more references to that.
...
**Decision:** ↔R2   **Notes:** ____________
...
**Decision:** A. Makes sense to point - at some stage later I expect this to be extensible where users can add new keys for planner rules. ____________   **Notes:** ____________
...
**Decision:** A (resolved 2026-06-13) — drop the fall-through; scoped shorthand resolves only `<scope>.<arg>`, full path required otherwise.   **Notes:** ____________
...
**Decision:** I was expected you target a CTE but you mock the upstream dependencies for the model that cte requires. ____________   **Notes:** ____________
...
**Decision:**  A - use paths, exclude and also note the new rule that any sql file is disoverable if not ruled out by exclude (so you may be able to ignore paths here) ____________   **Notes:** ____________
...
**Decision:** ↔R2  ____________   **Notes:** ____________
...
**Decision:** Go with recommendation   **Notes:** ____________
