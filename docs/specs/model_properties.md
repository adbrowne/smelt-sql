...
| `nondeterministic_columns` (superseded) | exempt | — | — | — | superseded by the per-column `columns.<c>.contract: plausible` declaration (`models.md`), a column-scoped equivalence contract rather than a narrowing licence of its own; tracked at `models.md` §Known Divergences decision 8 | — | exempt |
...
- **The maintenance plan** (`incremental_models.md`): the plan matrix's column-group factoring, per-cell admission obligations 2/4/5/6/7, the K8 partition-locality guardrail, the repair family (obligation 7, affected-key discovery), and the definition-change trigger's classification all consume the eight proofs above by name; the plan owns only which cells demand which proof and what a verdict costs (technique choice, refusal diagnostics), never the proof's own logic.
...
- **`nondeterministic_columns`'s list-form declaration is not yet removed from the parser (Open Question)**
  — it is superseded by `columns.<c>.contract` (`models.md` owns the surface) but still parses;
  removal is unscheduled.
