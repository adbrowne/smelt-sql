...
Every `INTERVAL` literal this walk encounters is parsed by one shared parser, over three accepted spellings — `INTERVAL '<n> <unit>'`, `INTERVAL '<n>' <UNIT>`, and `INTERVAL <n> <UNIT>` — folding identically into `Offset::Seconds` (seconds/minutes/hours/days/weeks — uniform durations) or `Offset::Symbolic` (month/year — non-uniform: a month is 28-31 days, a year 365-366); the unit is read from outside the quotes when the spelling puts it there. A symbolic literal in a bound-relevant position cannot populate a `Bounded{before, after}` value, so per the fail-closed constraint below it forces `NotDerivable` for that source rather than an approximate fixed-day guess — a rule that applies identically across all three spellings, so no spelling can smuggle a calendar-variable offset past the bound derivation. The same parser backs the driving-fact/anchor trace's constant-shift folding (below), so a `col ± INTERVAL '<n> month'` shift and a `RANGE BETWEEN INTERVAL '<n> month' PRECEDING` frame are classified identically.
