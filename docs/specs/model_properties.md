...
- **Heuristic text-scanning layer.** The mature core is AST-pure (`EventTimeTrace`, `Monotonicity`, `SelectItemKind`, `PartitionAlignment` — now including the window-`OVER` scope alongside `GROUP BY`/`DISTINCT`, the monoid combiner set); an older layer (`source_bounds.rs` bound extraction, `temporal.rs`, the window-`OVER`/`LIMIT` *admission* scans in `incremental.rs`) is uppercase-substring based and fail-closed. The admission scans in `incremental.rs` have not yet been rewired onto the AST-based `PartitionAlignment` signal — that consumer wiring is a mode-composition concern, not a proof-layer one. Set-operation distribution covers `UNION ALL` only; `INTERSECT`/`EXCEPT` are unclassified.
