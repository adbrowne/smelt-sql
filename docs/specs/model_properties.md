...
- **Remaining live code duplications** the consolidation must still remove — each of these is a proof that currently exists in more than one copy: (1) the nondeterminism-function list copied across `rules/incremental.rs`, `rules/cumulative.rs`, and `analysis/monotonicity.rs`; (2) aggregate-name extraction done twice (typed `SqlFunction::is_aggregate` vs a string re-parse in `cumulative.rs`). Driving-fact/anchor resolution is now unified — `cumulative.rs`'s selection delegates to `source_bounds.rs`'s alias-scoped `resolve_single_anchor`, the same primitive `resolve_join_driving_fact` uses. Interval-literal parsing is now unified — one parser (`parse_interval`), shared by `source_bounds.rs`, `monotonicity.rs`, and `temporal.rs`'s day-granular estimate, with month/year folding to `Offset::Symbolic` rather than the two previously-divergent ≈30d/≈365d rules — and the two bound-derivation orchestration sites both call `derive_and_classify_bounds`. `analysis/temporal.rs`'s `EffectiveWindow` walk (day-granular, advisory batch-sizing/filter-widening) and `analysis/source_bounds.rs`'s `BoundResult` walk (second-granular, pushdown-eligibility proof) remain two separate walks: they answer different questions with deliberately different fail-closure — `EffectiveWindow` still treats a bare `LAG`/`LEAD` (no explicit `RANGE` frame) as a bounded row/period-based estimate for backfill chunk sizing, while `BoundResult` refuses the same construct (`NotDerivable`) because it cannot be safely pushed to a source scan without an explicit `RANGE BETWEEN INTERVAL` bound. Collapsing these into one verdict would either lose the batch-chunking heuristic or loosen the pushdown proof; tracked as future work, not silently merged.
