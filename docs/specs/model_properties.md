...
- **The property layer is unbuilt as a consolidated whole**, and several proofs carry known gaps captured in §Surface as `partial`/`not-yet`. The remaining new classifiers (fan-out/cardinality, additive-diff, window-independence/ordered-execution) and the model-scoped declaration surfaces are tracked by `docs/plans/20260704-model-updates.md` (design: `docs/research/20260704-maintenance-fundamentals.md`). The algebraic discriminants classifier (`analysis::discriminants::combiner_discriminants`) is now built beyond the monoid set — is-monoid, needs-inverse, decomposable, and value-vs-order-monotone are all classified; `cumulative`'s monoid gate reads it rather than its own allowlist.
