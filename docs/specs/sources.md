...
| `retention` | no | absent (**trusted replayable**) | How far back the source can be re-read: an interval, **rolling** — anchored to the current run and advancing with it, not a fixed calendar date, so a region's replayability is re-evaluated every run rather than fixed at declaration time. A backfill window reaching past it is refused at plan time. Absent = assumed fully replayable. Always a **declared** fact, never one smelt observes by probing the warehouse (§Semantics "The trust rule"). Well-formed only when the value is a parseable non-zero interval **and** the source also declares `timeseries:` — a bound with no clock has no reach to be compared against, so it is refused rather than accepted and ignored (§Semantics "Retention refusal"). |
...
- `key_recurrence` — the delivery-contract recurrence bound (e.g. an at-least-once feed whose redeliveries land within three days). It lives inside the block because it is delivery-contract metadata of the same species as the other sub-facts. Consumed by key temporal locality (`incremental_shapes.md` §"Key temporal locality") when a consuming model's `unique_key` resolves exactly to the declared columns; **always runtime-checked, never trusted** (`KeyedRecurrenceBoundViolated`).
...
**Command placeholders.** `command:`'s argv is opaque to smelt except for one closed
substitution grammar, applied per-argv-element before spawning: `{run_date}` (the run
window's start, ISO `YYYY-MM-DD`) and `{run_end}` (the window's exclusive end, same form).
`{{` and `}}` escape to literal `{` and `}`. Any other `{name}` is `MalformedExternalStep`
at *declaration* time — caught by the LSP and `smelt list`/`smelt run`'s pre-flight parse,
never deferred to the run that first reaches the step. A placeholder this run has no value
for (a run with no window) is `ExternalStepNotInvocable` — declaration-time validity does
not guarantee run-time resolvability, since the run window is a property of the invocation,
not the declaration.
...
projection identity, consuming model address, source key)` — **projection identity** is a
canonical hash of the P4 projection's column set; **consuming model address** is the CONSUMING
model's own address, added so that two consumers of the same source under the same projection
identity each get their own comparandum partition, satisfying the per-consuming-edge comparandum
requirement (`incremental_models.md` §"The graph layer") by construction: a sibling consumer's
diff-then-refresh can never mutate, narrow, or (for byte-identical consuming-model bodies)
unsoundly satisfy this edge's own next diff, because it never reads or writes this edge's
partition. Sharing digests across consuming edges of the same source and projection — an
optimization that would need per-consumer consumption cursors to stay sound, since a naive shared
refresh would make a sibling's next diff silently report "no change" for a delta it never consumed
— is deliberately **not** taken; the per-edge duplication this namespace costs is judged cheaper
than that mechanism. Consumers with differing projections never share, and a fail-closed full-row
digest (an unprojectable consumption, P4) is its own distinct projection identity — it is never
silently widened onto, or narrowed from, another consumer's own projection.
...
|---|---|---|
| `MalformedSource` | Error | A source `.yml` parses as YAML but violates the shape above: missing `columns`; `materialization:` present; a malformed `name:` override; an unrecognised `mutation_profile.kind` or malformed sub-fact (bad interval, unknown column in `delta_identity`/`key_recurrence.key`, missing `key_recurrence.window`); both `source_lateness:` and `mutation_profile.lateness` declared; a change-feed sub-fact on a non-feed kind (and vice versa); a malformed `watermark`/`unique_key`/`referential_integrity` (a column not declared in `columns:`, or a `referential_integrity` column set that is not a subset of a declared `unique_key` when one is declared); a malformed `retention:` — an unparseable interval, a zero-length interval, or a `retention:` declared without `timeseries:` (no clock to compare the bound against); a per-column `data_latency:` inside `columns:` (retired — declare lateness once as `mutation_profile.lateness` on the source). |
| `SourceTypeError` | Error | A `columns[].type` value is not a recognised smelt `DataType` (`types.md`). |
| `SourceMutationProfileViolated` | Error (fails the consuming run) | A verification tripwire disproved a declared narrowing fact: a processed partition's row count decreased, or its fingerprint changed at an unchanged row count, under `append_only` (a count *increase* in a closed partition is recorded as a late append instead, never this diagnostic); a delta-identity collision under `redelivery: none`; a retraction event under `retractions: false`. Names the source, the violated declaration, and the mitigation. |
...
| `SourceRetentionExceeded` | Error (plan-time refusal, or run-time refusal for a whole-table recompute) | A backfill window reaches past the declared `retention:` — the recompute would silently rebuild from partial input; points at the declaration and the stored-state provenance. |
| `SourceRetentionDowngraded` | Warning (plan derivation) | A model's required reach into a declared-`retention:` source could not be *proven* to fit inside the bound (an unbounded or otherwise underivable reach) — admitted, but the model's pre-bound region stops being claimed replayable rather than silently trusted. Names the source, the retained bound, and why the reach was unprovable. |
...
4. **Verification mechanisms** for `append_only` run as part of consuming maintenance runs, cheapest first: the watermark-monotonicity probe (per-partition row counts recorded and re-checked, where *partition* means the source's declared `timeseries.granularity` grid rather than the raw `partition_column`'s distinct values — a TIMESTAMP partition column under `granularity: day` is bucketed onto the day grid before the baseline is grouped, since a baseline recorded per distinct instant would do the closed-partition reasoning below at the wrong unit; an integer partition key, and a DATE column at `day` granularity, already are their own buckets and are grouped unchanged — catches deletes and reloads, and separately classifies a row-count *increase* in an already-closed partition as a late append rather than a violation), the frontier checksum (a sampled per-partition fingerprint over skeleton columns, gated to closed partitions and an unchanged row count — catches in-place updates without re-flagging a legitimate late append, since a delete-plus-insert netting to a count increase cannot be told apart from one and so is itself read as a late append), and full re-scan comparison (audit only). `unique_key` and `delta_identity` use the uniqueness probe scoped to the consuming run's scan window, full-table on demand via `smelt verify`. These are the source-side instances of the general probe-obligation rule (`model_properties.md` §"Probe obligation" generalizes this trust rule to model-scoped declarations) — its registry names each mechanism above by row, diagnostic, and cadence. The watermark-monotonicity probe and the frontier checksum need a cross-run recorded baseline, which a posture that persists nothing cannot supply; under such a posture the narrowing declaration's fold licence is withheld (admission falls back to the *undeclared* row of §"`mutation_profile`"'s licence table) and the downgrade is recorded as `MaintenanceStateDowngraded` (`state.md` §"The optionality rule"). `unique_key` and `delta_identity`'s uniqueness probe is unaffected — it verifies within the consuming run's own scan window and needs no cross-run state.
5. **Retention refusal.** A region recompute (backfill) whose window reaches past declared `retention:` must be refused at plan time (`SourceRetentionExceeded`): stored output for that region is better than anything recomputable, and overwriting correct state with a partial re-derivation is the one unrecoverable move. Undeclared retention is trusted replayable; the blast-radius bound is the ledger-anomaly probe — a backfill reading an empty or short region where the recorded state says data was once processed fails loud. `retention:` is **rolling**: the bound is anchored to the current run and advances on its own as old partitions age out, so a window's admissibility is re-evaluated against the bound in effect at plan time on every run, never fixed at the point the model was authored — a region reachable last month can be refused this month with no change to the model. `retention:` is a **declared** fact, never one smelt discovers by probing the warehouse: like every other narrowing declaration under the trust rule (§Semantics "The trust rule"), an over-claimed bound (longer than the producer actually keeps) is the unsafe direction, and it is admitted only paired with the verification mechanism above. A malformed or inert `retention:` is refused at parse time (`MalformedSource`), not accepted and silently ignored: an unparseable or zero-length interval is ordinary fail-loud parsing, and a `retention:` declared on a source with no `timeseries:` has no reach to be compared against, so accepting it would silently license every replay it was written to forbid. A reach the unified bound/reach derivation cannot *prove* fits inside the bound (`model_properties.md` §"Reach versus retained history"'s `UnprovableWithin` verdict — an unbounded or otherwise underivable reach) is not admitted silently either: absence of a proof is not evidence of fit, so the plan records a downgrade naming the source, the retained bound, and the reason (`SourceRetentionDowngraded`), and the model's pre-bound region stops being claimed replayable — the same posture the degradation contract takes for a missing state structure (`state.md` §"The degradation contract"), applied here to a missing proof instead of a missing capability. A reach *proven* to fit (`Within`) or a source with no declared bound (`NoDeclaredBound`) records nothing: an honoured bound is not news. The quantity a run compares against `retention:` is **the model's derived reach plus the age of the oldest region the run writes**, aged against the run's own clock — a forward-only run (no explicit window) has age zero, so steady-state maintenance is never affected, while a backfill of an old region ages into the bound. This refusal fires **before any statement executes** for that model, naming the source, the required look-back, the retained bound, and the run window that produced the age — a refused model leaves its stored output untouched rather than overwriting it with a partial re-derivation.

A whole-table recompute (`--full-refresh`, `smelt rebuild`) reaches past *every* finite `retention:` bound by construction — there is no window to age, so the decision is read directly from the model's declared `retention:` sources rather than derived from its reach. When the model already has stored output, this is refused (`SourceRetentionExceeded`): the stored table is the answer of record over the history that has since departed the source, and overwriting it with a recompute over only what survives is the same unrecoverable move a backfill refusal exists to prevent. Three cases are licensed instead of refused, each with a recorded downgrade rather than silence: a first build (no stored output to destroy), a full refresh smelt itself forces (a schema-evolution path with no `ALTER`-based alternative), and an explicit operator override. A licensed whole-table recompute reports its loss once per retained source it reads, naming the source and its retained bound — the same never-silent posture the aged-backfill refusal takes. This gate applies only to a `refresh: incremental` model with a materialized target; `table`/`view` models are recomputed from scratch every run by construction and hold no answer of record for the gate to protect.
...
11. **A non-zero exit from a step's `command:` is a run failure naming the step and its exit code** (`ExternalStepFailed`), and leaves every model downstream of its produced sources unbuilt — required steps run to completion, sequentially, before any model executes, so an unbuilt downstream model is never a race with a step still in flight. The same fail-loud posture as any other run failure, never a silent skip to stale data. A step's start, success, and non-zero exit are each reported to the run's `RunReporter` (`run_state.md` §"Run manifest") so an embedder observes them without parsing logs, and a succeeded step is recorded in the run manifest.
12. **A run that cannot invoke a reached step refuses rather than proceeding** (`ExternalStepNotInvocable`): a dry run, an environment declining external invocation, a `command:` that cannot be spawned (not found or not executable), or a `command:` placeholder with no value this run. Reading the produced sources' possibly-stale existing content instead is never the fallback — except under one narrow, explicit opt-in: `smelt run --skip-external-steps` declines to invoke every reached step *and* trusts that its produced sources are already current for the run's window, recording each as skipped (`RunOutcomeKind::Skipped`) in the run manifest rather than invoking it. This is the caller's affirmative claim, not a probed proof — smelt does not verify it. It exists for an orchestrator that is itself the authority on freshness: its own task graph guarantees a separate task already landed the window before this run starts (a Databricks Job's loader task running ahead of its `smelt run` task, say). A caller that cannot make this guarantee must not pass the flag; absent it, refusal semantics are unchanged. `smelt explain` — not `--dry-run` — is the non-refusing preview surface for a step: it renders what a step produces and how it is invoked without spawning anything or requiring a run window.
...
**Retention narrows replayability, not the equivalence invariant's quantifier** (`incremental_models.md` §"The equivalence invariant"): a partition that ages out of `retention:` was still scanned by the runs that consumed it, so it stays in the processed-input set `S` and the stored table remains the answer of record over it — trimming makes the *executable* `full_refresh` oracle unavailable over the departed region, it does not make the stored answer wrong. The rejected alternative — narrowing `S` to currently-retained history — was rejected because it would make every maintained model over a trimmed source permanently degraded relative to its own past output, for no correctness gain: the existing replayability split (`incremental_models.md` §"The replayability split") already has the vocabulary for "not independently re-derivable" without touching what the invariant promises. `retention:` is deliberately a **rolling** bound (anchored to the current run, not a fixed calendar date) because that is the physical fact real warehouses expose (BigQuery `partition_expiration_days`, Delta's retention window): declaring a fixed date would be false the day after it was written. It stays a **declared, verified** fact rather than one smelt observes by probing the warehouse, for the same reason every other narrowing world-fact does (§Semantics "The trust rule") — an over-claimed bound is the unsafe direction, and smelt has no channel to discover the true bound except by being told and then checked.
...
- **External steps are invoked on the run path and rendered by `smelt explain`.** `external_step:` declarations are discovered, classified, fail-loud validated, a node in the DAG reachable by the same selector surface as any model, listed by `smelt list`, invoked by `execute_project` ahead of every consumer of the sources they produce (`ExternalStepNotInvocable`, `ExternalStepFailed`), and rendered by `smelt explain` — the whole-project graph carries a top-level `external_steps` map (`cli.md` §"`smelt explain --json` output schema"), and `smelt explain <step>` reports what it produces, how it is invoked, and that smelt does not author it (`cli.md` §"`smelt explain <external step>`"). Tracked by `docs/outcomes/20260906-external-dag-steps/outcome.md`.
