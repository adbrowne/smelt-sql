...
- **External steps are a graph node and selectable, but not yet invoked.** `external_step:` declarations are discovered, classified, and fail-loud validated (`MalformedExternalStep`, `SourceProducerConflict`), and a step is a node in the DAG reachable by the same selector surface as any model (`model_selection.md` §"Graph traversal", §"Selection methods") and listed by `smelt list`. Run-path invocation (`ExternalStepNotInvocable`, `ExternalStepFailed`) and `smelt explain` rendering are unbuilt. Tracked by `docs/outcomes/20260906-external-dag-steps/outcome.md`.
