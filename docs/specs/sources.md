...
- **External steps are parsed and validated, but not yet ordered or invoked.** `external_step:` declarations are discovered, classified, and fail-loud validated (`MalformedExternalStep`, `SourceProducerConflict`); DAG membership, run-path invocation (`ExternalStepNotInvocable`, `ExternalStepFailed`), and `smelt explain` rendering are unbuilt. Tracked by `docs/outcomes/20260906-external-dag-steps/outcome.md`.
