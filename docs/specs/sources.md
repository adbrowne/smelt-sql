...
- **External steps are invoked on the run path; `smelt explain` rendering is unbuilt.** `external_step:` declarations are discovered, classified, fail-loud validated, a node in the DAG reachable by the same selector surface as any model, listed by `smelt list`, and invoked by `execute_project` ahead of every consumer of the sources they produce (`ExternalStepNotInvocable`, `ExternalStepFailed`). `smelt explain` rendering what a step produces, how it is invoked, and that smelt does not author it is not yet built. Tracked by `docs/outcomes/20260906-external-dag-steps/outcome.md`.
