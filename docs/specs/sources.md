...
- **The structured `mutation_profile` block does not parse.** Today's parser accepts only the flat string enum `append_only | mutable | change_feed` (`crates/smelt-core/src/sources.rs`); the sub-facts (`lateness`, `redelivery`, `retractions`, `ordered`, `delta_identity`, `key_recurrence`), the `mutable` → `mutable_snapshot` rename, `watermark:`, `unique_key:`, and `retention:` are unimplemented. `key_recurrence` exists today as a **top-level** key — its subsumption into the block (decision 9) is a surface migration pending with the rest.
...
- **Landed-delta recording is unbuilt.** The per-source delta intervals the graph layer consumes are specified as derived-and-recorded; today the run state records covered intervals per *model* only (`run_state.md`).
