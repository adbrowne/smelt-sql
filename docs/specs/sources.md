...
are byte-identical may **share** one sidecar partition's digests only where each consumer's
diff-then-refresh cycle remains its own (`incremental_models.md` §"The graph layer" — the
per-consuming-edge comparandum requirement): a shared partition whose refresh is committed by one
consumer's transaction would make a sibling consumer's next diff silently report "no change" for
a delta it never consumed, so sharing digests is admissible only with per-consumer consumption
cursors, never with a shared refresh. Consumers with differing projections never share, and a fail-closed
...
- **Whether the built sidecar upholds the per-consuming-edge comparandum requirement under a
  shared projection-identity partition is unverified** — the namespace admits sharing for
  byte-identical projections, and §"Naming and namespace" now conditions that sharing on
  per-consumer consumption cursors; no test exercises two consumers of one source through a
  shared partition. Audit alongside the multi-consumer wiring of
