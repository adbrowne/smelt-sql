...
- **`referential_integrity` is specified but unparsed.** `crates/smelt-core/src/sources.rs` does not yet parse the key, so no source can declare it and the count-preservation tripwire (`SourceCountPreservationViolated`) does not exist; every enrichment join is proven row-preserving only by being spelled as a `LEFT JOIN` today. This is the world-fact `model_properties.md` §"Skeleton-source closure" (P1) consumes for its row-preservation conjunct. Tracked by `docs/plans/20260715-composed-axes-conditional-maintenance.md` (E2).
