...
- **`referential_integrity` parses and validates; P1 closure now runs live over external sources, and the count-preservation tripwire query exists but is not yet wired into a live run's own transaction.** `crates/smelt-core/src/sources.rs` parses the bare-string and list forms and enforces the subset rule against a declared `unique_key` (`MalformedSource`), so a source can now declare the world-fact `model_properties.md` §"Skeleton-source closure" (P1) consumes for its row-preservation conjunct. An `UpstreamMutation` cell driven by a `mutation_profile: mutable_snapshot` source that declares `referential_integrity` now closes P1 for its own inner-join enrichment the same way a `LEFT JOIN` does (`smelt_logical::maintenance::derive::mutation_enrichment_closure`), and the closed verdict licenses the SAME delta-restricted recompute a maintained-model edge's own closure does (`smelt_logical::maintenance::choice::resolve_recompute_restriction`) — restricting the recompute to the fingerprint sidecar's synthesized changed-key set (the fixture: `examples/timeseries/models/daily_events_enriched.sql`, whose `raw.users` dimension declares both `unique_key` and `referential_integrity`; the undeclared inner-join shape stays `Open`, `crates/smelt-logical/tests/skeleton_closure_pinned.rs`). The count-preservation tripwire's query is emitter-authored (`smelt_logical::maintenance::emit::emit_count_preservation_probe`) and proven against a real backend directly — a violated declaration (a fact row whose key has no match in the dimension) is detected. What remains unbuilt: neither the closure-licensed restriction nor the tripwire is dispatched by a live run yet — `crates/smelt-runtime/src/execute.rs`'s regular incremental batch loop does not consult either, so today's live runs still take the ordinary (unrestricted, unverified) column-scoped `MERGE`; only a `LEFT JOIN` is unconditionally trusted end-to-end in a live run today. Tracked by `docs/plans/20260715-composed-axes-conditional-maintenance.md` (E2, F5).
