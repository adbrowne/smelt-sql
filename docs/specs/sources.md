...
- **The built append-only probe's fingerprint leg fires on a late append, contrary to the decided rule.** `model_properties.md` §Constraints "Declared lateness is orchestration-only" fixes that a row-count *increase* in a closed partition is a late arrival to re-process and that only a count decrease or a fingerprint change is a violation. The probe as built (`crates/smelt-logical/src/maintenance/emit.rs`, `emit_append_only_posture_probe`) compares the closed partition's whole-partition content fingerprint whenever `check_fingerprint` is set, and an append changes that digest — so a late append into a closed partition is reported as `SourceMutationProfileViolated` rather than re-processed. The decided reading requires the fingerprint leg to run only when the partition's count is unchanged (equal count and changed digest ⇒ in-place mutation; increased count ⇒ late arrival, re-present the window; the combination of an update and an append in one closed partition is then undetectable by this probe, which is a stated weakening, not a silent one). The succession grain (`incremental_shapes.md` §"Run shape and late events") and every window-forward consumer of an `append_only` source with non-zero lateness depend on the decided reading. Tracked with the probe-backed-facts work, `docs/outcomes/20260809-probe-backed-facts/outcome.md`.
