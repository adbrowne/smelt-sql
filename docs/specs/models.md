...
| `refresh: batched` \| `keyed` \| `cumulative` \| `versioned` | Hard error: unknown refresh value, with a fix-it naming the `refresh: incremental` + shape-facts (`timeseries:` / `unique_key:`) replacement, applied by hand |
