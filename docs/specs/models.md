...
- **`nondeterministic_columns` → `columns.<c>.contract` migration.** The per-column contract supersedes the list form. The grammar ownership question this note used to track — `columns.<c>.contract` vs. column `tests:` — is settled: `columns.<c>.contract` is a `schema_evolution.md`/`incremental_models.md`-owned key and `columns.<c>.tests` is `data_tests.md`-owned, both listed in the `columns:` key table above; the two do not overlap. A proposed `on_column_add: backfill | leave_null | recompute` knob (a species of `backfill:`) is noted, not yet surface.
