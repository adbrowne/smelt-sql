...
| `merge_key` | string[] | — | The **write/dedup key** a column-scoped MERGE technique writes on — never an identity fact, never a driver of grain. A single string is sugar for a one-element list. Admitted on any `refresh: incremental` output; frontmatter wins over the `smelt.yml` model override when both set it. Distinct from `unique_key:` (declares identity and moves a clocked output into the composed key+time shape); `merge_key:` never does. |
| `timeseries` | object | — | The output's declared **clock** — the other shape-defining Relation-Contract fact (`event_time_column`, `partition_column`, `granularity`). See `timeseries.md`. Declared-and-checked on any `refresh: incremental` output that carries a time axis; with identity also declared, the clock is admitted iff key temporal locality is established (`incremental_shapes.md` §"Key temporal locality"). `granularity` is also the model's partition-axis grain for cross-model propagation (`incremental_models.md` §"The graph layer"). |
...
| `safety_overrides` | object | all false | Named escape hatches for the partition-grain safety checks (e.g. `allow_having`). Admitted only on a partition-shaped output (a declared clock, no declared identity). See `incremental_shapes.md` §"Partition-grain declaration (`grain: partition`)". |
...
| `data_latency` | object | **Retired.** Declared lateness is an orchestration fact that lives on the source (`sources.md` `mutation_profile.lateness`) and is never a plan input; a per-column `data_latency` is a hard error with a fix-it naming the source key. | `sources.md`, `model_properties.md` §Constraints |
...
| `timeseries:` + `unique_key:` on a model, but key temporal locality cannot be established | Hard error (`incremental_shapes.md` §"Key temporal locality") |
...
| `grain: key` asserted with no declared `unique_key:` and no `GROUP BY`-derivable identity in the model's own SQL | Hard error naming the asserted grain and the empty derived key |
...
| `refresh: batched` \| `keyed` \| `cumulative` \| `versioned` | Hard error: unknown refresh value, with a fix-it naming the `refresh: incremental` + shape-facts (`timeseries:` / `unique_key:`) replacement |
| A `.sql` frontmatter `batched:` sub-block (`batched.unique_key` / `batched.safety_overrides` / `batched.nondeterministic_columns`) | Hard error naming each sub-block key the caller declared, with a fix-it giving the top-level replacement and the caller's own value under the new spelling (`merge_key:`, `safety_overrides:`, or `columns.<c>.contract: plausible` respectively) |
| A `smelt.yml` `models.<name>.batched:` sub-block (any key) | Hard error naming each declared sub-key's top-level replacement and the caller's own value (`unique_key` → top-level `merge_key:`, `safety_overrides` → top-level `safety_overrides:`, `nondeterministic_columns` → `columns.<c>.contract: plausible`) |
| Unknown frontmatter key | Hard error (`deny_unknown_fields`) |

### Batched sub-block retirement

A `batched:` sub-block declared anywhere — `.sql` frontmatter or a `smelt.yml` model entry — is refused: the sub-block's three keys are only ever a nested surface for options that already have top-level homes. `batched.unique_key` → top-level `merge_key:`; `batched.safety_overrides` → top-level `safety_overrides:` — both carry the same precedence as every other Relation-Contract fact (SQL frontmatter over the `smelt.yml` model override; `smelt_yml.md` §"Precedence rules" rule 2, "Incremental"). `batched.nondeterministic_columns` has no replacement inside `batched:` at all: the per-column `columns.<c>.contract: plausible` declaration (`incremental_models.md` §"Non-determinism and the payload rule") is its sole replacement, declarable only in the model's own `.sql` frontmatter — a `smelt.yml`-side `nondeterministic_columns` key is refused the same way, regardless of its value, with a fix-it pointing at the `.sql`-frontmatter-only replacement. A `plausible` contract on a column that also serves as the model's `event_time_column`, `partition_column`, or a `unique_key` member is refused — those skeleton positions govern windowing, partition placement, or dedup identity and must stay deterministic, the same bar the retired list form enforced.

`merge_key:` is **not** the identity fact: it names the row key a column-scoped MERGE technique writes on, for a partition-shaped output whose column-scoped MERGE needs a row key without becoming key-addressed. Declaring `merge_key:` never moves an output into the composed key+time shape and never satisfies the `refresh: incremental` shape-defining-fact requirement — only `timeseries:` and `unique_key:` do that (`incremental_models.md` §"Per-cell write addressing").
...
- **Names a reusable combiner/table shape without changing contract or shape** → a **function**, not a mode (e.g. `smelt.latest(value, ordering)` expanding to `MAX_BY` — `incremental_shapes.md` §"The column-family catalogue").
...
- **The shape-profile demotion of the mode specs has landed.** The grain shape profiles (`incremental_shapes.md` §"The partition grain", §"The key grain") are composition table + local machinery only; the maintenance contract they compose is owned by the same spec's contract and plan sections.
