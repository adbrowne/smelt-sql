...
- **The refresh surface described here is unimplemented.** The parser's `RefreshStrategy` still accepts `full`/`batched`/`cumulative`/`materialized_view` (with `keyed` mid-rename from `cumulative` and `versioned` not parsing); `grain:`, top-level `unique_key:`, `versioning:`, `maintenance:`, top-level `safety_overrides:`, `backfill:`, and `columns.<c>.contract` do not exist as frontmatter keys; the `batched:` sub-block this spec removes is still the live surface (`batched.unique_key`, `batched.nondeterministic_columns`, `batched.safety_overrides` — `batched_models.md`). The `smelt migrate` assist for the hard cut does not exist. Grain validation against the derived plan requires the plan itself (`maintenance_plan.md` §Known Divergences). Migration ordering: `docs/research/20260705-refresh-as-maintenance-plan/08-code-placement.md` §2.8.
