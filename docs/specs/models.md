...
`keyed` and `materialized_view` produce the same key-addressed contract but differ in **freshness owner** — smelt-pull (correct as of the last `smelt build`) vs engine-push (kept current continuously by the backend's native incremental-view maintenance). That different operational commitment is why they are peers rather than one mode with a hidden maintainer flag. `keyed` covers the running-aggregate, latest-value, and milestone/retroactive-enrichment patterns in one mode — a model's **column family** (derived from its SQL, never declared) determines which of those patterns a given column realises, so a single table may mix them (`keyed_models.md` §Design "One mode; the column family is the pattern"). `versioned` keeps every version of a key with a validity interval (the SCD Type-2 pattern, named without the vendor jargon).
...
**`materialization` is the storage axis only.** dbt's `materialized` value conflates three questions — what kind of node this is (`test`), how output is stored (`table`/`view`), and how it is refreshed (`incremental`, `materialized_view`). smelt keeps these on three orthogonal axes (see "Three orthogonal axes"). `materialization` answers only "how is output stored", with three storage modes: `view` (no data, recompute on read), `table` (persisted), and `ephemeral` (inlined, no stored object). This matches the backend's own storage-only notion of materialization. A unit test is a different *kind* of node — it produces no output and nothing depends on it — so it is a `smelt.test` declaration on the kind axis (`testing.md`), not a `materialization` value. A refresh mode (`batched`, `keyed`, `versioned`, `materialized_view`) is a different *axis* — `materialization: table` (implied) + `refresh:` — because two models can share a storage mode while differing in how they are recomputed.

**`materialized_view` is a refresh mode, not a storage mode.** A backend-managed materialized view persists data (so its *storage* is `table`) and is kept current *by the engine* (so its distinguishing property is on the *refresh* axis, the engine-owned peer of the smelt-owned `keyed`). Making it a fourth `materialization` value — as an earlier design did — repeated exactly the dbt conflation this axis split exists to avoid: it put "who owns freshness" on the storage axis. Relocating it to `refresh: materialized_view` (implied `table` storage) keeps `materialization` answering only "does this persist data" and keeps the freshness-owner question on the refresh axis where `keyed` already lives. The physical `CREATE MATERIALIZED VIEW` DDL a backend may emit is a lowering detail (`multi_backend.md`), not a user-selected storage kind.

**Refresh modes are peers, grouped by statefulness — never a strategy sub-knob.** The refresh values are distinct named peers, each naming exactly one equivalence contract (Surface §"Refresh axis"). A single `refresh:` value with a `strategy:` sub-knob (dbt's `materialized='incremental'` + `incremental_strategy`) was rejected: the sub-knob silently swaps the equivalence contract under one name, the single most common source of dbt confusion. The peers divide by two observable properties — output shape and freshness owner (Surface §"Refresh axis"): `batched` produces a partitioned table built forward in batches; `keyed`/`versioned`/`materialized_view` produce a keyed lookup whose rows carry state across all processed inputs. `keyed` and `batched` are not variants of one strategy — they differ in output shape (keyed vs partitioned), equivalence (end-state vs per-partition), and execution (ordered vs window-independent). `keyed` itself is not further split by pattern — the running-aggregate, latest-value, and milestone patterns share one contract and differ only in a per-column, SQL-derived combiner family (`keyed_models.md` §Design), which is exactly the litmus rule's fourth clause applied to what would otherwise have been three peer modes. Each keyed mode has its own spec (`keyed_models.md`, `versioned_models.md`, `materialized_view.md`); deeper rationale in `docs/research/20260703-model-updates.md` (Parts 1, 16, 17) and `docs/research/20260705-keyed-collapse-application.md`.
...
- **Names a reusable combiner/table shape without changing contract or shape** → a **function**, not a mode: a pattern like "latest value per key" or "first-observed milestone" is intent-naming sugar over an existing column family (e.g. `smelt.latest(value, ordering)` expanding to `MAX_BY`), not a new `refresh:` value — it earns no peer name because it changes neither the equivalence contract nor the output shape (`keyed_models.md` §Surface "The column-family catalogue").

Modelling window-forward-vs-snapshot-diff as a `strategy:` selector was rejected for the same reason a refresh `strategy:` was: a selector that silently swaps *what is scanned* invites the reader to believe it swaps *what the relation means*. The derivation is instead read off the driving source — a source with a `timeseries:` clock is consumed forward; a mutable one is diffed — so a `keyed` model over an append-only stream and one over a snapshot table are the same mode, differing only in a physical fact `smelt explain` reports (subject to the per-column admission matrix, `keyed_models.md` §"Admission matrix"). Full derivation: `docs/research/20260703-model-updates.md` Part 19.
...
- **Keyed refresh modes beyond `cumulative` (the built seed of `keyed`) are not fully built.** The refresh axis names `keyed`, `versioned`, and `materialized_view` as normative peers, each with its own spec, but only `full` and `batched` are fully built today alongside `cumulative` — the built seed that the keyed-collapse work renames and generalises into `refresh: keyed` (`keyed_models.md` §Known Divergences). Of today's non-`full` values that do not parse, `versioned` still does not parse — `RefreshStrategy` accepts only `full`/`batched`/`cumulative`/`materialized_view`, so declaring `versioned` produces an unknown-refresh-value error. `refresh: cumulative` itself is being renamed to `refresh: keyed`: declaring `cumulative` will become a hard config error pointing at `keyed` rather than a silent alias (`keyed_models.md` §Known Divergences; decision record `docs/research/20260705-keyed-collapse-application.md` D2). `materialized_view` parses but hard-errors on every current backend (none has native IVM; `materialized_view.md`). Delivery: `keyed` (the rename + family union) by `docs/plans/20260705-keyed-collapse.md`; `versioned` / `materialized_view` by `docs/plans/20260704-model-updates.md`.
...
- **Snapshot-diff (snapshot-reconcile) mechanics for keyed modes are specified for `keyed`, open for `versioned`.** `keyed_models.md` §"The two run shapes" now specifies the unclocked posture in full (a whole-scan reconcile, retained departed keys, `--event-time` flags a hard error, `--auto` always-stale). What the equivalent mechanics are for `versioned` remains open (`docs/research/20260703-model-updates.md` §19.8).
...
- **Related specs**:
  ...
  - `keyed_models.md` — the `refresh: keyed` mode (running-aggregate, latest-value, and milestone/retroactive-enrichment patterns, unified)
