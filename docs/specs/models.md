...
| `grain` | enum | — | **Check-only assertion**, never a driver: `partition` \| `key` \| `key_per_partition`. If written, it must match the grain derived from `(timeseries?, unique_key?, partition_column ∈ key?)`, else a hard error; it drives nothing. See Refresh axis. |
...
| `versioning` | enum (`interval`) | — | Admitted only where identity is declared and no clock is on the model: each key keeps every version with a validity interval (the SCD2 shape). See `incremental_models.md` §"Interval versioning (`versioning: interval`)". |
...
A model output that declares **identity** admits the sub-declaration `versioning: interval` — every version of a key is kept with a validity interval (the SCD2 shape; `incremental_models.md` §"Interval versioning (`versioning: interval`)"). It is deliberately not a shape of its own: row addressing is still by key; the interval is structure within the key.
...
| `grain:` (assertion) / `versioning` / `maintenance` / `safety_overrides` / `backfill` without `refresh: incremental` | Hard error |
...
| `versioning: interval` without a declared `unique_key:` | Hard error (SCD2 keeps versions *per key*) |
| `versioning: interval` + `timeseries` on the model | Hard error (the SCD2 close-out escapes every time window) |
...
- **The refresh trichotomy and `grain:` are parsed and enforced; the demotion of `grain:` to a derived label is spec-ahead-of-code.** `RefreshStrategy` is the `full`/`incremental`/`materialized_view` trichotomy; `batched`/`keyed`/`cumulative`/`versioned` as `refresh:` values are hard errors with a fix-it. Today `grain: partition | key | key_per_partition` is a parsed frontmatter key that is **required** with `refresh: incremental` and drives the shape — this spec instead makes the shape-defining facts (`timeseries:` / `unique_key:`) the required declarations and `grain:` a **check-only assertion** derived from them (§"Refresh axis", §"The Relation Contract"). Deriving the `grain` label from `(clock?, identity?, partition_column ∈ key?)`, admitting `refresh: incremental` on the facts alone (no `grain:`), and validating a written `grain:` assertion against the derived facts and plan, are not yet built. `maintenance:` and `columns.<c>.contract` are parsed frontmatter keys (`crates/smelt-core/src/metadata.rs`). Still missing: top-level `unique_key:`, `versioning:`, top-level `safety_overrides:`, the per-cell `maintenance.cells[].write` addressing pin, and a top-level `backfill:` block do not yet exist as frontmatter keys; the `batched:` sub-block this spec removes is still the live surface for `unique_key`/`safety_overrides` (`incremental_models.md`); `grain: key_per_partition` parses but has no dedicated validation or execution path yet. The `smelt migrate` assist for the hard cut does not exist. Migration ordering: `docs/research/20260705-refresh-as-maintenance-plan/08-code-placement.md` §2.8; `docs/research/20260716-relation-contract-and-per-cell-addressing.md`.
