...
- **`accumulating_snapshot` is a specified but not-yet-enumerated keyed peer.** An `accumulating_snapshot` refresh — a keyed once-write/milestone peer for late enrichment, consuming windowed input like `cumulative` with a bounded forward-attribution horizon — is normatively specified in its own spec (`accumulating_snapshot.md`), and the maintenance framework lists it among the keyed modes that uphold end-state equivalence (`model_maintenance.md`). It is one step **more** prospective than `versioned`/`latest_value`: it is **not yet part of the `refresh:` enum** above (Surface §"Refresh axis" and §"Three orthogonal axes") and is on a separate delivery track. `RefreshStrategy` accepts only `full`, `batched`, `cumulative`, and `materialized_view`, so declaring `refresh: accumulating_snapshot` currently produces an unknown-refresh-value error. Its worked design is `docs/research/20260703-model-updates.md` Part 20; delivery is tracked by `docs/plans/20260704-accumulating-snapshot.md`. It joins the enum and the Surface refresh-axis table when that plan lands.
- **Source mutation profile is inferred, not yet a first-class source declaration.** The input-consumption cell (Surface §"Input-consumption axis") is today derived from the *presence* of a `timeseries:` clock on the driving source: clocked ⇒ window-forward, otherwise ⇒ snapshot-diff. The one non-derivable world-fact — whether an un-clocked source is truly mutable vs merely a clock-less append log — has no dedicated `sources:` declaration yet. Until it exists, snapshot-diff is the conservative default for any un-clocked source, and the *observation-series* rejection (a non-replayable input under a partitioned output, `docs/research/20260703-model-updates.md` §19.6 cell 2) is deferred with it. Tracked in `docs/plans/20260704-model-updates.md`.
