...
- **`accumulating_snapshot` is a specified but unbuilt keyed peer.** An `accumulating_snapshot` refresh — a keyed once-write/milestone peer for late enrichment, consuming windowed input like `cumulative` with a bounded forward-attribution horizon — is a normatively specified mode (`accumulating_snapshot.md`), listed alongside the other keyed peers on the refresh axis. It is not yet built: `RefreshStrategy` accepts only `full`, `batched`, `cumulative`, and `materialized_view`, so declaring `refresh: accumulating_snapshot` currently produces an unknown-refresh-value error. The parser lags the spec here exactly as it does for `versioned` and `latest_value`. Its worked design is `docs/research/20260703-model-updates.md` Part 20; delivery is tracked by `docs/plans/20260704-accumulating-snapshot.md`.
