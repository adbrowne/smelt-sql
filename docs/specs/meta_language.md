...
- **Map method call on a non-identifier receiver does not yet parse.** §"Map API" admits any `Map<K, V>`-typed expression as a method receiver, including a loader call (`smelt.config.load_yaml(path, Map<Text, S>).entries()`). The parser currently emits `MAP_METHOD_CALL` only for an identifier receiver (`m.entries()`); a method call chained on a call expression is not yet recognised (the postfix position handles only array subscript), so the loader-call-receiver form is rejected at parse time. Together with build-path lowering of the loader family, this is tracked in `docs/plans/20260531-diagnostic-parity.md`.
