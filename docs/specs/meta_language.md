...
- **Consumers of generator-emitted models receive `Unknown`-typed columns.** The `emitted_model_typed_schema` Salsa query synthesises a typed `ModelSchema` from each emission's `ModelDef.body` (rule 7's `columns:` clause). The synthesised schema is not yet routed to the `SalsaRefSchemaProvider` branch that serves `FROM smelt.<emitted-path>` references in hand-authored or other emitted models — so a downstream `SELECT id FROM smelt.cohorts.us_west` types every column as `Unknown`. Consumer routing lands as a follow-on change; the production side (emission schema synthesis) is complete. Tracked in `docs/plans/20260528-generator-emission-schema.md`.

- **SQL-level inference gaps inside emission bodies do not emit diagnostics.** `check_file_diagnostics` skips SQL diagnostics for generator files (rule 4's W4 stage runs through a different code path that does not pass through the per-file diagnostic walk). A `ModelDef.body` containing an undeclared column reference, type mismatch, or other SQL-level inference gap produces a partially-typed `ModelSchema` via `emitted_model_typed_schema` but no diagnostic; the gap surfaces only when the emission's column is consumed downstream and that consumer fails to type-check. The no-silent-`Unknown` invariant (`types.md` §"Strict-by-default doctrine") is currently not satisfied for these cases. Tracked in `docs/plans/20260528-generator-emission-schema.md`.
