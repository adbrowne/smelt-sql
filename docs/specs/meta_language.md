...
`ModelDef` is a built-in closed meta record type with seven fields:
...
| `tags` | `List<Text>` | no | `[]` |
| `description` | `Text` | no | `''` |
| `timeseries` | `Record{…}` | no | inherits the generator's frontmatter `timeseries:` block |
| `safety_overrides` | `Record{…}` | no | inherits the generator's frontmatter `safety_overrides:` block |
...
- `timeseries` is a `Record{event_time_column: Text, partition_column: Text, granularity: Text, week_start: Text?, assert_monotonic: Bool?}` mirroring `TimeseriesConfig` (`incremental_shapes.md` §"The partition grain"); `event_time_column`, `partition_column`, and `granularity` are required sub-fields, `week_start` and `assert_monotonic` are optional. When present, it **replaces** the generator file's frontmatter `timeseries:` block in full for this emission only (whole-block replacement, not key-level merge — a partial override cannot silently keep a stale sub-field from the frontmatter block). Valid only when `materialization == 'incremental'`; present on any other materialization emits `ModelDefOverrideRequiresIncremental` at the field name.
- `safety_overrides` is a `Record{allow_window_functions: Bool?, allow_having: Bool?, allow_limit: Bool?, allow_subqueries: Bool?, allow_nondeterministic: Bool?, allow_distinct: Bool?}` mirroring `PartitionGrainSafetyOverrides`, every sub-field optional. When present, it replaces the generator file's frontmatter `safety_overrides:` block in full for this emission only. Valid only when `materialization == 'incremental'`; present on any other materialization emits `ModelDefOverrideRequiresIncremental` at the field name.

An unknown sub-field or a missing required sub-field inside `timeseries` / `safety_overrides` emits `RecordFieldUnknown` / `RecordFieldMissing` respectively, naming `ModelDef.timeseries` / `ModelDef.safety_overrides` as the enclosing type — the same diagnostic codes user-declared `smelt.record` literals use, per the closed-record discipline.
...
| `ModelDefOverrideRequiresIncremental` | `ModelDef.timeseries` or `ModelDef.safety_overrides` is present on a literal whose `materialization` is not `'incremental'` | `ModelDef.{field} is only valid when materialization is 'incremental'` |
...
- **Completion** at a `ModelDef { <cursor> … }` field-key position offers the closed seven-field set, with the required fields (`name`, `body`) surfaced first.
...
3. **`ModelDef` record literal evaluation.** A `ModelDef {…}` literal evaluates each field's value in the surrounding splice context and constructs a `ModelDef` value bound to the built-in closed seven-field set. The existing record-literal rules apply (required-field enforcement, duplicate-field detection, unknown-field rejection, per-field type-mismatch checking) via the same code paths that handle user-declared `smelt.record` literals. The defaults for optional fields (`materialization`, `tags`, `description`) are applied at value-construction time when the field is omitted from the literal; `timeseries` and `safety_overrides` instead fall back to the generator file's frontmatter blocks (§"`ModelDef` meta record type").
...
**Why `ModelDef` is user-constructible (unlike `ColumnRef` / `ModelRef` / `SourceRef`).** The reflection witnesses are produced exclusively by reflection accessors — they witness workspace state, they do not represent user intent. `ModelDef`, by contrast, is precisely user intent (the user's declaration of a model the workspace should contain). Restricting `ModelDef` construction to reflection would produce no `ModelDef` values; the closed record needs literal construction to be useful at all. The construction is scoped to generator-file bodies (via `ModelDefOutsideGeneratorFile`) so that `ModelDef` values cannot leak into non-shape contexts; the asymmetry with the other closed witnesses is contained.

**Why the closed field set opened to seven.** Research §4.10.4's `ModelDef` sketch listed `{name, body, materialization, description, tags}` plus "frontmatter-equivalent fields (sketch, not enumerated further)". v1 committed to exactly these five fields, paced by examples that demand more. The partition grain (`incremental_shapes.md` §"The partition grain") supplied the first concrete pressure: a single generator emitting multiple partition-grain models whose event-time columns differ cannot express that under one file-wide `timeseries:` frontmatter block. The set opens by exactly two fields, `timeseries` and `safety_overrides`, spelt like the frontmatter keys they override, with whole-block replacement (not key-level merge — a partial override cannot silently keep a stale sub-field) and incremental-only applicability enforced by `ModelDefOverrideRequiresIncremental`. Other frontmatter-equivalent keys (`owner:`, `backend_hints:`, `target:`) stay closed; the closed-record discipline matches `ColumnRef` / `ModelRef`'s rationale, and no residue bullet demands them yet. An emitted model's frontmatter-equivalent fields beyond the closed seven are inherited from the generator file's frontmatter (which applies file-wide to every emission) exactly as before.
...
- **Per-`ModelDef` frontmatter beyond the closed seven fields** — `owner:`, `backend_hints:`, `target:`, etc. must live in the generator's file-wide frontmatter (inherited by every emission) or wait for a spec edit that adds to the closed `ModelDef` field set. (`timeseries` and `safety_overrides` are per-emission overridable as of the seven-field set — §"`ModelDef` meta record type".)
