...
**Build-path execution.** Spread expansion is a compile-time rewrite that runs on the build/compile pipeline, not only during analysis: a spread in a **SELECT list** that the analyzer accepts is materialised into its comma-separated Data-World select items before code generation, so `SELECT id, ...[name, email] FROM t` compiles to `SELECT id, name, email FROM t` and no spread token in a SELECT list reaches the database engine (§Semantics — "Lists and spread" rule 10). The HOFs (`map`/`filter`/`reduce`), the pipe operator, lambdas, the meta-world ternary, and `smelt.config.var` are likewise evaluated at compile time on the build path and lowered to plain Data-World SQL — see their §Surface entries' "Build-path execution" notes. Both the analysis surface (the LSP, `file_diagnostics`) and the build enforce the same accepted surface, per `architecture.md` §"Diagnostic parity rule (analysis ↔ build)". Build-path lowering of spreads in the other valid positions (GROUP BY, ORDER BY, IN-lists, VALUES rows, function-argument positions), and of the reflection / loader families, is not yet implemented — see §"Known Divergences".
