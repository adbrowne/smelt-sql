...
**Build-path execution.** Spread expansion is a compile-time rewrite that runs on the build/compile pipeline, not only during analysis: a spread in a **SELECT list** that the analyzer accepts is materialised into its comma-separated Data-World select items before code generation, so `SELECT id, ...[name, email] FROM t` compiles to `SELECT id, name, email FROM t` and no spread token in a SELECT list reaches the database engine (§Semantics — "Lists and spread" rule 10). Both the analysis surface (the LSP, `file_diagnostics`) and the build enforce the same accepted surface, per `architecture.md` §"Diagnostic parity rule (analysis ↔ build)". Build-path lowering of spreads in the other valid positions (GROUP BY, ORDER BY, IN-lists, VALUES rows, function-argument positions) is not yet implemented — see §"Known Divergences".
...
- **Build-path execution covers SELECT-list spread only; other spread positions and other meta constructs are analyzer-validated but not yet lowered at build.** In-model list spread **in a SELECT list** is expanded to plain SQL on the build/compile path (§"Lists and spread" — Build-path execution), so a model using it both type-checks in the editor and executes. Not yet lowered by the compiler (analysis-clean but not yet buildable in-model — the construct reaches the engine verbatim): (i) spreads in the other valid positions (GROUP BY, ORDER BY, IN-lists, VALUES rows, function-argument positions); (ii) the remaining in-model constructs — HOFs (`map`/`filter`/`reduce`), the pipe operator, lambdas, the meta-world ternary, `smelt.config.var`, and the reflection / loader families. Tracked in `docs/plans/20260531-diagnostic-parity.md`.
