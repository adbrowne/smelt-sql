...
**Why generator bodies forbid `smelt.models.*`.** A generator body that references `smelt.models.with_tag('cohort')` introduces a dependency on workspace shape, which depends on every generator's emissions — a chicken-and-egg cycle. Two consistent designs were considered: (i) admit `smelt.models.*` inside generator bodies but freeze its result to hand-authored models only (a partial-shape rule); (ii) forbid `smelt.models.*` inside generator bodies entirely. Option (i) is more permissive but produces non-obvious results — a user reading `smelt.models.with_tag` inside a generator could reasonably expect the call to see all models, including this generator's emissions. Forbidding the call entirely produces a clear diagnostic at the call site (`GeneratorBodyForbidsModelReflection`) and a forward path: the user adjusts to use `smelt.sources.*` or literal paths. Option (ii) is the cleaner rule — generator bodies are *shape-determining*, model reflection is *shape-observing*, the two layers don't cross.
...
- **Multi-model production is normative in this spec; implementation is forthcoming.** The `generates: models` frontmatter directive, the `ModelDef` meta record type and its closed field set, the workspace-shape resolution pipeline (stages W1–W4), the ten new diagnostic codes (`GeneratesUnknownValue` through `GeneratorBodyForbidsModelReflection`), the `<generator>` anonymous-frame extension, and the LSP support for `ModelDef` literals are all defined normatively in §Surface and §Semantics above. Implementation (the `generates: Option<String>` field in `ModelMetadata`, the `SmeltType::ModelDef` variant and `MODEL_DEF_FIELDS` constant, the body-shape parser dispatch for generator files, the W1–W4 Salsa-cached shape-resolution pipeline, the generator-body reflection forbid, the collision detection, and the LSP `ModelDef` hover / completion / goto-def paths) has not yet landed. The killer demo `examples/per_cohort_union/` is the integration-test target. Tracked in `docs/plans/20260509-meta-language-overall.md` and (forthcoming) `docs/plans/20260509-meta-language-E2.md`.
