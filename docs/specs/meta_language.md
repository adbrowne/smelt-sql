...
- **Records, `Map<K,V>`, schema-typed config loaders not yet implemented.** The surface and semantics above for records (`smelt.record` declarations, inline record types, record literals, field projection, width subtyping) and for `Map<K, V>` (the closed method API: `m.entries()`, `m.keys()`, `m.values()`, `m.get(k)`, `m.has(k)`) are normative; the implementation, the eleven record / map diagnostic codes (`SmeltRecordRedefinition`, `RecordFieldUnknown`, `RecordFieldMissing`, `RecordFieldDuplicate`, `RecordFieldTypeMismatch`, `RecordLiteralUnknownTarget`, `RecordFieldNotProjectable`, `RecordFieldTypeForbidden`, `RecordCyclicDeclaration`, `RecordInDataWorld`, `MapKeyTypeNotText`, `MapApiUnknown`, `MapApiArityMismatch`, `MapApiNamedArgument`, `MapApiUnexpectedArgument`, `MapGetMissingKey`, `MapApiArgTypeMismatch`), the new `SmeltType::Record` and `SmeltType::Map` variants, the LSP hover/goto/completion paths, and the schema-typed YAML/JSON/TOML loaders (full surface in `meta_config_loading.md`) have not yet landed. Tracked in `docs/plans/20260509-meta-language-overall.md`.
