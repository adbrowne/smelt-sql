...
A run acquires an exclusive advisory lock on `.smelt/lock` for its duration, released on completion or error. A second smelt process attempting to acquire the lock while it is held fails loudly — the error names the PID recorded in the lock file (`"state locked by PID <n>"`) — rather than silently interleaving writes with the first process. The lock is project-wide (one lock file at `.smelt/lock`, not per-target), since layout migration and `meta.json` writes are project-wide operations even when a given run only touches one target's subtree. The lock is acquired once for the whole run and held for its entire duration regardless of how many models the run engine executes concurrently (`cli.md` §"Parallel execution with `--jobs`") — concurrent in-process model execution is not concurrent *state-writer* access: every write to a shared, whole-store file under `.smelt/targets/<target>/` (the interval ledger, the landed-delta store) is additionally serialized within the single locked run so two models finishing at the same moment cannot each load-modify-save the same file and drop one write. The reconciliation ledger is no longer one of these whole-store files — see §"Relationship to the reconciliation ledger" — its own transactionality is the engine's, not this lock's.
  ...
  "models": {
    "<model_name>": {
      ...
      "probes": [                           // omitted if empty; absent entirely on older manifests
        ...
        // outcome is "dispatched" | "skipped" | "baseline_established"
      ],
...
The run-state intervals this spec owns and the maintenance plan's **reconciliation ledger** — the `(output-region × column-group)` bookkeeping that records each region-group's processed-input vector, frontier watermarks for idempotent groups, and delta identities for additive groups (`incremental_models.md` §"The frontier record (reconciliation ledger)") — are **not the same mechanism and do not substitute for each other**. Run-state intervals are project-wide observability: they exist to answer "what has this project run, and where are the gaps" for humans and tooling, and a project may run at `state.mode: stateless` and forgo them entirely. The reconciliation ledger is **required correctness structure** for every `grain: key` (and `key_per_partition`) model maintained under a derived plan — it exists whenever the plan does, independent of `state.mode`, because it is what lets a fold-family technique detect a re-run (never fold a delta already in the entry's processed set) and lets a crashed run resume exactly. Neither reads the other: the reconciliation ledger is per-model, keyed by `(region × column-group)`. Both gradings are **engine-resident**, each write transactional with the write it records — the additive grading in `_smelt_ledger`, the frontier grading in `_smelt_frontier` — never `.smelt/`; this is its normative residency (`state.md` §"The residency rule"; structure and operations per `incremental_models.md` §"The frontier record (reconciliation ledger)"). A `.smelt/targets/<target>/reconciliation.json` left by a binary that predates this residency move is imported into the engine tables on the first run that can reach a ledger-capable backend, then removed; on a backend with no ledger builder (anything but DuckDB today) it is left untouched, and that run says so rather than silently dropping it. The interval ledger here is project-wide observability state, still under `.smelt/`. This spec also owns the **per-source landed-delta record** that forward propagation consumes (`sources.md` §"Landed-delta (derived, recorded)"; `incremental_models.md` §"The graph layer") — which partition intervals of a source landed, keyed by source address, recorded in run state alongside the interval ledger, not in the reconciliation ledger. A project could run `state.mode: stateless` with `grain: key` models and still get correct, ledger-enforced reprocessing refusal — the reconciliation ledger's presence is a property of the *plan*, not of the project's state posture. This spec continues to own the run-state **storage and serialisation** (`.smelt/` layout, manifest format, run IDs, landed-delta intervals); the reconciliation ledger's structure, grading, and operations are owned by `incremental_models.md`.

### Per-source watermark

The per-source landed-delta record (`landed_deltas.json`, §"`.smelt/` directory layout", above) carries a **watermark** field per source address: the point the bound propagation has already consumed through for that source. It is not a new state structure — it is a field on the existing landed-delta record, sharing that record's file, write path, transactionality, and classification (`state.md` §"The state-structure inventory": observability, same row as the landed-delta record).

- Written in the same locked, atomic write as the run's landed-delta record (§"Locking", §"Atomic writes"). It advances only on a run that completed **every** model consuming that source within the propagation graph; a selective run (one whose selection or propagation scope excludes any consumer of the source) never advances it, because an advanced watermark would silently drop that span for the unselected consumers. A failed run never advances it. The stall this implies for a routinely-selective project is accepted; the per-`(source, consumer)` alternative is recorded as rejected-for-now in `incremental_models.md` §Design.
- `state.mode: stateless` writes no watermark, exactly as it writes no landed-delta record. The absent-watermark behaviour is the **refuse-loudly leg** of the optionality rule (`state.md` §"The optionality rule"), not the degrade leg: a source named in `--since-upstream` with no `--landed` and no persisted watermark propagates nothing, and the refusal names the missing watermark (`incremental_models.md` §Surface "Run flags") — never a silent no-op and never an implicit full-graph recompute, whose cost is unbounded and unasked-for. The capability itself is not blocked: an explicit `--landed` (or an explicit full run) remains available; absence of the watermark withdraws only the convenience of omitting it.
- `smelt run --since-upstream` with no `--landed` for a source treats the span `watermark → now` as that source's landed delta; an explicit `--landed` always overrides. Where a recorded **observed-delta** record covers the span — an upstream maintained model, whose runs record one (`incremental_models.md` §"Observed deltas on model edges") — the span is refined to the recorded delta, read live. An **external source** has no observed-delta record (only smelt's own conditional-write path ever writes one), so the unrefined span itself propagates — the coarser, always-correct form (widen-never-narrow). A recorded **empty** delta over the span is a real fact — the "delta empty" no-op leg of the settle-bound × observed-delta composition (`incremental_models.md` §"Observed deltas on model edges") — distinct from an absent record, which leaves the coarser form (the run's written window for a model upstream; the unrefined span for an external source).
...
- **The implementation still keys intervals by calendar-date string.** §"Interval ledger" specifies RFC3339-instant keys; the shipped ledger records `"2026-01-01"`-style date keys and its arithmetic operates on calendar dates, so sub-day event-time boundaries coarsen to whole days in recorded coverage. The move to instant keys lands together with the scheduler's change to typed delta currency, since both rework the same bookkeeping (`docs/outcomes/20260816-scheduler-delta-signatures/outcome.md`). Decision record: `docs/research/20260816-open-questions-triage.md` item 15.
