...
- Written in the same locked, atomic write as the run's landed-delta record (§"Locking", §"Atomic writes"), advanced only on a run that completed the models consuming that source; a failed run never advances it.
- `state.mode: stateless` writes no watermark, exactly as it writes no landed-delta record; the absent-state behaviour is the existing degradation contract (`state.md` §"The degradation contract" / §"The optionality rule"): a consumer with no watermark recomputes the full dirty set for that source and reports why, never silently propagating nothing.
- `smelt run --since-upstream` with no `--landed` for a source reads `watermark → now` from the recorded **observed-delta** table live (`incremental_models.md` §"Observed deltas on model edges"); an explicit `--landed` always overrides. A recorded **empty** delta over that span is a real fact — the "delta empty" no-op leg of the settle-bound × observed-delta composition (`incremental_models.md` §"Observed deltas on model edges") — distinct from an absent record, which falls back to the run's written window.
