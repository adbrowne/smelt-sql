...
- **A missing `meta.json` denotes the legacy pre-versioning layout** — root-level `runs/`, `intervals.json`, `landed_deltas.json`, `schemas/` (no `targets/<target>/` nesting). The first locked open of `.smelt/` under a version-aware binary migrates a legacy layout to the current version: existing root-level artifacts move under `targets/<target>/` for the target of the run doing the migration, and `meta.json` is written with the current `state_version`. A legacy root-level `reconciliation.json`, if present, is not migrated — the reconciliation ledger is engine-resident (`state.md` §"The residency rule") and has no place in the versioned `.smelt/` layout. Migration happens under the advisory lock (see "Locking" below) so a concurrent second process cannot observe or act on a half-migrated layout.
...
A run acquires an exclusive advisory lock on `.smelt/lock` for its duration, released on completion or error. A second smelt process attempting to acquire the lock while it is held fails loudly — the error names the PID recorded in the lock file (`"state locked by PID <n>"`) — rather than silently interleaving writes with the first process. The lock is project-wide (one lock file at `.smelt/lock`, not per-target), since layout migration and `meta.json` writes are project-wide operations even when a given run only touches one target's subtree. The lock is acquired once for the whole run and held for its entire duration regardless of how many models the run engine executes concurrently (`cli.md` §"Parallel execution with `--jobs`") — concurrent in-process model execution is not concurrent *state-writer* access: every write to a shared, whole-store file under `.smelt/targets/<target>/` (the interval ledger, the landed-delta store) is additionally serialized within the single locked run so two models finishing at the same moment cannot each load-modify-save the same file and drop one write. The reconciliation ledger is out of scope for this lock — it is engine-resident and serialized by its own backend transaction (`state.md` §"The residency rule").

The run lock and the layout-version check are **backend-independent**: they live under `.smelt/` on the invoking machine, depend on no backend capability, and are therefore realised identically on every target, including a target that claims no engine-resident correctness structure (`state.md` §"The residency rule"). No target refuses them, and no target's absence downgrades them — they are not correctness structures. Both fire in `execute_project` before any state artifact is written, so a refused second run leaves `.smelt/` and the warehouse untouched. The lock protects `.smelt/`, not the warehouse: under `state.mode: stateless` nothing is written under `.smelt/`, so there is nothing to serialize, and `lock()` is an explicit, specified no-op — two concurrent stateless runs both proceed. This is the absence of *state*, not the absence of a lock; smelt does not serialize concurrent writers to a warehouse table at any posture.
...
```jsonc
{
  ...
  "models": {
    "<model_name>": {
      ...
      "probes": [                           // omitted if empty; absent entirely on older manifests
        ...
        { "fact": "contract.retain_departed", "probe": "ContractDepartedKeyUnmarked", "outcome": "dispatched", "observed": 3 }
      ],
  ...
  "external_steps": {                     // omitted entirely if no step ran this run
    "<step_address>": {
      ...
      "duration_ms": 1204,
      "outcome": "success"                // "success" (invoked) or "skipped" (declined-and-trusted, sources.md §Semantics 12); a failed step aborts before this exists
    }
  }
}
```
...
`external_steps` (`sources.md` §"Externally-produced sources (black-box steps)") is a map from step address to a record of every step this run **either invoked successfully or declined-and-trusted**: `command` is the resolved argv (placeholders substituted, e.g. `{run_date}`) for an invoked step, or empty for a declined-and-trusted one (nothing was spawned); `produces` is the list of source addresses the step declares; `duration_ms` is the invocation's wall time, or `0` for a declined-and-trusted step; `outcome` is `"success"` for an invoked step or `"skipped"` for one declined under `--skip-external-steps` (`sources.md` §Semantics 12) — a step's non-zero exit or refusal (`ExternalStepFailed`, `ExternalStepNotInvocable`) aborts the run before any manifest exists (see "Abort semantics" below), so a failed or refused step never appears here; it is surfaced by the run's own reporter output and exit status instead. The key is omitted entirely (not an empty object) for a run that invoked no step, so a manifest written before this field existed still round-trips.
...
Cumulative per-model interval coverage, keyed by RFC3339 UTC instants (half-open `[start, end)`), so coverage can be recorded at the same sub-day (hourly/second) boundaries incremental models filter on. Interval arithmetic (merge, gap detection) operates on instants; a whole-day interval is simply midnight-to-midnight. Consumed by incremental backfill and gap detection (`incremental_models.md`).

Each model's entry also carries `cell_frontiers`, a map from a `contract.cells[].deferral` cell's own stable address to its maintained frontier (a dated string, the same format as an interval bound) — the per-cell counterpart of the model-level covered-interval frontier `deferral`'s model-level scheduling already reads. Absent from the map means "never recorded" (a cell's first run) and licenses no skip, mirroring the model-level frontier's own "nothing recorded, never skip" posture. `#[serde(default)]` on read — a ledger written before this field existed deserialises with an empty map, needing no migration.
...
- **Recovery is idempotent re-run.** smelt does not checkpoint mid-run; recovery is re-running the same selection/range, which converges because each committed unit is idempotent (`incremental_shapes.md` §"First-run and backfill"). The interval ledger lets a re-run skip already-covered ranges and surface gaps.
...
- **The implementation still keys intervals by calendar-date string.** §"Interval ledger" specifies RFC3339-instant keys; the shipped ledger records `"2026-01-01"`-style date keys and its arithmetic operates on calendar dates, so sub-day event-time boundaries coarsen to whole days in recorded coverage. The move to instant keys lands together with the scheduler's change to typed delta currency, since both rework the same bookkeeping; that work is not yet scheduled (`docs/research/20260904-incremental-state-review.md` §"Recommended next sequence"). Decision record: `docs/research/20260816-open-questions-triage.md` item 15.
...
- **JSON files are the decided layout, not a placeholder.** Research §8 sketched an embedded `.smelt/state.db`; the decision is that plain JSON files (`runs/*.json`, `intervals.json`, `schemas/*.json`) remain the normative layout, reconsidered only if the virtual-environments snapshot/environment map lands and grows past what human-inspectable files serve well. Decision record: `docs/research/20260816-open-questions-triage.md` item 13.
...
- **`--auto`'s frontier detection reads only the local `.smelt/` interval store, which does not reconcile against a cloud target's backend-resident data.** A target's warehouse schema can already hold rows for dates the invoking process's `.smelt/targets/<target>/intervals.json` has never recorded — concretely, a scheduled job running with a fresh or Volume-resident `.smelt/` whose interval history was never populated, even though the target's physical schema already holds days of data landed by other means. `compute_auto_time_range` (`crates/smelt-cli/src/commands/run_setup.rs`) then finds no `latest` covered date and `--auto` refuses rather than picking a window; a bootstrap cannot simply default to a full refresh either, because a model parameterised on `{run_date}` still needs a concrete window to run against. Candidate directions — reconciling `--auto` against the target's own resident state, keying one interval store by physical location rather than target name so a job and a locally-run invocation of the same schema share history, deriving a bootstrap window from a declared source's own observed range, or having the invoking job hand `--auto` an explicit window it computed itself — are recorded in `docs/outcomes/20260912-databricks-dogfood-spine/outcome.md` §"Blocked", with none yet chosen as the general answer for every cloud target. That outcome's own dogfood spine unblocks itself with a narrower, local fix — seeding one target's interval history directly from its already-known ingestion history — rather than resolving this question generally; see `docs/outcomes/20260912-databricks-dogfood-spine/phases/11j-plan.md`.

## References

- **Code**: `crates/smelt-state/src/lib.rs` (`RunManifest`, `ModelRunRecord`, `TimeRangeRecord`, `generate_run_id`), `src/file_store.rs` (`.smelt/` reader/writer), `src/intervals.rs` (`IntervalStore`), `src/reconciliation.rs` (`ReconciliationLedger` in-memory representation; storage is engine-resident, not this module's file store — see `state.md` §"The reconciliation ledger"), `src/landed_deltas.rs` (`LandedDeltaStore`, `SourceLanding`, per-source landed-delta recording), `src/schema_tracking.rs` (`DeployedSchema`), `src/history.rs` (history queries)
...
- **Related specs**: `state.md` (the state-ownership doctrine: correctness/observability classification of everything this spec stores, and the degradation contract for its absence), `incremental_models.md` (interval semantics, idempotent recovery; the transactional merge ledger — required correctness structure, distinct from this spec's opt-in observability), `schema_evolution.md` (deployed-schema snapshots and migration), `virtual_environments.md` (the snapshot/environment consumers), `output_fingerprint.md` (the equivalence key), `architecture.md` (`state.mode` surface, `smelt-state` crate)
