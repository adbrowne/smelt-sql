...
- **The shipped store still writes the reconciliation ledger under `.smelt/`.** Both storage gradings persist to `.smelt/targets/<t>/reconciliation.json` (`crates/smelt-state/src/reconciliation.rs`) rather than the engine-resident table this spec now describes normatively. Tracking: `docs/outcomes/20260904-state-residency/outcome.md` (criterion 1).

## References

- **Code**: `crates/smelt-state/src/lib.rs` (`RunManifest`, `ModelRunRecord`, `TimeRangeRecord`, `generate_run_id`), `src/file_store.rs` (`.smelt/` reader/writer), `src/intervals.rs` (`IntervalStore`), `src/reconciliation.rs` (`ReconciliationLedger`, `ReconciliationStore` — the file this outcome's move deletes, see §Known Divergences), `src/landed_deltas.rs` (`LandedDeltaStore`, `SourceLanding`, per-source landed-delta recording), `src/schema_tracking.rs` (`DeployedSchema`), `src/history.rs` (history queries)
