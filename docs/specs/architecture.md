...
- **Substrate unification between backbuild and the walk/maintenance layers is in progress.** The crate-wide walk rule, the single definition-diff engine, and backbuild's membership in the single-owner emitter families (all §"Property composition walk rule" and §"Constraints & Invariants" #12) describe the target state; today `crates/smelt-logical/src/backbuild/` re-derives column provenance, discriminated-union recognition, and functional-dependency checks beside the walk, carries its own definition-diff representation beside `analysis/model_diff.rs`, and its emitters sit outside the `statement_parity` gate (structurally unavoidable while backbuild is unwired — there is no executed statement to compare). Tracking plan: `docs/plans/20260808-substrate-unification.md`.
