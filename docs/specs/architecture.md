...
For the Spark printer, additional rewrites apply (QUALIFY → subquery, `ARRAY[…]` → `ARRAY(…)`, `x::T` → `CAST(x AS T)`, `DATE 'lit'` → `DATE('lit')`, trailing-comma stripping, further function-name remaps such as `UNNEST` ↔ `EXPLODE`, `BOOL_OR` → `SOME`). Identity for those dialects holds only *modulo those rewrites*; they are not the byte-identity target. The capability matrix lives in `crates/smelt-dialect/src/dialect.rs::BackendCapabilities`.
...
14. **Function-registry single ownership.** A built-in SQL function's *name*, *classification* (aggregate / window / scalar), and — for functions on the registry-driven typing path — its *inferred type* **and its per-dialect, per-position emission** have exactly one authoritative home: the `BuiltinRegistry` in `crates/smelt-types/src/signatures.rs`. Recognition (whether a call names a known SQL function), the `ExprKind` seed used by the expression-kind checker, registry-first type inference, and per-dialect, per-position emission all derive from that one table, so a name added to the registry is automatically recognised, classified, typed (once migrated), and emitted correctly — and a name absent from it is diagnosed (`UnrecognizedFunction`), never half-known. Emission verdicts are keyed on `(DialectId, Position)` — `Any`/`Scalar`/`Aggregate`/`WholePartitionWindow`/`Window` — because a backend's support for a built-in routinely differs between positions, and lookup never falls back from one position to another (`multi_backend.md` §"Emission is scoped to call position"). A verdict is one of `Native`, `Rename`, `Template`, `Restructure`, `Rewrite`, `Unsupported`, or `Conditional`: a `Template` is registry *data* — a `{n}`-placeholder spelling over the call's own positional arguments — interpreted by one generic printer routine that knows no function names (`multi_backend.md` §"Template emission"), and a `Conditional` is an ordered arm list ending in a mandatory `otherwise` arm, **settled on the compile path** from the projection's own type inference before the printer ever runs, so the printer receives only the settled verdict and holds no type context (`multi_backend.md` §"Operand-conditional verdicts"). CI gates enforce this; they must not be lowered without a reviewer sign-off note:
    ...
    - **Build-time validation gate** (`cargo test -p smelt-types --test registry_coverage`) — a `Template`'s placeholder range, argument coverage, and variadic rejection, a template's call-shape at a window position, and a `Template` verdict used as a `Conditional` arm are all validated when the registry is built, so a malformed template or an unresolved-arity conditional entry is a build-time failure, never a runtime one.
