...
- **Diagnostic range encoding rule is not yet upheld.** `smelt_db::Diagnostic` carries `crate::Range` (line/column) today, computed via `smelt_parser::ast::offset_to_position` which counts codepoints labelled as "column". The conversion runs inside every `_for_select` diagnostic helper, every Salsa accumulator, every emission-body translation helper (`shifted_range`, `shifted_body_text_range`, `remap_body_range`, `body_position_to_byte`). LSP clients receive codepoint columns regardless of negotiated `positionEncodingKind`; non-ASCII columns drift from byte and UTF-16 positions. The rule above is the target state; the migration is tracked in `docs/plans/20260530-byte-offset-diagnostics.md`, which lifts conversion to the LSP / CLI boundary, introduces `line-index`, and lands LSP encoding negotiation.
