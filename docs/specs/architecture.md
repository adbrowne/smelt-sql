...
The shared pre-execution gate (§"Diagnostic parity rule (analysis ↔ build)") enforces parity structurally: both `smelt-cli` and `smelt-ui` must pass the same `Error`-severity `file_diagnostics` check before any model is compiled or executed, making it impossible for a consumer to silently skip a lifecycle stage the other one runs. A dedicated dual-consumer fixture test (`execute_parity`) that runs the same project through both entry points and asserts identical model outputs, manifest contents, and selection sets would strengthen this further, but is not yet implemented — it is tracked as future work in Known Divergences. Always run `cargo test -p smelt-runtime` when touching the compile pipeline, the execute loop, or either consumer's run path.
...
- **Address-uniqueness across kinds is specified but not yet enforced.** §Resolution makes two files resolving to the same `smelt.<path>` a hard workspace-load error, and §"Workspace loading parity rule" describes the `project_address_collisions` query intended to enforce it. Today no such enforcement runs for cross-kind collisions: a model and a seed (or source) that resolve to the same address are silently reduced to one entity, dropping the other with no diagnostic. The `project_address_map` / `project_address_collisions` queries and the `DuplicateAddress` diagnostic that close this are tracked in `docs/plans/20260605-address-collision.md`.
- **Diagnostic parity (analysis ↔ build) — residual gaps after `docs/plans/20260531-diagnostic-parity.md`.** The plan closed the primary parity drift (P2–P7d, June 2026): the CLI run/build path now runs the full `Error`-severity `file_diagnostics` gate via the shared `gate_diagnostics` helper (not just `UnknownSmeltFn`); `execute_project` runs the same gate; the built-in planner rules (cumulative classifier, incremental batch-safety/bounds) surface through a uniform rule → diagnostics interface and are visible to both the editor and the build; in-model meta constructs (spread, HOFs, `columns_of`, `config.var`, wide reflection, config loaders) are evaluated at compile time and lowered to plain SQL. Residual gaps not in scope of that plan: (1) the `execute_parity` CI gate (a single-entry-point `execute_project` migration for the CLI run path) remains future work — the shared gate provides parity without the migration; (2) user-authored planner rule registration (extensibility) is future work — built-in rules already use the uniform interface and any new rule inherits parity by construction.
