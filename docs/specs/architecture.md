...
14. **Function-registry single ownership.** A built-in SQL function's *name*, *classification* (aggregate / window / scalar), and — for functions on the registry-driven typing path — its *inferred type* **and emission** have exactly one authoritative home: the `BuiltinRegistry` in `crates/smelt-types/src/signatures.rs`. Recognition (whether a call names a known SQL function), the `ExprKind` seed used by the expression-kind checker, registry-first type inference, and per-dialect emission all derive from that one table, so a name added to the registry is automatically recognised, classified, typed (once migrated), and emitted correctly — and a name absent from it is diagnosed (`UnrecognizedFunction`), never half-known. Two CI gates enforce this; they must not be lowered without a reviewer sign-off note:
