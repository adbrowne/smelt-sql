...
- **Maintenance-plan purity has no structural gate yet.** The rule (§"Constraints & Invariants" #12) is upheld by convention today — `crates/smelt-logical/src/maintenance/` is a tracer with no production consumer, so there is nothing yet to check for re-derivation. The CI gate lands with the production consumers in `docs/plans/20260707-maintenance-plan-impl.md`.
