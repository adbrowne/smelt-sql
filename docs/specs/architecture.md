...
**File kind is grammar, not location.** `data/foo.sql` containing a bare SELECT is a model addressable as `smelt.data.foo`; `random/x.sql` declaring `smelt.define helper(...)` is callable as `smelt.random.x.helper(...)`. The recommended layout is convention; the resolver only cares about path and content.
...
- **Universal `smelt.<path>` resolution is pre-implementation.** The current parser still recognises `smelt.ref(...)`, `smelt.source(...)`, and `smelt.fn.<path>(...)` as distinct constructs. The path-syntax migration touches the lexer, parser, AST, dialect printer, LSP completion, and `crates/smelt-parser-compat/`. Downstream specs (`functions.md`, `scoping.md`, `types.md`, `incremental_models.md`, `planner_integration.md`) have been aligned with the unified scheme; examples under `examples/` will be migrated as part of the implementation plan. (Spec updated 2026-05-01; downstream specs aligned 2026-05-01.)
