...
- **Dialect conformance gates (§Constraints #13) are aspirational in part.** The accept-direction differential harness exists (`smelt-parser-compat` vs pg_query/sqlparser-rs/sqlglot) but does not yet use a real DuckDB execution oracle; the fidelity-direction (round-trip-executes) gate, the external-corpus suite, and the type-oracle strictness rules (exact width/precision comparison, known-unknowns ledger) are not yet implemented. Known DuckDB constructs that currently fail to parse include `TRY_CAST`, `GROUP BY ALL`/`ORDER BY ALL`, `IGNORE NULLS`, SQL-standard function forms (`trim(BOTH … FROM …)`, `substring(x FROM i FOR n)`, `position(s IN x)`, `overlay(…)`), `LIKE ANY`, dollar-quoted strings, list comprehensions, and `MAP` literals; known silent mis-lexes include hex literals, underscore digit separators, `E'…'`/`B'…'` strings, `INTERVAL 3 MONTH`, and the `GLOB` operator. Tracked in `docs/plans/20260711-parser-type-testing-hardening.md`.
