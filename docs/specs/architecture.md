...
- **Maintenance-plan purity has no structural gate yet.** The rule (§"Constraints & Invariants" #12) is upheld by convention today. The *plan* half now has production consumers (`smelt-db` diagnostics, `smelt-runtime` technique lowering, the propagation layer — all reading `derive_maintenance_plan`'s output), but no mechanical check yet asserts they are the only admission callers. The *statement-emission* half is not yet single-owner at all: production maintenance SQL is authored in the backends and `smelt-runtime` rather than by `smelt-logical`'s emitters (`maintenance_plan.md` §Known Divergences "Statement emission is not yet single-owner"); unification and the statement-parity CI gate are tracked in `docs/plans/20260710-emit-unification.md`.
