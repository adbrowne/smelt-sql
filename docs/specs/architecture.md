...
- **The pre-execution diagnostic gate currently blocks on only `UnknownSmeltFn`, not the whole `Error` set.** The §"Diagnostic parity rule (analysis ↔ build)" verdict ("fail on any `Error`-severity diagnostic, over the executed SQL, through one shared gate for both consumers") is not yet met: the CLI run path filters `file_diagnostics` to `UnknownSmeltFn` only, the `smelt-runtime` `execute_project` path runs no gate at all, the CLI does not yet route through `execute_project` (so neither the `execute_parity` nor the `example_builds` CI gate exists), the built-in planner rules do not yet expose their checks through a uniform rule → diagnostics interface and `file_diagnostics` does not aggregate them (so the cumulative-aggregate classifier and incremental batch-safety/bounds verdicts are strict on the build/dispatch path but invisible to the editor), and several analysis-accepted constructs do not survive codegen — nested `smelt.define` calls, block `PASSING` fragment arguments, and in-model meta-language constructs (list spread, HOFs, `columns_of`, `config.var`, `models.with_tag`) are emitted as invalid engine SQL or silently mis-materialized. Tracked in `docs/plans/20260531-diagnostic-parity.md`.
