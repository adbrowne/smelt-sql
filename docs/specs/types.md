...
`TypeMismatch`, `CannotInferType`, `UnknownCastType`, `UnrecognizedFunction`, `SourceTypeError`, `WindowInScalarContext`, `AmbiguousColumn`, `UndeclaredColumn`, `ColumnTypeUnresolved`, `ArgTypeMismatch`, `FunctionBodyTypeMismatch`, `ReturnTypeMismatch`, `InvalidFunctionTypeRef`, `MissingArgument`, `RowRequirementUnsatisfied`, `FragmentColumnMissing`, `AnnotationTooWide`, `FragmentKindMismatch`, `ParameterShadowsColumn` (warning), `AliasColumnArityMismatch`, `EmptyValuesClause`, `DecimalPrecisionOverflow`, `NonPortableCollation`.

`NonPortableCollation` — emitted when portable code declares or uses a non-binary collation on a string (§17). Anchored at the `COLLATE` clause span (or the source-declared collation). Recovery: the comparison degrades to `Unknown` (reason `Unresolved`). The user must compare byte-wise (the default binary collation) or declare an engine on the model to use the engine's native collation.
...
**`AT TIME ZONE` conversion operator.** The SQL expression `expr AT TIME ZONE zone_str` converts `Timestamp → Timestamp WITH TIME ZONE` and `Timestamp WITH TIME ZONE → Timestamp` — the operator toggles the `with_timezone` axis; the timezone name itself is not tracked in the type. It parses as a left-associative postfix operator binding tighter than comparison and arithmetic (`ts AT TIME ZONE 'UTC' > ts2` groups as `(ts AT TIME ZONE 'UTC') > ts2`), so it chains: `ts AT TIME ZONE 'UTC' AT TIME ZONE 'EST'` groups as `(ts AT TIME ZONE 'UTC') AT TIME ZONE 'EST'`. Nullability propagates from the operand. Only the `Timestamp` operand round-trip is covered; an operand of any other type infers `Unknown`. Cross-engine emission for backends without a native `AT TIME ZONE` (Spark uses `to_utc_timestamp`/`from_utc_timestamp` instead) is not yet implemented — see Known Divergences.
...
Collation governs string comparison, ordering, and **equality** — and because it governs equality, it governs which rows survive `GROUP BY`, `DISTINCT`, joins, and `MIN`/`MAX`, not merely their presentation order.

**Collation is a value-domain axis.** Each string type (`Text`, `Varchar`, `Char`) carries a collation; it lives in `DataType`, not in `TypedColumn`, and participates in type equality, promotion, and `CAST` — consistent with the axis-placement rule in §Design and with how DuckDB and Spark model collation (both treat collation as part of the string type). The default collation is `Binary` (byte-wise comparison, case- and accent-sensitive, normalisation-sensitive). String-unification (§4) is unchanged for binary-collated strings: `Text`, `Varchar(_)`, `Char(_)` remain interchangeable when their collation matches.

**Binary is the only portable collation.** The portable surface guarantees one string-comparison semantics on every engine: byte-wise (`Binary`). `Binary` is identical and version-stable across DuckDB (`BINARY`, the default), Spark (`UTF8_BINARY`, the default), and Postgres (`COLLATE "C"`). Under binary collation, `=`, `<`, `ORDER BY`, `GROUP BY`, `DISTINCT`, joins, and `MIN`/`MAX` on strings produce identical results on every engine.

**Non-binary collation is not in the portable surface.** Declaring or using any non-binary collation (case-insensitive, accent-insensitive, locale-aware) in portable code is a `NonPortableCollation` error. The diagnostic directs the user to either compare byte-wise (the default) or declare an engine on the model. The two collations of a comparison must match: combining two different collations is a `TypeMismatch` (mirroring DuckDB's "cannot combine types with different collation") — but in portable code every string is `Binary`, so this rule only governs engine-bound and source-declared collations.

**Binary is pinned on emission.** Because Postgres's default collation is the database locale (not binary), `to_backend_sql()` must pin `COLLATE "C"` on Postgres for portable string comparisons and columns; without the pin, "portable" code silently inherits a locale-dependent comparison on Postgres. DuckDB and Spark default to binary, but pinning `BINARY` / `UTF8_BINARY` makes the contract robust against a session that changed its default collation.

**Engine-bound models get native collation.** A model with an engine declaration opts into that engine's native collation machinery (Postgres ICU/libc collations, Spark `UNICODE_CI`, DuckDB `NOCASE`, …). Its materialised output columns carry the engine's collation; downstream portable consumers compare them `Binary` unless they too are engine-bound. (Engine declarations are a deferred feature — see Known Divergences.)

**Soundness oracle.** `cargo test -p smelt-db --test type_property_tests` must cover collated string columns: portable (`Binary`) comparison must agree across engines, and any engine-bound collation divergence is either fixed or registered `ByDesign` in `divergences.rs`.
...
**Collation: binary is the portable surface, non-binary requires an engine.** The three engines model collation incompatibly — Spark 4.0 puts it in `StringType`, DuckDB attaches it to the type via `COLLATE`, Postgres derives it as an expression property via coercibility — and they disagree on the default (binary on Spark/DuckDB, the database locale on Postgres), on named-collation vocabulary, and on the ICU/Unicode version backing locale collations. The one collation that is bit-identical and version-stable across all three is **binary** (byte-wise), so the portable surface exposes only that; non-binary collation is rejected (`NonPortableCollation`), exactly as `Decimal / Decimal` is rejected (§15) — *refuse rather than silently approximate*. *Synthesising a portable case-insensitive or locale-aware collation* (mapping smelt's chosen semantics onto each engine's native collation) was rejected for the same reason as decimal division: "case-insensitive" folds differently across engines and Unicode versions (`'ß'`/`'SS'`, Turkish dotted-I, accent folding), so smelt would emit code that agrees approximately but not row-for-row, and collation's effect is on *equality* — it changes which rows exist, so approximate agreement is silent data divergence, not a rounding difference. The decision that makes this load-bearing rather than cosmetic is **pinning binary on emission**: Postgres's default collation is the database locale, so without an emitted `COLLATE "C"` a "portable" comparison silently inherits locale semantics on Postgres — the collation analogue of the decimal ANSI-mode deployment contract, but one that bites in the language surface. The long-term ergonomic answer — a library function with a portable signature and engine-dispatched bodies, or a canonical normalise-to-binary-key fold for grouping — is sketched in the research doc (`docs/research/20260612-collation-type-system.md` §8) and not committed here.
...
- **Out of scope for v1**: nested composite nullability (struct fields, array elements — conservatively nullable); multiple row variables per function; user-defined polymorphism in `smelt.define`; engine-bound decimal division (the `/` operator on `Decimal` operands is rejected in portable code — see §15).
...
- **`AT TIME ZONE` parses, prints, and infers types; cross-engine emission is not yet implemented.** The SQL expression `expr AT TIME ZONE zone_str` converts between naive and tz-aware timestamps: `Timestamp AT TIME ZONE 'zone' → Timestamp WITH TIME ZONE`; `Timestamp WITH TIME ZONE AT TIME ZONE 'zone' → Timestamp`. smelt parses it (a dedicated `AT_TIME_ZONE_EXPR` AST node), round-trips it losslessly, and infers the toggled `with_timezone` type. The operator is native syntax on DuckDB and PostgreSQL, so those backends need no rewrite on emission; Spark has no `AT TIME ZONE` syntax and uses different functions (`to_utc_timestamp`, `from_utc_timestamp`) instead, so a portable Spark-targeting mapping still requires a design pass. Only a `Timestamp` operand is typed (`Date`/`Time`/other operands infer `Unknown`, matching DuckDB's own narrower acceptance for those types). Escape hatches for Spark today: an explicit `CAST` or the `TIMEZONE(zone, expr)` scalar function (not yet registered; available on DuckDB and Spark with differing argument semantics).

- **Collation is enforced as binary-only in portable code; richer collation is deferred.** §17's portable contract is enforced: a `COLLATE` clause naming a non-binary collation emits `NonPortableCollation` and the expression degrades to `Unknown`, while binary collation names (`C`, `POSIX`, `BINARY`, `UTF8_BINARY`, case-insensitive) pass through with the operand type unchanged. The following remain deferred. The **value-domain collation field on the string `DataType` variants** and **engine-bound native collation** (including the collation-mismatch `TypeMismatch` rule) land with the engine-declaration feature — portable code is uniformly binary, so no portable consumer needs to store a collation, exactly as engine-bound decimal division is deferred (§15). **Pinning `COLLATE "C"` on PostgreSQL emission** lands with the Postgres runtime backend; until then smelt is **binary-by-omission** — sound on DuckDB and Spark (whose defaults are binary) but a generated Postgres comparison silently inherits the database locale. **Folding collation into the output fingerprint** (`output_fingerprint.md` treats it as breaking-by-default / conservative rebuild), an **ordering-only carve-out** (whether `ORDER BY x COLLATE …` for final presentation is allowed in portable code), **pattern matching** (`LIKE`/regex) under collation, and the **library-polyfill path** for portable non-binary collation are likewise deferred. Design rationale and the per-engine survey live in `docs/research/20260612-collation-type-system.md`; the staged delivery is tracked in `docs/plans/20260613-collation-axis.md`.
...
- **Silent `Unknown`s still exist for the meta-language surface.** `ColumnTypeUnresolved` is a live, catalogued code (`diagnostics.md`) and the rule that emits it is wired — it fires whenever a resolved schema column carries `Unknown` with reason `Unresolved`. The `Unknown` reason-discriminant (`Unresolved`/`Dynamic`/`Propagated`) is implemented in `smelt-types`. What remains is that several inference sites still produce `Unknown(Dynamic)` where a well-typed result would be preferable — these are not `Unresolved` and therefore do not fire `ColumnTypeUnresolved`, but they are still silent gaps: generator-emitted and `smelt.columns_of`-reflected model schemas, and meta-language HOF values placed in SQL column position (`meta_language.md`). Function-derived columns (`smelt.functions.*` struct-spread and `TableExpr` results) resolve — see `function_schema_inference.md` — and are no longer a silent-`Unknown` source. `(VALUES …) AS t(c₁, …)` derived-table columns resolve to concrete types. Cross-family binary arithmetic (`42 + '3'`, `TRUE + 1`) is enforced: the result degrades to `Unknown(Unresolved)` and a `TypeMismatch` Error is emitted at the operator span. The composite-path gaps (array-literal element unification and `UNION` branch unification with mixed families) still degrade to `CannotInferType` without a `TypeMismatch`. The remaining meta-language gaps are tracked in `docs/plans/20260519-functions-meta-gaps.md`.
