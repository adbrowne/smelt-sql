...
- **Nullability oracle covers single-table queries only.** The value-based DuckDB oracle property test in `crates/smelt-db/tests/nullability_property_tests.rs` exercises single-table `SELECT` queries. The outer-join rule (§11) is verified by the deterministic regression `regression_left_join_declared_not_null`. Broader property-test coverage for two-table joins and set operations (`UNION`/`INTERSECT`/`EXCEPT`) is tracked in `docs/plans/20260610-nullability-soundness.md`.
