...
- **Collation tracking is not yet implemented.** §17 specifies the target contract; no code path tracks collation today. `DataType` carries no collation field, `to_backend_sql()` emits no `COLLATE` clause, and there are no `NonPortableCollation` or collation-mismatch diagnostics. The practical consequence is that smelt is **binary-by-omission**: accidentally correct on DuckDB and Spark (whose defaults are binary) but **silently locale-dependent — and therefore unsound — on Postgres**, whose default collation is the database locale. The binary-pin-on-emission rule (§17) is what closes that gap. Until the axis lands, the following are also deferred: folding collation into the output fingerprint (`output_fingerprint.md` treats it as breaking-by-default / conservative rebuild); an ordering-only carve-out (whether `ORDER BY x COLLATE …` for final presentation is allowed in portable code); pattern matching (`LIKE`/regex) under collation; engine-bound collation (gated on the engine-declaration feature, as with decimal division); and the library-polyfill path for portable non-binary collation. Design rationale and the per-engine survey live in `docs/research/20260612-collation-type-system.md`.
