...
- **Signature nullability is not yet implemented.** The `NOT NULL` qualifier surface in §11 (parser acceptance, call-site/return/row checking, non-nullable parameter binding) does not exist in code yet; all signatures are currently implicitly nullable. The hover `NOT NULL` suffix and the shared canonical type renderer are likewise pending. Tracked in `docs/plans/20260610-nullability-soundness.md`.
- **Outer-join nullability is unsound.** Inference does not apply §11's null-supplying-side rule: a column declared `nullable: false` in a source retains `nullable: false` after appearing on the null-supplied side of an outer join, violating the sound-upper-bound contract. Tracked in `docs/plans/20260610-nullability-soundness.md`.
- **Non-nullable-claim audit is pending.** Several rules that produce `nullable: false` have not been verified against §11's closed enumeration. Known unsound constructs that currently over-claim non-nullable include: `TRY_CAST`, `NULLIF`, `CASE` without `ELSE`, `LAG`/`LEAD` without an explicit default, and certain builtin-registry entries. The single-table value-based oracle (`crates/smelt-db/tests/nullability_property_tests.rs`) is the standing gate that catches these; the full audit is tracked in `docs/plans/20260610-nullability-soundness.md`.
- **Nullability oracle covers single-table queries only.** The value-based DuckDB oracle property test in `crates/smelt-db/tests/nullability_property_tests.rs` exercises single-table `SELECT` queries. Coverage for two-table joins (verifying the outer-join fix) and set operations (`UNION`/`INTERSECT`/`EXCEPT`) extends in a follow-up tracked in `docs/plans/20260610-nullability-soundness.md`.
...
- `crates/smelt-db/tests/nullability_property_tests.rs` — value-based DuckDB oracle for nullability soundness (§11 verification gate, single-table coverage)
