...
- **Non-nullable-claim audit is pending.** Several rules that produce `nullable: false` have not been verified against §11's closed enumeration. Known unsound constructs that currently over-claim non-nullable include: `TRY_CAST`, `NULLIF`, `CASE` without `ELSE`, `LAG`/`LEAD` without an explicit default, and certain builtin-registry entries. The value-based oracle (`crates/smelt-db/tests/nullability_property_tests.rs`) covers single-table queries, two-table joins (all four join types), and set operations (`UNION`/`INTERSECT`/`EXCEPT`) and is the standing gate that catches these; the full audit is tracked in `docs/plans/20260610-nullability-soundness.md`.
