...
>
> **Status: experimental (not yet implemented).** The profile is specified ahead of implementation; `grain: key` parses, but the `versioning: interval` sub-declaration does not parse today (§Known Divergences). Delivered by a phase of `docs/plans/20260707-maintenance-plan-impl.md`.

## Surface

### Composition

Per the composition contract (`maintenance_plan.md` §"The composition contract"), the SCD2 profile is composed as:
...
| **Invariant upheld** | end-state equivalence in its **interval-keyed specialisation** — the user-visible set of `(key, version, validity interval)` rows equals what a full rebuild would compute from the same processed snapshots, independent of merge order (§Semantics) | `maintenance_plan.md` §"The equivalence invariant", `maintenance_plan.md` §"Per-cell admission" |
...
The profile upholds the **end-state equivalence invariant** in its interval-keyed specialisation (`maintenance_plan.md` §"The equivalence invariant"): the user-visible set of `(key, version, validity interval)` rows equals what a full rebuild would compute from the same set of processed snapshots, independent of the order in which non-overlapping snapshots were merged. smelt owns freshness (pull) — the history is correct as of the last `smelt build`.
...
5. **End-state equivalent and order-independent** (`maintenance_plan.md` §"The equivalence invariant"). Merging non-overlapping snapshots in any order converges to the same version history.
...
- **Related specs**:
  - [`maintenance_plan.md`](maintenance_plan.md) — the equivalence invariant (interval-keyed variant), the algebraic ladder, the composition contract, validator-not-chooser; the plan matrix, per-cell admission, and the graph layer this profile's admission instantiates
