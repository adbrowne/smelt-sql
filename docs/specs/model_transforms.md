...
| Succession-patch keyed `MERGE` | keyed-succession classification (`model_properties.md`) + an `append_only` driving source | in one transaction: record each event's `(k, t, is_delete)` in the model's event skeleton, insert each non-delete event's presented row, and patch its skeleton-located immediate predecessor's `LEAD`-derived columns and successor's `LAG`-derived columns (`valid_to`, `is_current`-shaped columns); a delete event contributes the skeleton record and neighbour patches but no presented insert; idempotent on `(k, t)`, so a re-folded window is a no-op (`incremental_shapes.md` §"The succession grain") | unbuilt |
