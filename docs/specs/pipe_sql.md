...
The dialect layer carries a `supports_pipe_syntax` capability per backend (`BackendCapabilities`, `architecture.md` §"Identity properties"). When a backend advertises native pipe support, the printer emits the pipe query directly — every stage verbatim, in source order — instead of the lowered form; otherwise it emits the lowered standard SQL. BigQuery advertises native support and takes the passthrough path; DuckDB, Spark and PostgreSQL report `false` and take the lowering path. Both forms compute the same relation: the same pipe query is executed on a live GoogleSQL warehouse (natively emitted) and on DuckDB (lowered) and asserted to produce identical rows. Backends whose native pipe dialect omits an operator (e.g. Spark omits `RENAME`) may pass through the supported operators and lower only the unsupported ones.
...
- **Native passthrough beyond BigQuery.** BigQuery is the only backend emitting pipes natively. Databricks and DuckDB-via-extension also have native pipe dialects; both remain on the lowering path because neither reports the capability.
- **A pipe query cannot read a decomposed-state (rung 2) model.** The presentation projection that hides a state-bearing model's `__part` columns rewrites a SELECT list, and a pipe query has none to edit, so such a model is refused at compile time naming the form rather than silently presenting state columns (`incremental_shapes.md` §"Decomposed state (rung 2) in keyed models" → "Presentation projection"). A pipe query reading anything else is unaffected.
...
- **Per-operator native capability matrix.** The `supports_pipe_syntax` flag is whole-feature. Backends whose native dialect supports only a subset (Spark omits `RENAME`/`CALL`/`WINDOW`/`DISTINCT`/`ASSERT`) will need a per-operator capability set before native emission can mix passthrough and lowering. Undecided; BigQuery, the one backend on the native path, supports every operator smelt emits.
