...
- **`JOIN` and set-operation stages are not yet lowered.** For any pipe query that contains `JOIN` or set-operation stages, the printer falls back to emitting the pipe query verbatim, violating §Constraint 2 for `supports_pipe_syntax = false` backends. Tracked in `docs/plans/20260627-pipe_sql.md`.
