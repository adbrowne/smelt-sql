...
- **Unknown-key severity — implementation gap.** The normative rule (Constraint 6) is fail-loud: an unknown key on function frontmatter is an **Error** (`FrontmatterParseError`), matching the model-frontmatter doctrine in `architecture.md` §"Constraints & Invariants" §8. The current implementation still emits the unknown-key diagnostic at **Warning** severity, so typos like `deterministc: true` are accepted past the warning rather than blocking the build. Escalating the unknown-key case to Error is the remaining work; an audit should first confirm no in-the-wild function frontmatter relies on the lenient behaviour.
