...
| Call | Expands to | Column family |
|---|---|---|
| `smelt.latest(value, ordering)` | `MAX_BY(value, ordering)` | order-monotone overwrite |
...
matrix respectively. They exist purely as the vocabulary the keyed-modelling guide
teaches through; using the underlying aggregator directly is equivalent in every respect.
...
| Code | Triggered by |
|---|---|
| `DuplicateFunctionDefinition` | Two `smelt.define`s in the **same directory** share a name. (Directory-scoped — the same leaf name in different directories is allowed, since each has a distinct call path.) |
...
| `FrontmatterParseError` | YAML parse failure, an unknown key, or a malformed sub-entry. Error severity in all cases (fail-loud — see Constraint 6). |
...
4. **`smelt.define` name-uniqueness is directory-scoped**, matching the path-derived `smelt.<path>` identity: two `smelt.define`s in the **same directory** sharing a name is a hard error (`DuplicateFunctionDefinition`), but the same leaf name in two different directories is unambiguous (they have distinct call paths) and is allowed. `smelt.extern` and built-ins, by contrast, share one **workspace-wide** flat name namespace (externs are flat and ambient — their declaring path never reaches the call surface; see `architecture.md` §"Resolution: `smelt.<path>` is the universal addressing scheme"). A `smelt.define` whose leaf name clashes with a built-in is a separate extern-style error (`ExternCollidesWithBuiltin`-shaped), not `DuplicateFunctionDefinition`.
...
6. An unknown frontmatter key on a `smelt.define` / `smelt.extern` is an **Error** (`FrontmatterParseError`). Function frontmatter is user-authored, so the fail-loud unknown-key doctrine (`architecture.md` §"Constraints & Invariants" §8) applies exactly as it does to model frontmatter: a typo like `deterministc: true` is rejected, not silently accepted. Adding a new *recognised* key is still non-breaking — once the parser knows the key, it is accepted.
...
- **The pattern functions (`smelt.latest`, `smelt.once`, `smelt.current`) are unshipped.** Whether they ship as built-in `smelt.`-namespace functions (the default, provided the built-in registry can host transparent bodies) or as documented template snippets is settled alongside the `refresh: keyed` classifier union; the canonical once-write spelling `smelt.once` expands to is fixed at the same time. Tracked in `docs/plans/20260705-keyed-collapse.md` (Phase K6); normative content in `keyed_models.md` §Surface "The column-family catalogue" and §Known Divergences.
