...
1. **Lambda parameters.** Inside the body of a `fn x => expr` lambda, the lambda parameter `x` is innermost scope and shadows everything else. Lambda parameters are bound by `TypeContext::add_lambda_param` and looked up before any other scope. A lambda parameter always shadows a same-named function parameter, CTE column, or FROM-scope column. To reach a shadowed outer name inside a lambda body, authors must assign the outer binding to an intermediate before the lambda, or use a qualified reference.
...
- **Column-list rebinding.** A CTE declared with an explicit column list, `WITH cte(c₁, c₂, …) AS (SELECT …)`, rebinds the inner SELECT's inferred column types to the declared names positionally. The types are preserved; only the names change. When the column list is shorter than the inner SELECT's output, the trailing columns keep their inferred names. When the column list is absent, the inner SELECT's own aliases are used unchanged. Arity mismatch between the declared column list and the inner SELECT's actual column count is currently not flagged (see Known Divergences).
...
- **Bare-column resolution from JOIN aliases inside `TableExpr` bodies.** When a `TableExpr` is provided by the caller as a complex expression (e.g. `smelt.a JOIN smelt.b ON …`), the body's bare-column resolution depends on the alias surface that survives the join. Remaining work is tracked in `docs/plans/20260422-smelt-functions.md`; until it lands, prefer explicit CTE renames inside the body.
...
- **`fragment_param_kinds` seeding.** The body-check pure function seeds a parameter's declared kind into `TypeContext::fragment_param_kinds` so that bare references to a `SelectItems<Kind>`-typed parameter inside `PASSING` bodies inherit the right kind. The seeding contract is wired but lightly tested; corner cases (e.g. a `SelectItems<Agg>` parameter referenced inside a non-aggregate splice point) may surface as the remaining work in `docs/plans/20260422-smelt-functions.md` lands.
- **CTE column-list arity mismatch produces no diagnostic today.** When `WITH cte(c₁, c₂, …) AS (SELECT …)` declares more names than the inner SELECT produces columns (or fewer, ignoring the trailing-keep rule), the compiler currently applies the rebinding silently rather than flagging the mismatch. A dedicated diagnostic for this case is not yet implemented. Tracked in `docs/plans/20260528-values-derived-table-typing.md`.
