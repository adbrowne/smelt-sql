...
- **`directory` filter semantics.** The `directory` parameter of `find_models()` matches the `ModelInfo.directory` attribute — the final component of the model's full `path`, not the full path (see §Surface). The full-`path` attribute and the path-derived `directory` definition are not yet reflected in the user guide. Tracked in `docs/plans/20260509-meta-language-overall.md`.
