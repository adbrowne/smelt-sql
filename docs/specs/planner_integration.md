...
- **`joins:` cardinality is stored as a raw string; the mapping is normative.** The `JoinSpec.cardinality` field stores the raw string from frontmatter (`"1:1"`, `"1:N"`). The string→`Cardinality`-enum mapping is now specified normatively (Semantics rule 8 "Cardinality string→enum mapping"): exact `1:1` → `OneToOne` (enables the elision); any other string → a non-`OneToOne` value that never enables it (fail-safe). `EliminateUnusedLeftJoin` operates on the `Cardinality` value produced by that mapping. The remaining implementation note is purely about *where* the mapping is applied — `EliminateUnusedLeftJoin` consumes `Cardinality` values produced by upstream logical-plan construction rather than reading `JoinSpec.cardinality` directly — not about *what* the mapping is.
