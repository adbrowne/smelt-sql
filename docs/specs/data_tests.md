...
- **No scan lowering yet, for any kind.** §Semantics step 2 ("Lower to a scan when unproven") is not yet wired to an executed query for any test kind — an unproven `not_null`/`unique` test (and every `accepted_values`/`relationships` test, which is always unproven today) is parsed and validated but does not yet run a failing-rows scan or appear in `smelt check`'s PASS/FAIL/WARN report; only the `proven — no scan emitted` verdict from step 1 is reported. Tracked in `docs/plans/20260719-prod-w3-adoption.md`.
...
- **Code**: `crates/smelt-core/src/metadata.rs` (`ColumnTest`, `validate_column_tests`, `validate_column_tests_against_schema`), `crates/smelt-logical/src/data_tests.rs` (`resolve_not_null_verdict`, `resolve_unique_verdict`), `crates/smelt-cli/src/commands/check.rs` (proven-verdict reporting)
