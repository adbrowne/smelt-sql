...
**`smelt.test <name>` as a top-level declaration kind was rejected.** An earlier shape — making tests a sibling of `smelt.define` / `smelt.extern` at the file level — was considered (and briefly carried in `architecture.md` Known Divergences) on the theory that tests were a different kind of thing than a model. We reject it for the reasons in `architecture.md` Design §"Materialization is orthogonal to transparency": a test is a model whose materialization happens to be "execute and assert" rather than "persist as a table", and shoehorning it as a separate top-level kind doubles the grammar surface (parser, frontmatter parser, name-resolution, LSP affordances) without buying anything the unified shape doesn't already have. `materialization: test` is the shape the implementation builds on (`crates/smelt-core/src/metadata.rs::TestConfig`).
