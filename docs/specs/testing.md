...
Tests are discovered by the same project-wide scan as SQL models (discovery is not gated by `paths:`; see `architecture.md` §"Resolution"). They can be co-located in multi-section files alongside the models they test, or placed in a separate `tests/` directory — which needs no special registration, as every non-excluded directory is scanned.
...
- **Property-based test column discovery.** For CTE-targeted tests (`target_cte` specified), omitted columns are detected by walking the CTE body against the user-provided `inputs`; columns referenced in the CTE but absent from `inputs` trigger the property loop. Type inference for the missing columns falls back to `Text` when no other evidence is available. Behavior when type inference is unavailable is undefined.
- **Whole-model property-test column detection.** For whole-model tests (no `target_cte`), there is no CTE anchor for omitted-column detection. Whole-model tests always run one-shot regardless of `cases`; the `cases` field has no effect. Tracked in `docs/plans/20260605-property-test-dispatch-and-week-start.md`.
