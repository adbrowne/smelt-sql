...
8. **`default_materialization` is the storage axis only.** The legal values are the storage modes `table`, `view`, and `ephemeral`. `materialized_view` is not a storage value — it lives on the refresh axis as `refresh: materialized_view` over an implied `table` (`models.md` §Design); setting `default_materialization: materialized_view` is rejected with a migration hint pointing to `refresh: materialized_view`. It does not touch the other two axes. The kind axis is not a materialization (a unit test is a `smelt.test` declaration, `testing.md`), so there is nothing test-shaped to default. The refresh axis is intentionally per-model: `refresh: incremental` (with its `grain:`) derives its driving partition shape and unique key from the individual model's SQL, so a project-wide default would be meaningless; refresh strategies are set per-model (frontmatter or `models.<name>`), never as a blanket fallback. `ephemeral` is retained as a defensible storage default.
...
10. **`probes:` governs declared-fact probe cadence.** `cadence: per_run` (the default) dispatches every `built` probe (`model_properties.md` §"Probe obligation") on every consuming run; `periodic` with `every_n_runs: <n>` dispatches once every `n` runs (a model's first consuming run always dispatches, regardless of `every_n_runs`); `off` skips dispatch entirely. A skipped dispatch — `off`, or a `periodic` run that is not the nth — is recorded per model on the run manifest as `unverified this run` (`run_state.md` §"Run manifest"): the declaration is **trusted** for that run, not that the technique it licenses is abandoned. Per-declaration cadence override is out of scope today — one project-wide policy governs every probe (§Known Divergences).
