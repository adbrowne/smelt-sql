...
| `reads` | every remaining `source_bound`, one story per distinct (`old`, `new`) pair, listing the sources that share it | widened or `Bounded` → unbounded: *Reads more per run* / "Each run now reads <new window> from <sources> (was <old window>)."; the mirror: *Reads less per run* / same shape. A window renders as "all history" for `Unbounded`/`NotDerivable`, "<d> either side of the run window" when `before == after`, and "<d> before and <d'> after the run window" otherwise; `<sources>` lists the sources sorted by name, the last joined with "and" |
