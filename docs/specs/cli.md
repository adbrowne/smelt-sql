...
| `3` | A non-trivial migration is pending and unapproved (`smelt migrate`, §`definition_deltas.md`). The command ran correctly; the deploy changes what a table means and no approved plan covers it. |
...
**`smelt migrate` specifics:** exits `0` when there is no definition delta or the delta is eclipsed-only (including a successful `--apply` whose freshly re-derived plan hash matches the recorded approval); exits `3` when a non-eclipsed plan is derived and unapproved, or when `--apply` finds the recorded hash absent or stale for the freshly re-derived plan; exits `2` for the usual usage/config errors (see `definition_deltas.md` §Surface "`smelt migrate`").
