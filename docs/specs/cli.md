...
- **Generator-emitted model `origin` field in `smelt explain --json` is normative; implementation is forthcoming.** The `origin` field in §"`smelt explain --json` output schema" surfaces generator emissions distinctly from hand-authored models (per `meta_language.md` §"Multi-model production" and `data_catalog.md`'s parallel `origin` field). Implementation lands alongside the multi-model production pipeline. The `--select generator_file:<path>` selector (per `model_selection.md`) is the corresponding selection-side surface. Tracked in `docs/plans/20260509-meta-language-overall.md`.
