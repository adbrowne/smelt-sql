...
- **DATE** — every value matches `YYYY-MM-DD` with year ∈ [1000, 9999], month ∈ [1, 12], day ∈ [1, 31]. Calendar correctness is not validated (`Feb 30` passes shape). Consequently, a column whose every value matches this shape but contains a calendar-invalid date (e.g. `2025-02-30`) infers as DATE and then **fails at load** when the value is coerced to a real calendar date — consistent with the hard-coercion Semantics rule (§"Semantics" rule 2). To load such a column as text, pin it to `VARCHAR` in the sidecar `columns:` declaration.
...
- **Migration tooling.** `smelt migrate` migrates a model's stored table after a definition
  change (`definition_deltas.md`); it has no seed-migration mode. A bundled examples migration
  and a documentation note remain the seed-side story.
