...
| `KeyedOnceWriteUnproven` | Error | A once-write (`COALESCE`-first-non-null) column has no once-write provenance proof, or breaks the family's NULL-preservation obligation by carrying a fallback argument after its `MAX`/`MIN` reduction (a total projection would lock the default into the target). Names the column and the three fixes: a key-derived expression, a declared functional dependency, or remodelling the column out into its own model; the fallback case additionally names dropping the fallback and applying the default downstream. |
