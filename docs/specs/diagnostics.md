...
| Code | Severity | Trigger |
|------|----------|---------|
| `SuccessionWindowFunctionNotLead` | Error | A window function in the projection is not `LEAD(t)`/`LAG(t)` over the clock column, or not a scalar expression over one (a `LEAD`/`LAG` over any other column included). Names the offending call. |
...
| `SuccessionClockTie` | Error | Runtime: a delta presents two events with equal `(k, t)`, or an event whose `(k, t)` is already stored (as a presented row or a tombstone) with different row-local content or delete flag. The run's transaction rolls back; reports the key, clock value, and a sample (`incremental_shapes.md` §"Run shape and late events"). |
...
- **All nine succession-grain codes are specified and unimplemented.** No classifier, technique,
  ...
  `SuccessionRowLocalColumnViolation`, `SuccessionSingleSourceOnly`,
