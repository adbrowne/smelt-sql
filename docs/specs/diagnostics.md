...
| `MaintenanceSkeletonChanged` | Error | A field was added or changed in a skeleton position (a grain change); refused as a column backfill. `smelt migrate`'s skeleton-change verdict names the same code. Owned by `definition_deltas.md` §"Skeleton changes are a new relation". |
...
- **Five of the ten plan/graph `Maintenance*` codes are specified and unimplemented.** `MaintenanceNoAdmissibleTechnique`, `MaintenanceScanUnbounded`, `MaintenanceGranularityMismatch`, `MaintenanceWriteAddressingRefused`, `MaintenanceWritePatternUnavailable`, and `MaintenanceSkeletonChanged` have `DiagnosticCode` variants, folded into `file_diagnostics()` by the thin `maintenance_plan` Salsa query (`crates/smelt-db/src/queries/maintenance.rs`), which assembles inputs and calls the pure `derive_maintenance_plan` (`crates/smelt-logical/src/maintenance/derive.rs`), the pure `check_declared_granularity` leaf classifier (`crates/smelt-logical/src/maintenance/granularity.rs`), and the open write-pattern registry's `resolve_write_pin` (`crates/smelt-logical/src/maintenance/mod.rs`). `MaintenanceSkeletonChanged` fires from the Salsa query itself now, diffed against the `ProjectInput::deployed_columns` input — deployed column names read once, at the workspace-loading edge, into a project-scoped Salsa input (`workspace_ingest::read_deployed_columns`; never inside the query itself, per the Salsa purity rule) — so the LSP and `smelt explain` both surface it ahead of a run, not only `smelt-runtime`'s maintenance driver at run time. `MaintenanceReachNotDerivable`, `MaintenanceUnboundedFootprint`, and `MaintenanceGraphUnsupportedNode` have no `DiagnosticCode` variant yet — their derivation paths (footprint-bounded targeted writes, the graph layer) are not yet wired into the Salsa query. The coverage gate (`crates/smelt-db/tests/integration/diagnostics_catalogue.rs`) only asserts enum → catalogue coverage, so a catalogue row may precede its variant; these rows exist ahead of the variants they document. Landing: `docs/plans/20260707-maintenance-plan-impl.md`. `MaintenanceRepairKeysNotDiscoverable` and `MaintenanceRepairSliceUnbounded` likewise have no `DiagnosticCode` variant yet, so neither reaches `file_diagnostics()` — but the repair family they belong to has a deriving proof, technique, and emitter (`incremental_models.md` §"The repair family"; landed by `docs/outcomes/20260809-repair-family/outcome.md`). Both are surfaced pre-execution instead, by `smelt explain`'s refusal block via `maintenance::ledger::render_refusal` (`crates/smelt-logical/src/maintenance/ledger.rs`), which names them by exactly these catalogue strings; a future `DiagnosticCode` addition for either must reuse the same string.
