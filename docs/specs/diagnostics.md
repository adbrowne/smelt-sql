...
- **`ContractFrozenHorizonInvalid` has a live derivation site; `ContractLateArrivalOutsideHorizon` has a live probe-emitter/dispatch site; the two `deferral` codes remain catalogue-ahead-of-variant.** `ContractFrozenHorizonInvalid` (`incremental_models.md` §"The contract lattice") now has a `DiagnosticCode` variant, raised both at frontmatter-parse time (an unparseable `contract.frozen_horizon`, `smelt_core::metadata::MetadataError::ContractFrozenHorizonInvalid`) and by the grain-admissibility check (`smelt_logical::contract::frozen_horizon::validate_frozen_horizon`, folded into `check_file_diagnostics`). `ContractLateArrivalOutsideHorizon` is raised by `smelt_runtime::contract_probes`'s baseline comparison (`smelt_logical::contract::frozen_horizon::late_arrivals`), dispatched at the same pre-write site as the other declared-fact probes — a runtime probe failure, like `SourceMutationProfileViolated` and `DeclaredMonotonicityViolated`, so it has no `DiagnosticCode` variant (the coverage gate only asserts enum → catalogue coverage, not the reverse). `ContractDeferralInvalid` and `ContractDeferralExceeded` have no `DiagnosticCode` variant, loader acceptance, oracle transform, or probe emitter yet. Landing: `docs/outcomes/20260809-contract-lattice-v1/outcome.md`.
