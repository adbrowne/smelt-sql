...
- **`ContractFrozenHorizonInvalid` has a live derivation site; the other three contract-lattice codes remain catalogue-ahead-of-variant.** `ContractFrozenHorizonInvalid` (`incremental_models.md` §"The contract lattice") now has a `DiagnosticCode` variant, raised both at frontmatter-parse time (an unparseable `contract.frozen_horizon`, `smelt_core::metadata::MetadataError::ContractFrozenHorizonInvalid`) and by the grain-admissibility check (`smelt_logical::contract::frozen_horizon::validate_frozen_horizon`, folded into `check_file_diagnostics`). `ContractLateArrivalOutsideHorizon`, `ContractDeferralInvalid`, and `ContractDeferralExceeded` have no `DiagnosticCode` variant, loader acceptance, oracle transform, or probe emitter yet. Landing: `docs/outcomes/20260809-contract-lattice-v1/outcome.md`.
