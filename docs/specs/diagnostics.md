...
- **`PropertyDowngrade` and `PropertyDiffBaselineUnavailable` are specified and unimplemented.** Both have `DiagnosticCode` variants but no emission site yet: `smelt explain --diff`, the `PropertyProfile` value, `diff_profiles`, baseline materialisation, and the LSP code lens are all still to be built (`property_diff.md`). Landing: `docs/outcomes/20260905-property-diff/outcome.md`.
