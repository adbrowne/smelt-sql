...
- **Three probe-obligation codes are specified and unimplemented.** `DeclaredMonotonicityViolated`, `DeclaredFunctionalDependencyViolated`, and `DeclaredBoundedDomainExceeded` (`model_properties.md` §"Probe obligation") have no `DiagnosticCode` variant yet — their probe emitters do not exist in `crates/smelt-logical/src/maintenance/emit.rs` (the coverage gate only asserts enum → catalogue coverage, so a catalogue row may precede its variant, the same posture as the `Maintenance*` rows above). Landing them is `docs/outcomes/20260809-probe-backed-facts/outcome.md` phase 2. The append-only posture's probe reuses the already-catalogued `SourceMutationProfileViolated` rather than a new code.
