...
| `SuccessionClockTie` | Error | Runtime: a delta presents two events with equal `(k, t)`, or an event whose `(k, t)` is already in the event skeleton with different row-local content. The run's transaction rolls back; reports the key, clock value, and a sample (`incremental_shapes.md` §"Run shape and late events"). |
...
- **All nine succession-grain codes are specified and unimplemented.** No classifier, technique,
  skeleton, or diagnostic dispatch exists yet for `SuccessionWindowFunctionNotLead`,
