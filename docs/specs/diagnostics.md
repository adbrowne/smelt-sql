...
| `SuccessionOrderNotMonotoneClock` | Error | A window's `ORDER BY` column does not trace as a strictly monotone event-time clock on the driving source (`model_properties.md` §"Event-time monotonicity trace" returns `NotTraceable`, or `Traceable` without `is_strict`), or the `ORDER BY` is descending or carries a second sort key. Names the column and the refusal reason, non-strict layer, or offending sort clause. |
...
| `SuccessionDrivingSourceNotAppendOnly` | Error | The driving source does not declare `mutation_profile.kind: append_only`. Names the source and its declared kind (`incremental_shapes.md` §"Run shape and late events"). |
| `SuccessionDeleteFilterMisplaced` | Error | A post-projection filter exists but is not exactly `WHERE NOT <row-local boolean column>` applied after the window functions, or the flag column is not proven `NOT NULL` (`incremental_shapes.md` §"Delete events"). |
...
- **All ten succession-grain codes are specified and unimplemented.** No classifier, technique,
  ...
  `SuccessionDrivingSourceNotAppendOnly`, `SuccessionDeleteFilterMisplaced`,
