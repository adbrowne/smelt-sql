...
| `SourceRetentionExceeded` | Error (plan-time refusal, or run-time refusal for a whole-table recompute) | A model's derived required reach into a declared-`retention:` source is *proven* to exceed the source's retained bound (`model_properties.md` §"Reach versus retained history"), or a whole-table recompute over a declared-`retention:` source is attempted with stored output already present and no license (`sources.md` §Semantics 5 "Retention refusal"). Names the source and the retained bound, plus the required reach in the backfill case. |
| `SourceRetentionDowngraded` | Warning (plan derivation, or run-time report for a licensed whole-table recompute) | A model's derived required reach into a declared-`retention:` source could not be proven to fit inside the bound (an unbounded or otherwise underivable reach) — admitted, but the model's pre-bound region stops being claimed replayable. Or: a whole-table recompute over a declared-`retention:` source was licensed (first build, a smelt-forced full refresh, or an explicit override) — admitted, but the recompute loses replayability for the source's retained region. Names the source, the retained bound, and (in the unprovable-reach case) why the reach was unprovable. |
| `MalformedExternalStep` | Error | An `external_step:` block violates the shape rules in `sources.md` — `produces:` absent or empty, a `produces:` entry that does not resolve to a declared source, `columns:` present alongside `external_step:` on the same file, a malformed `command:` (not a non-empty argv list, or an argv element using an unrecognised `{name}` placeholder outside the closed `{run_date}`/`{run_end}` set), or a malformed `cadence:` interval. |
| `SourceProducerConflict` | Error | Two declared external steps name the same source in their `produces:` list. |
| `ExternalStepNotInvocable` | Error (fails the run rather than proceeding; not a `DiagnosticCode` enum variant — a run-path refusal) | A run reaches a step that produces a source a selected model depends on, but the run cannot invoke it — a dry run, an environment declining external invocation, an unspawnable `command:`, or a `command:` placeholder with no value this run. |
| `ExternalStepFailed` | Error (fails the run; not a `DiagnosticCode` enum variant — a run-path refusal) | A reached step's `command:` exited non-zero. Names the step and its exit code; every model downstream of its produced sources is left unbuilt. |
...
| `KeyedRecurrenceDeclarationMismatch` | Error | Plan time: a declared `key_recurrence` disagrees with the recurrence bound derived from the model's SQL; names both values. The derived value is authoritative (`incremental_shapes.md` key-grain rule 16). |
...
| Code | Severity | Trigger |
|------|----------|---------|
| `SuccessionWindowFunctionNotLead` | Error | A window function in the projection is not `LEAD(t)`/`LAG(t)` over the clock column with the default offset of 1, or not a scalar expression over one (a `LEAD`/`LAG` over any other column, or with an explicit offset or default argument, included). Names the offending call. |
| `SuccessionPartitionKeyMismatch` | Error | Two or more window functions in the projection partition by different column sets, by a column set the classifier cannot resolve to a stable per-row key, or by a column not proven `NOT NULL`. |
| `SuccessionOrderNotMonotoneClock` | Error | A window's `ORDER BY` column does not trace as a strictly monotone clock to the driving source's declared `event_time_column` (`model_properties.md` §"Event-time monotonicity trace" returns `NotTraceable`, `Traceable` without `is_strict`, or `Traceable` to a different source column), is not proven `NOT NULL`, or the `ORDER BY` is descending or carries a second sort key. Names the column and the refusal reason, non-strict layer, traced column, or offending sort clause. |
...
| `SuccessionIdentityNotProjected` | Error | A key column or the clock column is not projected row-locally, so the derived `(k, t)` identity cannot be recovered from the presented table. Names the missing column. |
| `SuccessionSingleSourceOnly` | Error | The `FROM` clause is not exactly one source reference — a join, CTE, subquery, or set operation is present. Names the offending clause. |
...
| `MaintenanceSkeletonChanged` | Error | A field was added in a skeleton position (a grain change); refused as a column backfill. Owned by `definition_deltas.md` §"Skeleton changes are a new relation". |
| `MaintenancePartitionColumnChanged` | Error | A maintained model's declared `timeseries.partition_column` differs from the address recorded in the deployed-schema snapshot at last deploy; names both columns. No run flag bypasses it (the analyzer gate blocks unconditionally on any Error); remedy is to delete the model's recorded snapshot (`.smelt/targets/<target>/schemas/<model>.json`) and re-run. Owned by `incremental_shapes.md` §"The partition grain". |
| `MaintenanceColumnAddNotBackfillable` | Warning | A non-skeleton column addition cannot be backfilled in place (unbounded scan, no admissible technique, unresolvable expression, or group disagreement); the run proceeds and ALTERs the column in, leaving historical rows `NULL` until `smelt migrate`. Owned by `definition_deltas.md` §"Detection". |
...
| Code | Severity | Trigger |
|------|----------|---------|
| `ContractFrozenHorizonInvalid` | Error | A `contract.frozen_horizon` is unparseable or negative, declared on a non-partition-grain model, or declared on a model whose driving source has a declared mutation profile other than `append_only`. |
...
- **Five of the ten plan/graph `Maintenance*` codes are specified and unimplemented.** `MaintenanceNoAdmissibleTechnique`, `MaintenanceScanUnbounded`, `MaintenanceGranularityMismatch`, `MaintenanceWriteAddressingRefused`, `MaintenanceWritePatternUnavailable`, and `MaintenanceSkeletonChanged` have `DiagnosticCode` variants, folded into `file_diagnostics()` by the thin `maintenance_plan` Salsa query (`crates/smelt-db/src/queries/maintenance/`), which assembles inputs and calls the pure `derive_maintenance_plan` (`crates/smelt-logical/src/maintenance/derive/`), the pure `check_declared_granularity` leaf classifier (`crates/smelt-logical/src/maintenance/granularity.rs`), and the open write-pattern registry's `resolve_write_pin` (`crates/smelt-logical/src/maintenance/mod.rs`). `MaintenanceSkeletonChanged` and `MaintenanceColumnAddNotBackfillable` both fire only when the query has a real deployed-schema snapshot to diff against — `smelt-db`'s own Salsa query reaches them via the `DeployedSchemaInput` world-fact input, registered from `.smelt/targets/<target>/schemas/<model>.json` at workspace load and consumed by both the CLI and the LSP (`architecture.md` §"Workspace loading parity rule (CLI ↔ LSP)"), so both codes now surface in `file_diagnostics()` (and `smelt explain`) ahead of any run, not only from `smelt-runtime`'s maintenance driver — the production `ColumnAdded` derivation site — which continues to report the same refusals on the live path (an in-place-update-inadmissible add proceeds with a warning, matching the pre-execution gate's posture; `definition_deltas.md` §"Detection"). A model with no registered snapshot on record derives no trigger at all (fail-closed). `MaintenanceReachNotDerivable`, `MaintenanceUnboundedFootprint`, and `MaintenanceGraphUnsupportedNode` have no `DiagnosticCode` variant yet — their derivation paths (footprint-bounded targeted writes, the graph layer) are not yet wired into the Salsa query. The coverage gate (`crates/smelt-db/tests/integration/diagnostics_catalogue.rs`) only asserts enum → catalogue coverage, so a catalogue row may precede its variant; these rows exist ahead of the variants they document. Landing: `docs/plans/20260707-maintenance-plan-impl.md`. `MaintenanceRepairSliceUnbounded` likewise has no `DiagnosticCode` variant yet — the repair family's bounded-read obligation it belongs to has no deriving proof, technique, or emitter (`incremental_models.md` §Known Divergences "The contract, plan, and graph layer"). Landing: `docs/outcomes/20260809-repair-family/outcome.md`. `MaintenanceRepairKeysNotDiscoverable` has a `DiagnosticCode` variant, folded into `file_diagnostics()` the same way as the other `Maintenance*` plan/graph codes above — raised when neither of an upstream model edge's key-addressed routes admits nor its enrichment-keyed route does (`incremental_models.md` §"Upstream model edges"). Landing: `docs/outcomes/20260906-bigquery-correctness/outcome.md`.
...
- **Code**: `crates/smelt-db/src/diagnostics_types/` — full `DiagnosticCode` enum
- **Code**: `crates/smelt-types/src/signatures/` — `struct_field_unknown_ranges` pure helper
- **Code**: `crates/smelt-db/src/queries/function_diagnostics.rs` — `struct_field_type_unknown_diagnostics_for_file`
- **Tests**: `crates/smelt-db/tests/integration/diagnostics_catalogue.rs` — coverage gate: every `DiagnosticCode` variant must appear in this catalogue
- **Tests**: `crates/smelt-db/tests/integration/struct_field_type.rs`
