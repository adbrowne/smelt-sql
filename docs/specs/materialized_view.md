...
- **Not implemented; no backend advertises native IVM.** All current backends set `supports_native_ivm = false` (`multi_backend.md`), so `refresh: materialized_view` always resolves to the §"No silent fallback" hard error today. The mode, the capability flag wiring, and the first backend to provide it (Databricks Enzyme is the reference target) are delivered by `docs/plans/20260704-model-updates.md`.
