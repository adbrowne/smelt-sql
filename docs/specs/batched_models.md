...
- **Emitted diagnostic codes still spell it `incremental`.** The user-facing surface (`refresh: batched`, the `batched:` block) is landed — selection is by `refresh: batched`, and the retired `incremental:` block is a hard error naming the replacement. The *diagnostic codes* emitted for batched constraint violations (`TimeseriesRequiredForIncremental`, `CumulativeForbidsIncremental`) and internal identifiers (the config type `IncrementalConfig`, the rule module `crates/smelt-logical/src/rules/incremental.rs`, the `IncrementalStrategy` enum) still carry the old spelling. The rename (diagnostic codes, config type name, downstream doc sweep) is a dedicated phase of `docs/plans/20260704-model-updates.md`.
