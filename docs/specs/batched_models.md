...
- **The Rust `per_partition_equivalence` harness under-asserts session equivalence.** Its session assertion compares only `(session_id, utm_campaign)` — both invariant under the output-window derivation this spec describes — so a divergence in `(session_end, event_count)` between a day-by-day build and a full-window build would go undetected. It must also assert `(session_end, event_count)` to close this gap. Tracked in `docs/plans/20260711-derived-output-window.md`.
