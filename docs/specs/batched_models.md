...
- **The surface is `batched`; the code still spells it `incremental`.** This spec is normative for the user-facing surface — `refresh: batched`, the `batched:` block, and `Batched`-spelled diagnostic codes (`TimeseriesRequiredForBatched`, …). The implementation retains the `incremental` spelling throughout: the config type (`IncrementalConfig`), the rule module (`crates/smelt-logical/src/rules/incremental.rs`), the `IncrementalStrategy` enum, the CLI dispatch, and the emitted diagnostic codes (`TimeseriesRequiredForIncremental`, `CumulativeForbidsIncremental`) are unchanged. Today the mode is still selected by an `incremental:` block with `enabled: true` rather than `refresh: batched`. The rename (surface, config keys, diagnostic codes, CLI plumbing) is a dedicated phase of `docs/plans/20260704-model-updates.md`; until it lands, the `batched`-spelled surface in this spec is the target, not current behaviour.
...
- **Three execution paths in `crates/smelt-cli/src/main.rs`.** Legacy, optimizer+batched, batched-only — Phase 1 of the batched plan unified `IncrementalConfig` but the CLI dispatch is still tri-modal. Should converge.
