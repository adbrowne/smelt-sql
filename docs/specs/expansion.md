...
- **`<generator>` frame for multi-model production is landed.** Generator files (per `meta_language.md` §"Multi-model production") emit an outermost `<generator>` frame at the file body's range so that diagnostics from inside emitted-`ModelDef` HOF chains carry a navigable provenance root. The producer-side constructor is `make_generator_frame(path, body_range, file_text)` in `crates/smelt-db/src/function_body_check.rs`; `stamp_generator_frame_onto` appends it as outermost in the frame stack. The frame is stamped only on diagnostics that already carry inner HOF frames (`ExpansionFrames` data), preserving expansion invariant 4: structural file checks (`ModelDefDuplicateName`, `GeneratorBodyForbidsModelReflection`, etc.) with `data: None` remain frameless.
