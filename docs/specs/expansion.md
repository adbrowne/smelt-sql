...
- **`<generator>` frame for multi-model production is normative in §"`FrameInfo` shape"; implementation is forthcoming.** Generator files (per `meta_language.md` §"Multi-model production") emit an outermost frame at the file body's range so that diagnostics from inside emitted-`ModelDef` HOF chains carry a navigable provenance root. Implementation (a producer-side `Frame::new_generator(file_path, body_range)` constructor and the multi-model-production W2 evaluator wiring) has not yet landed. The `model_origin` extension carrying the offending `ModelDef.name` range is producer-side only until the renderer follow-up. Tracked in `docs/plans/20260509-meta-language-overall.md`.
