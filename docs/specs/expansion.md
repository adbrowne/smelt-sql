...
- **`<generator>` frame for multi-model production is landed.** Generator files (per `meta_language.md` §"Multi-model production") emit an outermost `<generator>` frame at the file body's range so that diagnostics from inside emitted-`ModelDef` HOF chains carry a navigable provenance root. The producer-side constructor is `make_generator_frame(path, body_range, file_text)` in `crates/smelt-db/src/function_body_check.rs`; `stamp_generator_frame_onto` appends it as outermost in the frame stack. The `model_origin` extension carrying the offending `ModelDef.name` range is producer-side only until the renderer follow-up. Tracked in `docs/plans/20260509-meta-language-overall.md`.
