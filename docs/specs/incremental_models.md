...
- **The mode value is cut; the sub-block retirement is pending.** `refresh: batched` is a hard error with a fix-it naming `refresh: incremental` + `grain: partition` (`crates/smelt-core/src/config.rs`; delivered by `docs/plans/20260707-maintenance-plan-impl.md`). The `batched:` sub-block (`batched.unique_key`, `batched.nondeterministic_columns`, `batched.safety_overrides`) is still the live surface for those options and still refused without `refresh: incremental` + `grain: partition` (`crates/smelt-core/src/metadata.rs`). Top-level `unique_key` parses; top-level `safety_overrides` does not yet parse; `columns.<c>.contract` does (`models.md` §Known Divergences). The sub-block itself is not yet refused as a hard error — `models.md` §"Batched sub-block retirement" states the target surface (per-key fix-it naming the top-level replacement). The `smelt migrate` assist does not exist. Tracked by `docs/plans/20260719-prod-w8-composed-axes-followups.md`.
