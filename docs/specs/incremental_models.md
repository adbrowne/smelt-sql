...
ordering-key-tie carve-out (§"The equivalence invariant", §"Ordering ties"); `MAX_BY`/`MIN_BY` and
...
and the fingerprint sidecar's own storage — DDL, digest-refresh upsert, GC delete (`sources.md` §"The fingerprint sidecar") — excepting the sidecar's **diff query**, which IS emitter-authored
...
restriction is the affected key set the upstream's delta names (`model_properties.md` §"Affected-key discovery"),
