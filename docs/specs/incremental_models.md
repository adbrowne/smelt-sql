...
- **Refused-incrementality replaces silent downgrade.** Today `commands/run.rs` logs safety-check rejections as `warn!` and proceeds as a full-table refresh. Constraint 10 declares the intended behaviour (hard refusal); the implementation cutover is part of the research-driven plan.
