...
**Equivalence is up to full-refresh non-determinism.** The equality above is bit-identical on the model's **deterministic** columns. A column explicitly opted into non-determinism (Semantics § "Non-determinism and the equivalence contract") is exempt: it need only be a *plausible full-refresh value*, not the same value a particular full refresh produced. The governing principle: a sequence of incremental runs must produce the same output as a full refresh, **and** non-determinism that already differs between two full refreshes may differ across incremental runs. This never extends to a column that governs *which* rows exist, *where* they are partitioned, or *how* they are deduplicated — those stay deterministic regardless of any opt-in.
...
The equivalence contract is *equivalence up to full-refresh non-determinism* (Semantics § "Per-partition equivalence"): incremental output must match a full refresh, except on columns that a full refresh would itself populate non-deterministically. This licenses a **scoped, opted-in** relaxation of the non-determinism rejection above.

A non-deterministic function is admitted when its value flows **only** into a column named in `incremental.nondeterministic_columns` — a *payload* column, written once per window and never read back to place, filter, group, or dedup a row. The opt-in covers both the run-nondeterministic (`NOW()`, `CURRENT_*`) and the row-nondeterministic (`RANDOM()`, `UUID()`) classes; the modeller has declared they accept the variation.
...
13. **Non-determinism stays in the payload.** Non-deterministic SQL is admitted only when its value flows exclusively into a column listed in `incremental.nondeterministic_columns`; it must never reach the `event_time_column`, `partition_column`, a `unique_key` column, or any row-set-membership or grouping position (`WHERE`/`HAVING`/`JOIN … ON`/`DISTINCT`/`GROUP BY`/window spec). These roles determine which rows exist, where they are partitioned, and how they are deduplicated, and must be deterministic regardless of any opt-in (Semantics § "Non-determinism and the equivalence contract"). Listing an excluded column in `nondeterministic_columns` is a configuration error.
...
- **Opt-in non-deterministic columns not yet implemented.** `incremental.nondeterministic_columns` and the flow/taint check that admits non-determinism confined to a listed payload column (Semantics § "Non-determinism and the equivalence contract"; Constraint 13) are specified here ahead of a plan; no implementation exists yet. Today the B5 non-determinism check (`crates/smelt-logical/src/rules/incremental.rs`) still treats all non-deterministic functions as one rejected class, escapable only via the blunt `safety_overrides.allow_nondeterministic`. The membership/grouping case (a non-deterministic `WHERE` / `GROUP BY`) is deliberately out of scope even once the payload opt-in lands — the sharpened contract would permit it, but reconciling frozen-per-window membership against a full refresh needs its own design (research §9.1a Open questions). No implementation plan exists yet.
- **Per-source clamp observability partly emitted.** Of the two surfaces in Semantics § "Observing the per-source clamp", only part ships today: `smelt explain --json` reports each source's `source_partition_col` and derived `(before, after)` offsets in `source_bounds` (`ExplainIncremental.source_bounds` / `SourceBoundJson`), but does **not** yet resolve the run-relative window `[run_start − before, run_end + after)` even when a run window is supplied. The **editor hover** readout — showing a `smelt.<path>` reference's clamp alongside its schema — is **not yet implemented**: LSP hover today is type/column/ref oriented (`crates/smelt-lsp/src/hover.rs`) and surfaces no incremental facts. Both the run-relative explain resolution and the hover readout are specified here ahead of a plan; no implementation plan exists yet. (No warning is specified on the clamp: a "derived lookback exceeds a declared budget" check has no home today — the only declaration, `data_latency`, is a floor combined via `max`, not a ceiling — and is deliberately out of scope.)
...
- **Event-time monotonicity trace: structural primitive emitted; consumers pending.** The monotonicity primitive (Semantics §"Event-time monotonicity trace"; Constraint 12) is implemented and exhaustively tested: the pure structural classifier in `crates/smelt-logical/src/analysis/monotonicity.rs` (`trace_event_time`) returns the `Traceable`/`StaticSeed`/`NotTraceable` verdict against the whitelist/blacklist above, gated by the `smelt-db` nullability check (`trace_event_time_checked`, below), and validated by a generative smelt-sql soundness oracle that compiles generated models through smelt's own backend codegen and searches input data for any `Traceable` verdict that breaks the pushdown commutation identity. It remains **unwired**: no consumer calls it yet, so the three relaxations that depend on it — `UNION`-branch partitionability, subquery/CTE pushdown conservatism, and join driving-fact resolution — are not yet unblocked, and the injected time filter continues to sit only at the outer clamp. Tracked in [`docs/plans/20260702-monotonicity-primitive-tested.md`](../plans/20260702-monotonicity-primitive-tested.md); design in `docs/research/20260703-model-updates.md` (Part 4).
...
- **Leaf-column resolution is name-based; no alias/FROM resolution.** The pure trace resolves its leaf column by matching the name (ignoring qualifier) against the source→partition-column map (`BoundContext.source_partition_cols`); a name matching **zero** sources or **more than one** returns `NotTraceable` (fail closed). There is no FROM-clause/alias-scope resolution at this layer. This is sound today and adequate for single-source consumers (`UNION` branches, single-source subquery/CTE bodies), but it is a prerequisite gap for the join driving-fact consumer: when two joined inputs share a partition-column name, the primitive returns the ambiguous-match `NotTraceable` rather than identifying the driving side. That consumer must add alias-scoped resolution against the model's `FROM` on top of the primitive (design in `docs/research/20260703-model-updates.md` §7.4, §4.8).
...
- **Research**:
  ...
  - [`docs/research/20260703-model-updates.md`](../research/20260703-model-updates.md) — model-updates research (eligibility audit, maintained-state camp, refresh surface); Part 4 derives the event-time monotonicity trace, Part 12 validates it against academic theory and production engines
- **Related specs**:
