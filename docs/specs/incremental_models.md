...
- **Bound derivation on the planner/classification path runs on the outer SQL body, not the expanded CST.** At **execution** time the deriver runs on the *expanded* SQL: the run pipeline calls `SqlCompiler::expand_function_calls` to inline `smelt.define` bodies (leaving `smelt.<path>` refs intact) before deriving source bounds, so a `RANGE BETWEEN INTERVAL` declared *inside* a function body is honored and the source read is widened correctly (no silent zero-lookback). The **classification/explain** path (`smelt explain --json`, batch-safety class) still derives from the outer SQL only, because the planner does not have the function registry in scope — a model whose sole bound is inside a function body may therefore *classify* as `fully_batch_safe` while *executing* with the correct widened read. This is cosmetic (execution is correct); converging the planner path needs the function registry threaded into it. Tracked in `docs/plans/20260521-incremental-timeseries-and-derived-bounds.md`.
- **Type inference does not flow through `TableExpr`-returning functions.** A `smelt.define f(...) -> TableExpr` call is opaque to the column/type checker: a model that *projects named columns* out of the call works, but referencing one of those columns in an expression the checker must resolve (e.g. a Form B `WHERE event_date BETWEEN session_start_date - INTERVAL '1 day' AND …` where `session_start_date` came from the function) raises a spurious *"Column not found"* diagnostic. This is why the `web_analytics` sessionization is **inlined** in the model body rather than calling a reusable `sessionize` function: the model needs that Form B filter (to widen the write window for cross-midnight sessions), and the filter references a function-derived column. Inlining keeps every column resolvable and keeps the `RANGE BETWEEN INTERVAL '1 day' PRECEDING` lookback visible to the deriver. Restoring a reusable `sessionize` function awaits column-type inference through `TableExpr` functions.
