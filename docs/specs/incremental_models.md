...
- **Run/backfill batch chunking classifies on the outer SQL body, not the expanded CST.** Both the execution bound derivation and `smelt explain --json` now see function bodies: the run pipeline and the explain path call `SqlCompiler::expand_function_calls` to inline `smelt.define` bodies before deriving source bounds, and the batch-safety analyzer additionally text-scans for `RANGE BETWEEN INTERVAL` frames, so a lookback declared *inside* a function body is honored for source-read widening and reflected in the explain `batch_safety` / `source_bounds`. The remaining gap is the run/backfill **chunk-sizing** classification (`compute_batches_for_model`, `smelt-runtime/execute.rs`), which still calls `analyze_batch_safety` on the outer `model.sql`. In practice this is benign: a model whose lookback lives only inside a function body and that carries no outer Form B filter would chunk as `FullyBatchSafe`, but its *reads* are still widened correctly by the expanded bound derivation, and any model that also widens its write window (the cross-midnight pattern) carries an outer Form B the chunker already sees. Threading expansion through the chunking call sites is the remaining convergence step. Tracked in `docs/plans/20260530-thread-fn-registry-classification.md`.
