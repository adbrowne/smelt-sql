...
- [ ] **Wire as_struct into `format_plan` / SQL emission** — Phase 42 deferred: function bodies still travel through `LogicalNode::Raw` placeholders (Phase 41), so the helper has no caller during `smelt build`. End-to-end SQL emission lands when body lowering replaces `Raw` with structured `Select`/`Cast`/etc. nodes (alongside Phase 46's TableExpr argument shapes or a dedicated body-lowering phase).
