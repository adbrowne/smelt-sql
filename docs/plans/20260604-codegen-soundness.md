...
| C2 | BUG-009: TableExpr `source.*` over a `smelt.<path>` arg emits valid SQL — alias the argument at the splice (`FROM <arg> AS <param>`), keep the body verbatim | done | BUG-009 | fix(runtime): alias TableExpr arguments so source.* emits valid SQL (closes BUG-009) | 2026-06-05 |
| C3 | BUG-007: analysis-time CTE-collision check — mint `CteShadowsCallerCte` (Error) for a model CTE colliding with a directly-called function body CTE; spec increment (scoping.md table + narrow expansion.md divergence) | done | BUG-007 | feat(db): diagnose function-body CTE collisions with caller CTEs at analysis time (closes BUG-007) | 2026-06-05 |
| C4 | Close-out: flip BUG-007/008/009 to `fixed` in the ledger with regression-test names; update master sub-plan table + ROADMAP; full `example_builds` + all gates green | done | — | docs(codegen-soundness): close out — CTE collisions diagnosed, source.* valid, ledger + roadmap updated | 2026-06-05 |
