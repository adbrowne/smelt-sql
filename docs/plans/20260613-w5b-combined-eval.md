...
|-------|-------|--------|--------|--------|------|
| P1 | Migrate Python discovery `smelt-cli` → `smelt-runtime` (CLI + UI consume via shared pipeline) | pending | D-24 (1/5) | refactor(runtime): move Python @model discovery into smelt-runtime; CLI + UI consume it (D-24) | |
