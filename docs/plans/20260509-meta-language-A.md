...
| 5     | done     | (pending commit) | 2026-05-10 |
| 6     | pending  |        |      |
| 7     | pending  |        |      |
...
- **Phase 3 unwired splice positions (deferred to Phase B).** `expand_spread_into_position` and the bidirectional `disambiguate_list_literal` are wired into the SELECT-list production path in `check_file_diagnostics` (both spread and bare list literal). GROUP BY, ORDER BY, function-call positional args, IN-list, and VALUES rows remain unwired — the parser DOES emit `LIST_SPREAD` CST nodes there per Phase 1, but `check_file_diagnostics` doesn't yet walk those positions. Phase B (when HOFs and reducers ship) is the natural place to extend wiring to all comma-separated grammar positions.
- **Phase 3 forbidden-position coverage gap.** The Phase A spec lists four forbidden positions: WHERE clauses, FROM-without-reducer, boolean composition, and named-arg values. Only WHERE detection is implemented in `check_forbidden_position_spreads`. The other three positions are not exercised by any diagnostic — the parser already rejects `...` in those positions via parse errors, but no `MetaSpreadInForbiddenPosition` is emitted. Phase B should consolidate this coverage as forbidden-position detection grows.
...
- **Phase 5 broken-fixture layout: three workspaces, not one.** Each Phase A diagnostic code that was wired in Phase 3 has its own broken workspace (`examples/meta_lists_broken_heterogeneous/`, `examples/meta_lists_broken_empty_unknown/`, `examples/meta_lists_broken_spread_forbidden/`). A single combined broken workspace was tried first and rejected: with multiple broken models in one workspace the per-model "exactly one Phase A diagnostic" assertion fired against sibling Phase A codes from other broken files. The three-workspace layout mirrors the existing `examples/broken/` precedent and keeps each diagnostic test surgical. The helper `check_workspace_emits_exactly_one_diagnostic` filters by the Phase A code set so a `ParseError` from parser error-recovery does not invalidate the assertion.
- **Phase 5 `MetaSpreadOnNonList` has no broken-workspace fixture.** Phase 5 ships clean + broken workspaces for the three Phase A codes whose tests are explicitly named in the plan. The fourth code, `MetaSpreadOnNonList`, is unit-tested in Phase 3 and incidentally covered by the helper's full-Phase-A-code-set filter on every workspace, but no dedicated broken fixture was added. The plan called the fourth fixture optional. Phase B may add it once function-arg / GROUP BY / VALUES wiring lands and broader fixture coverage becomes natural.
- **Phase 5 `coalesce(...[a, b], 0)` example dropped.** The plan's suggested `coalesce_spread.sql` happy-path fixture was omitted because Phase 3's deferred wiring leaves spread inside function arguments parser-recognised but not type-checked: `coalesce(...[a, b], 0)` produces unexpected interaction (no diagnostic, no expansion). Re-introduce when Phase B wires positional-argument splice walking.
