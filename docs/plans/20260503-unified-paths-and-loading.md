...
| 4     | done     | 4d04eb9 | 2026-05-03 |
| 5     | done     | 4e0bd13 | 2026-05-03 |
| 6     | done     | 3dfd35b | 2026-05-03 |
| 7     | done     | 80236ec | 2026-05-03 |
| 8     | done     | 2cae6f1 | 2026-05-03 |
...
- `examples/**/*.sql` — **Migrate `smelt.models.*` and `smelt.seeds.*` refs in example SQL files to scan-root-stripped form.** Under `paths: [models]`, `smelt.models.users` becomes `smelt.users`; `smelt.models.staging.orders` becomes `smelt.staging.orders`. Under `paths: [seeds]`, `smelt.seeds.category_hierarchy` becomes `smelt.category_hierarchy`. This is a large mechanical sweep (~1665 `smelt.models.*` refs, mostly in `examples/huge/`, plus 2 `smelt.seeds.*` refs in `examples/ecommerce/`). Use a sed script or similar to perform the rename. **Do NOT migrate `smelt.sources.*` refs** (sources still use the aggregate `sources.yml` and keep their `["sources", ...]` address until Phase 6). **Do NOT migrate `smelt.functions.*` refs** (functions are discovered separately via `discover_function_files()`, NOT via `config.paths`; they keep their full workspace-relative path and are addressed as `smelt.functions.*` without scan-root stripping).
...
- [ ] `compiler.rs` `make_path_ref_resolver_with_ephemerals` emits `{schema}.{segs.join("_")}` for models and seeds; the old `segs[0] == "models"` (leaf-only) and `segs[0] == "seeds"` (skip-first-join) branches are gone; the `segs[0] == "sources"` branch stays (sources.yml still active until Phase 6).
- [ ] No example SQL file uses `smelt.models.*` or `smelt.seeds.*` as a ref after migration; `smelt.sources.*` and `smelt.functions.*` refs remain unchanged (Phase 6 and no-change respectively).
