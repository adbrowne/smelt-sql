...
|-------|-------|--------|--------|------|
| P1 | `smelt-backends` aggregator crate; `smelt-cli` delegates its `BackendFactory` to it | pending | | |
| P2 | `smelt-ui` consumes the shared factory (gains Spark); remove its DuckDB-only selection | pending | | |
