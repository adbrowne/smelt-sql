...
- **2026-06-30** — **W7 fully done** (P5 docs + CLAUDE.md + Known-Divergence retractions complete). `guide/targets.md` expanded with Delta vs Parquet section and UI-runs-Spark note. `CLAUDE.md` Commands entry added for the gated Spark parity tests. `multi_backend.md` Known-Divergences retracted: "parity not yet verified" rewritten to affirm CI-gated verification; "cross-engine type conformance partly validated" retracted (decimal + NTZ + TZ all green from P3). `last_reviewed` bumped to 2026-06-30. **Parity initiative complete** — no further waves remain in the scaffolding queue.
