...
|-------|-------|--------|--------|--------|------|
| P1 | `HofNamedArgument` — dedicated code, fires before kind check | pending | D-19 | feat(db): HofNamedArgument for named args to map/filter/reduce (D-19) | |
| P2 | Frontmatter diagnostics: `BackendsWideningNotAllowed` malformed-branch → `FrontmatterParseError`; `FrontmatterParseError` Error in all cases | pending | D-14, D-31 | fix(db): route malformed function frontmatter to FrontmatterParseError; Error in all cases (D-14, D-31) | |
| P3 | `DuplicateFunctionDefinition` directory-scoped | pending | D-30 | fix(db): scope DuplicateFunctionDefinition to a directory, not the workspace (D-30) | |
| P4 | D-08/D-09 cleanup: confirm bare-path → `UndefinedModelRef`; retire `smelt.source()` call-form | pending | D-08, D-09 | refactor(runtime): drop legacy smelt.source() call-form; lock bare-path → UndefinedModelRef (D-08, D-09) | |
