...
| U4 | Route the **function/extern** path through `parse_frontmatter`: `FunctionProperties` from a lenient `RawFunctionProperties` serde derive; **delete** `parse_function_properties` (one parser remains) | done | — (unification) | refactor(planner,core): consume the shared frontmatter parser; delete parse_function_properties | 2026-06-04 |
