...
| 5     | done     | cbe369e | 2026-05-10 |
| 6     | done     | f9fbafc | 2026-05-10 |
| 7     | done     | 7380a94 | 2026-05-10 |
...
- **Phase 1 `LambdaArityNotSupported` detection is text-heuristic in Phase B.** Round 2 reviewer flagged that `is_fn_lambda_start()` originally matched LPAREN, mis-parsing `SELECT fn(x, y) FROM t` as a multi-arg lambda. Fix restricted detection to `Some(IDENT)` only. The parser does not produce a multi-arg LAMBDA CST node, but `LambdaArityNotSupported` still fires in Phase B via a text shape check on the HOF's second argument (`type_inference.rs::check_hof_position_diagnostics` lines 4972-4998 detect `starts_with("fn(")` / `starts_with("fn (")` and emit the diagnostic). The Phase 7 round-1 parser-expert review initially accepted a spec edit deferring the diagnostic to Phase F; round-2 review caught that the diagnostic actually does fire (via the text heuristic) and the round-2 follow-up reverted the spec edit and added a sentence noting the heuristic detection path. Phase F will replace the heuristic with structural detection on a real LAMBDA CST node.
...
- **Phase 7 expansion-expert round 2 — multi-frame chain integration is structural-only in Phase B.** `walk_hof_lambda_body_with_anonymous_frame` now accepts and propagates a `NestedPathCallHandler`, but the only production call site (`type_inference.rs::check_hof_position_diagnostics`) passes `nested = None` because that walker has no `smelt.functions.*` dispatch context. The end-to-end scenario "a `smelt.define` body containing a HOF call whose lambda body calls `smelt.functions.X(...)`" therefore does not produce stacked expansion frames at runtime; the parameter is wired through but the integration into the expansion path is not yet exercised. This is not a regression (pre-Phase B also lacked the path) — the reviewer flagged the round-1 commit message overstating what was wired. The reviewer accepted the structural wiring as Phase B scope and the integration path as a deferred enhancement (probably landing alongside reflection / multi-model production in Phases C–E2 when HOFs are routinely reachable inside generator bodies).
...
- **Phase 7 examples-curator round 2 — `LambdaArityNotSupported` fixture missing.** Round-1 examples-curator (commit `d6ef063`) added 4 missing fixtures for diagnostics surfaced during the broken-workspace gate, but at that time `LambdaArityNotSupported` was assumed deferred to Phase F (the parser does not produce a multi-arg LAMBDA CST node). Parser-expert round 2 (commit `084615a`) revealed the diagnostic actually does fire in Phase B via a text shape heuristic, so the fixture must land alongside. Round-2 examples-curator caught the gap; commit `7380a94` added `examples/meta_hofs_broken_lambda_arity_not_supported/` with a single multi-arg lambda `fn (a, b) => a + b` in a HOF position and removed the `#[ignore]` from the corresponding test. All 14 Phase B diagnostic codes now have at least one broken-fixture.

> Phase 7 expert review: parser-expert clean (R3), type-expert clean (R2), expansion-expert clean (R2 — multi-frame integration accepted as deferred Phase B scope), lsp-expert clean (R2), examples-curator clean (R3), docs-reviewer clean (R2). No stop-the-line fired.
