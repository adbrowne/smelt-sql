...
| [`docs/plans/20260705-keyed-time-partitioned-output.md`](20260705-keyed-time-partitioned-output.md) | **Keyed time-partitioned output** (1–8) — the (key, time)-addressed cell: admits an optional `timeseries:` block on keyed output under **key temporal locality** (three routes: key-embedded, key-determined, recurrence-bounded/checked), the slice-pruned merge target, the `key_recurrence` source declaration + transactional runtime check, and output-as-clocked-source (the clock-sink fix). Depends on the keyed collapse (K1–K6) — its execution prompt hard-stops if K1–K6 are not `done`. Design: `docs/research/20260705-keyed-time-superset.md`; spec `docs/specs/keyed_models.md` §"Key temporal locality". Defers the mode-agnostic scope-map `smelt explain` surface to its own sub-plan. | pending |
