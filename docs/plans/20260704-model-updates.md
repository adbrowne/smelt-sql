...
| [`docs/plans/20260707-maintenance-plan-spec-alignment.md`](20260707-maintenance-plan-spec-alignment.md) | **Maintenance-plan spec alignment** (SA1–SA7) — closes the remaining spec-diff rows of `docs/research/20260705-refresh-as-maintenance-plan/09-spec-readiness.md` §4: demote `batched_models.md`/`keyed_models.md`/`versioned_models.md` to shape profiles (admission as per-cell obligations), the technique-primitive contracts in `model_transforms.md`, the plan-purity invariant in `architecture.md`+`CLAUDE.md`, the `Maintenance*` family in `diagnostics.md`, the `model_properties.md` complete-proof-catalogue re-homing (all plan proof inputs incl. partition-locality + faithful-fold as referenced rows; completeness invariant; `nondeterministic_columns`→`contract` supersession), the second-ring sync sweep (cli/run_state/schema_evolution/timeseries + removed-mode-name residue). Docs-only. | done (2026-07-08) |
| [`docs/plans/20260707-property-composition-walk.md`](20260707-property-composition-walk.md) | **Property-composition walk** (Phases 1–7) — refactors `smelt-logical`'s analysis layer onto one shared bottom-up query-tree fold with per-property transfer functions (`model_properties.md` §"The composition walk"); closes property-discovery cells SC-4 (series-add reach), SC-5 (lateness+reach sum), SC-6 (FD-over-UNION widening), SC-7 (CTE-body admission hole) red-first; registers grain/FD/discriminant/determinism proofs on the walk; lands the architecture invariant + raw-scan guard. Design basis: `docs/research/20260707-property-composition-overview.md` + nine companions. Ordered **before** the maintenance-plan impl so MP4–MP5/MP12's plan derivation consumes the walk-backed property vector rather than the flat scans it replaces. | done (2026-07-08) |
| [`docs/plans/20260707-maintenance-plan-impl.md`](20260707-maintenance-plan-impl.md) | **Maintenance-plan implementation** (MP1–MP17) — drives code + user docs to `maintenance_plan.md`/`models.md`/`sources.md`: the refresh-axis surface cut + docs-site rewrite (MP1–MP2), structured `mutation_profile` (MP3), the column-grouping/skeleton derivations + production `derive_maintenance_plan` (MP4–MP5), Salsa query + `Maintenance*` diagnostics + `smelt explain` (MP6–MP7), testkit graduation (MP8), the reconciliation ledger frontier grade (MP9), composite keys + the first plan-admitted targeted-write cell (MP10–MP11), per-delta grade closing G-12 (MP12), choice ladder + `smelt bakeoff` (MP13), landed-delta recording + grain check (MP14), forward propagation `--since-upstream` (MP15), backward resolution `--include-upstreams` (MP16), the coverage-matrix conformance sweep (MP17). Migration ordering: `08-code-placement.md` §2.8. | done (2026-07-10) |
| [`docs/plans/20260712-generative-maintenance-conformance.md`](20260712-generative-maintenance-conformance.md) | **Generative maintenance conformance** (Phases 1–11) — the standing generative regression net for `maintenance_plan.md` §"The equivalence invariant": `ModelRecipe` generation over the Link-C harness, admitted/refused verdict protocol, S-tracked dual-mode full-refresh oracle, plan-claim probes (clamp consistency, write-window containment, mutation-sensitivity, ledger incl. the persisted `.smelt/reconciliation.json`, technique-interchangeability pins), simulated change feeds, definition-change steps, generated-DAG propagation sufficiency + backward resolution, and graduation/retirement of the disposable `property_discovery` probe layer into the standing `cargo test -p smelt-cli --test maintenance_conformance` gate. Design: `docs/research/20260711-generative-maintenance-conformance.md`. PREREQUISITE: `DUCKDB_LIB_DIR` + `LD_LIBRARY_PATH` exported into the loop env (unset → DuckDB legs skip green, a silent hole). | done (2026-07-13) |
| [`docs/plans/20260715-composed-axes-conditional-maintenance.md`](20260715-composed-axes-conditional-maintenance.md) | **Composed axes + conditional maintenance** (A0–G2, 34 phases) — the composed keyed+timeseries shape (key temporal locality routes 1–3, slice-pruned merge, settle bound, conformance recipes), the web-analytics tracer (`events_deduped` + tutorial chapter, early), the Relation Contract surface (top-level `unique_key:`, grain as derived check-only label, source-side contract in explain), graph admissibility + key→partition dirt projection for composed nodes, change-suppressed writes (M1: P2/P3 proofs, suppressed MERGE, staged-candidate DELETE+INSERT, slice-bounded compare), the open write-pattern registry + `cells[].write` pin, observed output deltas (M3-output), delta-restricted compute over model edges (M2) and external sources via fingerprint sidecars (M3-input), choice integration + docs sweep. Spec: `incremental_models.md` (+ companions); research: `20260715-conditional-maintenance-without-cdf.md`, `20260716-relation-contract-and-per-cell-addressing.md`. PREREQUISITE: `DUCKDB_LIB_DIR` + `LD_LIBRARY_PATH` exported into the loop env (unset → DuckDB legs skip green, a silent hole — the conformance and e2e legs are DuckDB-backed). | done (2026-07-19) |
| [`docs/plans/20260705-keyed-collapse.md`](20260705-keyed-collapse.md) | **Keyed collapse** (K1–K6) — see the 2026-07-07 supersession note below. K1–K2 landed (`keyed_models.md`, `RefreshStrategy::Keyed`); K3–K6's surviving scope is re-homed into the maintenance-plan programme. Decision record: `docs/research/20260705-keyed-collapse-application.md`. | done (superseded 2026-07-07 — do not run; see note) |
| [`docs/plans/20260705-keyed-time-partitioned-output.md`](20260705-keyed-time-partitioned-output.md) | **Keyed time-partitioned output** — the (key, time)-addressed cell under key temporal locality. Superseded: `grain: key_per_partition` is now declared surface (`models.md`), and the locality machinery lands via the plan's partition-locality verdicts + `key_recurrence` in `mutation_profile` (MP3/MP6/MP11). Design retained: `docs/research/20260705-keyed-time-superset.md`. | done (superseded 2026-07-07 — do not run; see note) |
| [`docs/plans/20260704-model-updates-l4-batched.md`](20260704-model-updates-l4-batched.md) | **L4 batched** (BL1–BL8) — **supersedes Group B**; itself superseded 2026-07-07: most of its residue landed via fundamentals-era commits (taint/pinning, run/partition granularity alignment, ordered self-reference, monotone-integer keys, backfill chunking); what remains (clamp observability) is owned by `smelt explain` (MP7). | done (superseded 2026-07-07 — do not run; see note) |
| [`docs/plans/20260704-model-updates-l4-versioned.md`](20260704-model-updates-l4-versioned.md) | **L4 versioned** (V1–V5) — **supersedes Group D/D2**; itself superseded 2026-07-07: the SCD-2 composition is re-scaffolded as a shape-profile composition against the demoted `versioned_models.md` after the maintenance-plan machinery lands (see scaffolding queue). | done (superseded 2026-07-07 — do not run; see note) |
| [`docs/plans/20260704-model-updates-l4-materialized-view.md`](20260704-model-updates-l4-materialized-view.md) | **L4 materialized_view** (MV1–MV3) — **supersedes Group D/D3**; itself superseded 2026-07-07: re-scaffold after the maintenance-plan machinery lands (delegate-to-native-IVM is untouched by the plan matrix; only its registration order moved). | done (superseded 2026-07-07 — do not run; see note) |
...
> **The refresh-as-maintenance-plan supersession (2026-07-07).** The spec set for the ratified
> refresh-as-maintenance-plan programme landed (`maintenance_plan.md` new; `models.md` refresh-axis
> rewrite — `batched`/`keyed`/`versioned` removed outright per decision 5; `sources.md` world-facts;
> research: `docs/research/20260705-refresh-as-maintenance-plan/`, ratification 09 §1). Strategy content
> is now **derived per `(column-group × trigger)` cell**, so every pending row written against the old
> mode surface is **superseded — do not run them**: their registry Status now reads
> `done (superseded 2026-07-07)` solely so the loop's READY check skips them; their plan files are
> retained for history with their internal phase states untouched. Surviving scope is re-homed:
> keyed-collapse K3–K6's ledger/posture/admission work → `20260707-maintenance-plan-impl.md` MP9/MP11/MP12;
> keyed-time-partitioned locality → MP3/MP6/MP11 + `grain: key_per_partition`; L4-batched residue →
> already landed (fundamentals-era commits) except clamp observability → MP7; L4-versioned and
> L4-materialized_view → re-scaffolded post-MP16 (queue below). The two new registered rows above run
> first: spec alignment (SA1–SA7), then implementation (MP1–MP17).
...
- **Keyed rungs 2–4** (decomposed state / retraction / multiset — the de-registered L4-cumulative scope)
  — re-scaffold against the demoted `docs/specs/keyed_models.md` **after the maintenance-plan
  implementation completes** (the per-delta ledger MP12 is their substrate).
- **Versioned (SCD-2) shape-profile composition** — re-scaffold against the demoted
  `docs/specs/versioned_models.md` after `20260707-maintenance-plan-impl.md` MP11 lands (the
  out-of-window keyed write becomes a plan-admitted technique, not a mode).
- **Materialized-view native-IVM delegation** — re-scaffold the ex-L4-MV scope (delegate-to-native-IVM
  emit + verbatim engine-error relay + remove the silent `create_table_as` fallback) any time after the
  surface cut (MP1); it is independent of the plan matrix.
