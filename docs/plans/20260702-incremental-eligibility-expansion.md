...
|----------|-------------------------|--------|
| `docs/plans/20260702-monotonicity-primitive-tested.md` | W1 — the pure event-time monotonicity trace primitive (`smelt-logical`), exhaustively unit- + property-tested against the DuckDB harness, nullability-gated in `smelt-db`, then the spec's open questions resolved from what the tests proved. **Unwired** into any user-visible gate. | pending |
