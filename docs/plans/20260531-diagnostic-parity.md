...
| P2c | Source-diagnostics producer: surface malformed per-entity source YAML (`MalformedSource`) into `file_diagnostics`, gated like any `Error` | pending | BUG-032 | | |
...
- **Commit**: `feat(db): surface malformed per-entity source YAML as MalformedSource diagnostics (closes BUG-032)`
