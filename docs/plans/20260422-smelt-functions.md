...
- **Phase 56 — `build_fn_body_map` walks the AST directly instead of calling `function_body()`** (2026-04-27). The plan specified using the `function_body()` Salsa query for the byte range, but the implementer iterates `define.body()?.syntax().text_range()` directly during AST traversal. Both paths produce identical byte ranges (the Salsa query's body extracts the same `define.body()?.syntax().text_range()`); the AST-walk is one parse cheaper because the surrounding loop already holds the define node. No behavioural difference; documented in the helper's docstring.
- **Phase 56 — function-file discovery added to `run.rs` / `backbuild.rs`** (2026-04-27). The plan asserted "Function discovery is already done — `function_files` are part of `models` since the user discovers them in `build.rs`." That is only true for the `--show-plan` branch of `build.rs`. The regular `build` → `seed` → `run` chain never touched `discover_function_files()`. Phase 56 added the call in both `commands/run.rs` and `commands/backbuild.rs`, kept the function files OUT of `LogicalGraph` (they're not materialisable), and routed them into `init_db`'s file set so `parse_file` resolves them and `build_fn_body_map` can walk them.
- **Phase 56 — `smelt-ui` standalone compile path skipped** (2026-04-27). `crates/smelt-ui/src/run_manager.rs::compile_sql` builds a `PrintContext` directly with `smelt_fn: None` and never goes through `SqlCompiler` / `CompilerRegistry`. The plan's "if it constructs compilers directly" caveat covered this. Until `smelt-ui` migrates to the registry, its standalone compile path will continue to emit `smelt.fn.*` verbatim — separate follow-up, not Phase 56 scope.
- **Phase 56 — `if !fn_bodies.is_empty()` guard on `set_function_bodies_all`** (2026-04-27). Belt-and-braces: the printer already falls through to `print_children` when the expander returns `None`, so `Some(empty_map)` is observably identical to `None` for models that contain no `smelt.fn.*` calls. Keeping the guard means legacy projects (no `functions/` directory) leave `SqlCompiler::fn_bodies` as `None` byte-for-byte, removing any risk of behavioural drift in the regression suite.
- **Phase 56 — TableExpr `smelt.fn.*` calls in FROM position likely need an alias for Phase 57 e2e** (2026-04-27, surfaced by reviewer). `build_fn_body_map` extracts the body including its wrapping `(...)` parens. For expression-shaped bodies (e.g. `safe_divide`) inlined into a SELECT-list slot, parens preserve precedence — fine. For TableExpr-shaped bodies (e.g. `sessionize`, `add_margin`) inlined into a `FROM` slot, the substituted result is `FROM (SELECT ...)`, which DuckDB requires to be aliased. Phase 57's e2e fixtures using `FROM smelt.fn.<TableExpr-fn>(...)` may need explicit `AS x` aliases on the call site, or the lowering layer needs to inject one. Flagged for Phase 57 to handle; not a Phase 56 wiring bug.
- **Phase 58 — EXTRACT / MAKE_TEMPORAL parser bug was already fixed by intervening work** (2026-04-27). The plan anticipated needing a parser fix (the alias-extraction / SELECT-item-splitter mishandling `FROM` inside `EXTRACT(part FROM expr)`). Writing the failing test first — `crates/smelt-db/tests/extract_alias_extraction.rs::extract_year_select_item_alias_is_recovered` — revealed the test passes today: `parse_extract_expr` already wraps the `FROM` inside an `EXTRACT_EXPR` CST node (`crates/smelt-parser/src/parser.rs:3170-3207`); `Expr::cast` recognises `EXTRACT_EXPR` (`crates/smelt-parser/src/ast.rs:1477`); `SelectItem::alias()` walks tokens *after* the wrapped expression node, so the inner `FROM`/`YEAR` tokens never bleed into alias detection. `infer_extract_type` and `MAKE_DATE`/`MAKE_TIMESTAMP` inference are also already wired (`crates/smelt-db/src/type_inference.rs:1066-1081`, `:3234`). So Phase 58 reduced to: pin current behaviour with five regression tests (alias + BigInt + Double-for-EPOCH + Date + Timestamp), re-enable the strategy entries with weight 1, and clean up `docs/TODO.md` (lines 36, 62, 77 — the EXTRACT exclusion entries). Zero new divergences at PROPTEST_CASES=200 and =500. The proptest generator only emits `YEAR/MONTH/DAY` parts (all `BigInt`); broader EPOCH coverage in the generator stays a future-work nice-to-have.
...
- **Phase 57 — subprocess invocation chosen over `commands::build::build()`** (2026-04-27). The plan's literal phrasing said "Run `BuildArgs` end-to-end through `commands::build::build()`", but `mod commands` is private to `crates/smelt-cli/src/main.rs:1` (`mod commands;`, not declared in `lib.rs`). Calling it from an integration test would require either exposing a public `pub mod commands` (invasive — drags every command module's deps into the public API) or adding a thin re-export. Choice A (subprocess via `env!("CARGO_BIN_EXE_smelt")`) was selected because (a) it matches what users actually do, (b) it mirrors the existing pattern in `crates/smelt-cli/tests/smelt_shop_idempotency.rs`, and (c) the build-test target already has the binary available at zero extra cost. Captures stdout/stderr on failure for triage.
- **Phase 57 — `compile_with_ephemerals` / `compile_with_sql` / `compile_with_sql_and_ephemerals` were not wired through fn-body / as_struct emitters** (2026-04-27, fixed in Phase 57). Phase 56 wired `SqlCompiler::set_function_bodies` via `compile()` but only `compile()` actually built the `as_struct_emitter` and `fn_expander` and threaded them into `PrintContext`. The production execution path in `commands/run.rs:600` calls `compiler.compile_with_ephemerals(...)` which had `smelt_as_struct: None, smelt_fn: None` hardcoded — so `smelt build` against a project using `smelt.fn.*` or `smelt.as_struct` emitted them verbatim and DuckDB then errored with `Catalog "smelt" does not exist!`. Phase 57's e2e tests surfaced this; the fix (Phase 57 commit) extracted `SqlCompiler::build_emitters(&syntax)` and called it from all four `compile_*` methods. Without this fix the function-body wiring claimed by Phase 56 had no observable effect end-to-end.
- **Phase 57 — Test 3 (`e2e_passing_clause_substitution_executes`) deferred** (2026-04-27). The current `SmeltFnExpander` (`crates/smelt-cli/src/compiler.rs`, see `move |fn_name, positional, _named|`) drops named arguments — it only substitutes positional parameters. Any TableExpr / PASSING-shaped function fixture (e.g. `examples/functions_demo/models/sessions_report.sql` with `user_col => …, ts_col => …`) would not substitute correctly, so the materialised SQL would not execute. Extending the expander to handle named-args / PASSING-clause substitution is its own work — out of Phase 57's scope (whose success criterion is end-to-end execution, not breadth of features). Test 3 is shipped as `#[test] #[ignore]` with the reason text pointing at this entry.
- **Phase 57 — Test 4 ran single-engine (DuckDB dev + DuckDB prod), not cross-engine Spark** (2026-04-27). The plan permitted "a second active backend if available in CI; otherwise just snapshot the Spark SQL for review". Spark Connect is not available in the test environment. The shipped form configures two DuckDB targets with different schemas + different DB files, then asserts `smelt build --target dev` and `smelt build --target prod` produce identical row sets. This dynamically verifies that `set_function_bodies_all` propagated to every compiler in the registry (the unit test `compiler_registry_set_function_bodies_propagates_to_all_targets` asserts the same statically). A true Spark-side cross-engine assertion is deferred until CI gains a Spark Connect oracle.
- **Phase 57 — Test 2 uses a CSV seed instead of a literal-VALUES model** (2026-04-27). The first attempt used `SELECT * FROM (VALUES …) AS t(order_id, …)` as the upstream `raw_orders` model body. `UpstreamSchemas::from_database` calls Salsa's `resolved_model_schema` which does not infer column types through a literal `VALUES` clause, so the resulting `UpstreamSchemas.models["raw_orders"]` is empty and `as_struct_emitter` returns `None` (pass-through), leaving `smelt.as_struct(o EXCEPT customer_id)` verbatim in the emitted SQL — DuckDB then errors at the `EXCEPT` keyword. Switched to a CSV seed under `seeds/raw_orders.csv` because `discover_seed_infos` infers typed columns from the CSV header + first 10 data rows (`crates/smelt-core/src/seeds.rs`). `UpstreamSchemas` then carries the typed `raw_orders` columns and `as_struct` lowers correctly. Filed as a deferral here because the literal-VALUES gap in `resolved_model_schema` is a real limitation, not specific to Phase 57.
...
| 58 | `EXTRACT(part FROM expr)` parser robustness so generators can include it | done | 4f89d9b | 2026-04-27 |
