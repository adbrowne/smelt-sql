...
- **Phase 24 — LSP hover wiring deferred** (2026-04-24). The pure foundation `declared_return_hover_text(sig)` is implemented and tested. Wiring it into the LSP `hover()` handler to surface on `smelt.fn.*` call sites requires finding the SMELT_FN_CALL node at the cursor position, resolving the signature, and formatting the hover — a non-trivial LSP traversal. Deferred to a cleanup pass or Phase 29's LSP completion work (same hover infrastructure).
- **Phase 29 — LSP cursor-in-PASSING-body column completion deferred** (2026-04-24). The plan's test 5 (`lsp_completion_in_passing_body_shows_context_columns`) requires detecting that the cursor is inside a `PASSING_BODY` node, finding the enclosing SMELT_FN_CALL and PASSING_NAME, resolving the callee signature, looking up the named parameter's context annotation, and then returning the context's columns as completions. This requires non-trivial LSP traversal infrastructure (cursor-in-node detection, PASSING-specific ancestry walk) beyond what Phase 29's scope permits. The type-checking (tests 1–4) is complete; completion is deferred to a dedicated LSP polish phase alongside the hover wiring above.
...
| 44b | Fragment-forward parser + type-system support; ship `monitored_session_rollup` (closes finding #8) | done | | 2026-04-26 |
