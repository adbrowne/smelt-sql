...
**2026-07-20 — Phase 5a** ("Dispatch change-suppressed column-scoped MERGE on the keyed run path"): the runtime-dispatch fix itself was implemented in `crates/smelt-runtime/src/execute.rs`'s `plan_is_keyed` branch (build `(maint_sources, explicitly_mutable)`, call `resolve_live_column_scoped_cell`, dispatch `execute_column_scoped_merge_full` alongside the cumulative fold when a live cell resolves) and compiled cleanly with `statement_parity` + `technique_lowering` + all keyed `maintenance_conformance`/`maintenance_pins` legs green — but the prescribed red→green TDD test (a `grain: key` model, `unique_key` set, enriching a `mutation_profile: mutable_snapshot` dimension declared `allow_full_scan`) cannot be constructed against the real pipeline: it fails earlier, at `execute_project`'s pre-execution diagnostic gate, with `MaintenanceNoAdmissibleTechnique`. Root cause: `derive_new_data`'s `Grain::Key` branch (`crates/smelt-db/src/queries/maintenance.rs`, the per-source loop that pushes one `Trigger::NewData{source}` for *every* source the model references) unconditionally refuses the whole plan if *any* referenced source isn't append-only — independent of `allow_full_scan`, independent of whether that source even contributes to the fold's combiner columns. A scratch unit test against `derive_model_maintenance_plan` confirmed the plan *does* carry the expected `ColumnScopedMerge`/`RowIdentity::Key` cell for `UpstreamMutation{raw.users}`, but the same plan also carries a hard `NoAdmissibleTechnique` refusal for `NewData{raw.users}` — `smelt-db/src/lib.rs` maps that to an unconditional `Error`-severity diagnostic, blocking `smelt build`/`execute_project` for *any* `grain: key` model referencing a non-append-only source at all, regardless of Phase 5a's dispatch fix. The `execute.rs` change was reverted (not committed) to keep red-green discipline — implementing runtime dispatch code with no reachable test would violate CLAUDE.md's red-green rule. Candidate options for the human-gated fix, out of this phase's scope (touching `derive.rs` is explicitly forbidden by Phase 5a's "critical files" list):
  - (a) Narrow `derive_new_data`'s `Grain::Key` obligation from "every referenced source must be append-only" to "every source that feeds the fold's `NewData` combiner must be append-only" — sources consumed only via `explicitly_mutable`/`allow_full_scan` enrichment joins would no longer trip the refusal. Needs care: confirm no admission-safety hole opens (a source silently excluded from the append-only obligation must still be covered by *some* trigger, i.e. the `UpstreamMutation` cell this phase's dispatch fix consumes).
  - (b) Treat this as its own follow-up phase/plan (5a′) scoped explicitly to the `derive_new_data` admission narrowing, with its own TDD tests and admission-safety review, landed before 5a's dispatch code is reintroduced.
  Recommendation: (b) — the admission-safety implications belong in their own reviewed phase, not folded into a "runtime-only, no derive.rs changes" phase by definition.
