...
|-------|-------|--------|--------|--------|------|
| P1 | NOT-NULL invariant: nullable `partition_column` or pruning `event_time_column` → `MalformedTimeseries` | pending | D-52 rule 7 | feat(db): reject nullable timeseries partition/pruning columns (D-52 rule 7) | |
