...
- Phase 5 leftover patched in Phase 6: `smelt build` did not discover generator-emitted models, and emitted-model bodies referencing per-record fields (e.g. `c.region` inside `SELECT * FROM orders WHERE region = c.region`) were not substituted with the record's per-emission values. Phase 6 added: `discover_emitted_model_files` integration in `crates/smelt-cli/src/commands/run.rs` and `crates/smelt-cli/src/lib.rs`; two-pass dep resolution + address-index in `crates/smelt-cli/src/logical_graph.rs` so emitted dotted paths resolve in the dep graph; `substitute_field_accesses_in_sql` (pure function) and the `<generator>` frame-stamping loop in `crates/smelt-db/src/queries/project.rs`; `register_loader_files_from_disk` helper in `crates/smelt-cli/src/lib.rs`. All additions are spec-required for the killer-demo `cohort_count_acceptance` test. Expanded Phase 6's allowed-files list to include these four files; recorded here as the acceptance gate.
- Phase 4 evaluator-gap surfaced in Phase 6: the generator-body evaluator in `crates/smelt-db/src/queries/project.rs::evaluate_body_emissions` (Case 2 — the `|> map(fn x => ModelDef {…})` path) currently only enumerates records when the pipeline driver is `smelt.config.load_yaml` or `smelt.config.load_json`. `smelt.sources.with_tag` is admitted by the type-system as a generator driver (Phase 3 spec rule `smelt.sources.* is admitted in generator bodies`) but resolves to zero emissions at runtime because `extract_loader_path` returns `None` for non-config-loader calls. `examples/staging_from_sources/` therefore ships with a hardcoded `[ModelDef {…}, …]` list literal instead of the idiomatic `smelt.sources.with_tag('raw') |> map(fn s => ModelDef {…})` body; a README in that fixture records the divergence and points at this plan. Extending the evaluator to iterate `smelt.sources.with_tag` results is out-of-scope for Phase 6 and is recorded as a Known Divergence in `docs/specs/meta_language.md` for follow-up.
- Phase 6 reviewer findings: three material findings surfaced (staging fixture driver, reference-page entries + alphabetical order, scope expansions). Findings 2 (reference page) was fixed; Finding 1 (staging driver) deferred per the evaluator-gap entry above; Finding 3 (scope expansions) recorded here. No round 3 required.
- Phase 7 salsa-expert finding "no O(workspace) scans on a single-file edit" deferred. The hand-authored-path collection block at `crates/smelt-db/src/queries/project.rs::emitted_models` iterates `workspace.files(db)` and parses each non-generator file to extract its smelt path. Since `workspace.files(db)` is the input set, any single-file edit invalidates `emitted_models` and re-runs the full scan. `models_all` / `models_with_tag` already exhibit the same pattern — the plan's verification item is aspirational under the current Salsa input-tracking granularity. Extracting `hand_authored_smelt_paths(workspace)` as its own memoised query would shrink the scan radius marginally but cannot avoid the underlying invalidate-on-any-file-edit behaviour without finer-grained file inputs. Tracked for follow-up alongside the broader incremental-reflection scaling work in `docs/plans/20260509-meta-language-overall.md`. Salsa-expert findings #3 (cross-generator vs hand-authored collision code) and #4 (stem-extraction asymmetry) were rejected as non-material — the spec explicitly mandates one code (`docs/specs/meta_language.md` line 562) and the asymmetry the expert flagged is "latent if X" by their own admission.

> Phase 7 expert review: expansion-expert clean (R3), lsp-expert clean (R2), salsa-expert clean (R2), examples-curator clean (R1), docs-reviewer clean (R2), cross-feature-impact-reviewer clean (R1). No stop-the-line fired. Per-expert round counts: expansion-expert hit the maximum 3 rounds without stop-the-line (R1 fixes in 97b93bd7; R2 found a documentation accuracy issue in the Known Divergences entry I had added, addressed in e2aa122f; R3 verified the doc fix and returned clean). All other experts converged in ≤2 rounds. Round 2 dispatched the four experts that had findings (docs-reviewer, lsp-expert, expansion-expert, salsa-expert); Round 3 was only needed for expansion-expert's doc-accuracy follow-up.
