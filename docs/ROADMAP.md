...
- ~~Smelt Functions Step 7 Phase 30–31~~ 🔄 (April 25, 2026) — Phase 30: `smelt-planner::logical::LogicalNode::FunctionCall` with `transparent` flag and `FunctionProperties`; `logical_plan` Salsa query in `smelt-db`. Phase 31: column provenance declared via per-declaration frontmatter `provenance:` key, gated by `unstable_schema: true` in `smelt.yml` (`DiagnosticCode::UnstableSchemaRequired` fires when the flag is absent). `ProjectInput::smelt_yml_text` is a Salsa-tracked field updated on each file-watch event so change detection works correctly in the LSP. Phases 32–34 (rule API, expansion, filter pushdown, join elimination) remain.
