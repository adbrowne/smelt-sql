...
- Grammar support for the remaining registered DuckDB gaps, sized by the differential-gate ratchet and the external-corpus ledger (~500 entries): list comprehensions and `LIKE ANY` (they currently fail loud). Dollar-quoted strings (`$$…$$` and `$tag$…$tag$`) are now lexed as ordinary string literals (parse/print/infer Text) — closed July 12, 2026. Underscore digit-separator numeric literals are now accepted syntax; hex-integer literals (`0x1F`) remain fail-loud, matching DuckDB's own lack of hex-literal grammar in this position. `GLOB` is now supported (parse/print/infer Boolean; `NOT GLOB` is not, matching DuckDB's own rejection of that form). SQL-standard function forms (`trim(BOTH…FROM…)`, `substring(FROM FOR)`, `position(IN)`) are now supported (parse/print/infer, reusing the existing TRIM/SUBSTRING/POSITION function-call typing) — closed July 12, 2026. `MAP {key: value, …}` literals are now supported (parse/print/infer `Map(key_type, value_type)`) — closed July 12, 2026.
