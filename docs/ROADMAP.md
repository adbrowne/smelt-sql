...
- ~~Smelt Functions Step 7 Phase 30–33~~ ✅ (April 25, 2026) — Phase 30: `smelt-planner::logical::LogicalNode::FunctionCall` with `transparent` flag and `FunctionProperties`; `logical_plan` Salsa query in `smelt-db`. Phase 31: column provenance declared via per-declaration frontmatter `provenance:` key, gated by `unstable_schema: true` in `smelt.yml` (`DiagnosticCode::UnstableSchemaRequired` fires when the flag is absent). `ProjectInput::smelt_yml_text` is a Salsa-tracked field updated on each file-watch event so change detection works correctly in the LSP. Phase 32: `PlannerRule` trait, `RuleResult`, `RuleContext`, `apply_rules_to_fixed_point` fixed-point loop, and `ExpandTransparentFunctionCalls` rule (with `Cast` wrapper for `needs_cast: true`). Phase 33: `pushed_filter: Option<Plan>` field on `FunctionCall`; `PushFilterIntoTransparentFunction` rule pushes WHERE predicates from enclosing `Select` into transparent, deterministic, Declared-provenance function calls; blocked by opaque provenance, non-transparent calls, non-deterministic functions, and re-push (idempotent). Phase 34 (join elimination) remains.
