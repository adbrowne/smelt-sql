...
**Created:** 2026-09-12
**Status:** blocked
**Blocked summary:** (2026-09-14) row 11m's implement step ran to completion with a live,
reachable credential — no credential gate this time. Four real infra defects were found and
fixed in-pass (bundle seed missing `functions/`, wheel build missing `--features databricks`,
aarch64 cross-compile missing a target `libpython`, `smelt_env` missing
`PYTHONPATH`/`databricks-connect`/`pyarrow`), each verified live. The smoke run (task 4) still
cannot complete, though: the deployed binary's subprocess cannot use the serverless job's ambient
Spark Connect channel (`SPARK_REMOTE` holds a `unix://` socket path only Databricks' own patched
client understands, not an `sc://` URL). This is a genuine architectural gap in the "ambient, no
host/token" design (phase 11d), not a one-line fix — three candidate routes are recorded in
`phases/11m-summary.md` and the newest `## Blocked` entry, none attempted. Criteria 1-10 remain
met; criterion 11 is still open and needs a dedicated planning pass, not a resume.
...
- **2026-09-14 — phase 11m: four real infra defects found and fixed (functions/ omitted from
  bundle seed, wheel build missing `--features databricks`, aarch64 cross-compile missing a
  target `libpython`, `smelt_env` missing `PYTHONPATH`/`databricks-connect`/`pyarrow`), but the
  smoke run (task 4) still cannot complete: the deployed binary's subprocess cannot use the
  serverless job's ambient Spark Connect channel at all.** `SPARK_REMOTE` reaches the subprocess
  but holds a `unix:///da...` socket path — Databricks' own internal IPC channel to the notebook
  kernel's already-established session — not an `sc://host:port` URL `databricks-connect`'s public
  client can parse. `load_next_day.py` works because it runs *inside* the task's own Python kernel
  process (where the internal ambient-session machinery is reachable); `run_smelt.py` shells out to
  a separately-compiled binary, whose subprocess only inherits environment variables, and the one
  set is in a scheme only Databricks' own patched client understands. This is a genuine
  architectural gap in the "ambient, no host/token" design (11d), not a one-line fix — three
  candidate routes (run in-process instead of shelling out; find and bridge the internal
  `_try_get_notebook_session()` mechanism; revisit whether ambient-only is right for a spawned
  subprocess at all) are recorded in `phases/11m-summary.md` with none attempted. Every landed fix
  this phase stays committed regardless of how this is eventually resolved. Criterion 11 remains
  open.
...
- **2026-09-14 — phase 11m blocked at its own documented prerequisite: the dogfood credential is
  ...
  11m per plan. `bash scripts/dbx-verify.sh` failed every schema check with
  `PERMISSION_DENIED: Invalid Token`, exactly as the plan's Prerequisites section predicted at
  plan time. Re-minting requires `bash scripts/dbx-auth.sh`, which drops into an interactive
  `gpg --decrypt` passphrase prompt this session has no way to answer. Per the plan's own
  instruction ("If the passphrase cannot be supplied in this session (headless), do no partial
  live work: emit `<<PHASE_BLOCKED>>`"), no live task was attempted — no redeploy, no smoke run,
  no scheduled runs, no state changed. This is a credential-refresh gate, not a design or code
  gap: once a human with the passphrase runs `bash scripts/dbx-auth.sh` and
  `source scripts/dbx-dogfood-env.sh` and confirms `bash scripts/dbx-verify.sh` is green, the next
  implement step should resume 11m at task 2 with no plan changes needed. See
  `phases/11m-summary.md`.
...
- 2026-09-13 (phase 11i implement, fifth attempt): **credential confirmed live, plan still
  ...
  the human unblock logged just above this entry holds. The sole remaining blocker is unchanged
  across five consecutive implement passes: no `phases/11i-plan.md` exists (only
  `phases/11i-summary.md`, itself a blocked-attempt record, not a plan). The implement step
  executes an already-planned phase; it does not author one. No code or doc changes made other
...
- 2026-09-13 (phase 11i implement, third attempt): **unchanged from the second attempt.** Still
  ...
  against the live workspace (`https://dbc-466c2133-56f4.cloud.databricks.com`) still returns
  `Error: Invalid Token`. No code or doc changes made other than this log entry; row 11i stays
  `planned`. Both prerequisites from the prior entry are still open — write
  `phases/11i-plan.md` from the outcome-table one-liner, and get a human to refresh the
  credential via `bash scripts/dbx-auth.sh` (interactive `gpg` passphrase, not headless-capable)
  followed by `source scripts/dbx-dogfood-env.sh` — before the next implement pass can do live
  work here.
...
- 2026-09-13 (phase 11g plan): **no reshape.** 11f closed the wheel blocker offline and
  ...
  with two adjustments: the evidence files are renamed `11g-*` (11e never wrote `11e-*`), and
...
- 2026-09-13 (phase 11f implement): **`maturin build --zig --compatibility manylinux_2_28`
  ...
  `objdump -T` on the built binary confirmed the max versioned symbol is exactly `GLIBC_2.28`.
  The docker path is implemented per the plan (documented fallback) but unexercised in this run.
  `ensure_zig`'s bootstrap uses `uv venv`/`uv pip install`, not `python3 -m venv` — the system
  `python3` on this box has no working `ensurepip` (`python3-venv` not installed), which the repo
  already works around elsewhere via `uv` (scripts/dbx-dogfood-venv.sh, scripts/bigquery-venv.sh).
