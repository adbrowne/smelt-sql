...
- **2026-09-14 — phase 9 done: `.smelt/` proved non-correctness-bearing on Trino, live and
  ...
  a two-model `table`-materialization project changes neither table's value, guarded against a
  silently-no-opped rerun; `state.mode: stateless` writes no `.smelt/` and produces the same
  table values as `state.mode: intervals`. Offline half
  (`trino_posture_plan_invariance.rs`, 2/2): `smelt explain --json`'s cells are byte-identical
  across both postures for the two incremental shapes that cannot yet run live on Trino
  (`lifetime_spend`/`KeyedFold`, `cs_merged`/`ColumnScopedMerge`) — a plain `grain: partition`
  cell with no key-addressed edge carries no `state_downgrade` at all, so it couldn't have stood
  in for the second shape. Along the way this phase's own live rerun tests exposed and fixed a
  pre-existing Trino backend bug, unrelated to `.smelt/` and never hit before because no prior
  test ran `smelt run` twice against the same table: `DROP VIEW IF EXISTS`/`DROP TABLE IF EXISTS`
  errors instead of no-opping when an object of the *other* kind already exists under that name,
  which `execute_model_default`'s drop-both-kinds-first step hits on every second run. Fixed with
  a tolerant wrapper in `smelt-backend-trino` swallowing that specific cross-kind message across
  all six call sites. See `phases/09-summary.md`.
...
- **2026-09-14 — phase 6 planning: no reshape; one runtime gap folded into the phase rather than
  ...
  analysis-time `DeclaredContractRequiresState` derivation in `smelt-db`'s
  `queries/maintenance/diagnostics.rs` is already fully dialect-driven
  (`backend_dialect_for("trino")` → empty `realisable_state_structures`), so phase 6's deferral leg
  is expected to arrive green; its tests are standing gates, not fixes. (2)
  `retain_departed::emit_departed_key_probe` emits dialect-free ANSI SQL and needs no
  `MaintenanceDialect`, so `retain_departed` needs no Trino-specific work beyond a gate. (3) A real
  gap: `smelt-runtime`'s `execute/project/mod.rs:3554` resolves
  `maintenance_dialect(backend.dialect())?` unconditionally inside the clocked-batch arm, before
  asking whether the model declares `contract.frozen_horizon` at all — on Trino that hard-errors any
  clocked run with `UnsupportedMaintenanceDialect`, which is a refusal where the degradation
  contract specifies a proceed-and-warn. Phase 4 established the lazy-resolution posture in
  `probe_plan.rs`; this site was missed. It serves success criterion 5 ("`frozen_horizon` … follows
  whatever the existing grain and posture rules already say"), so it stays inside phase 6 rather
  than leaving the outcome. Sibling `maintenance_dialect(...)?` sites on paths these three
  declarations do not reach are recorded for phases 7–10 instead of widening phase 6.
