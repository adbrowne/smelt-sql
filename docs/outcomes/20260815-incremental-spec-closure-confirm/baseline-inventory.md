...
| ID | Subsection | Bullet (bold lead-in) | Open Question? | Disposition | Verdict |
|----|------------|------------------------|-----------------|-------------|---------|
...
| IM-13 | - | The ledger's warehouse substrate is DuckDB-only (Open Question) (L1790) | Yes | drifted (still present, no longer tagged Open Question — Spark deferral decided, not undecided, per `docs/research/20260816-open-questions-triage.md`; underlying DuckDB-only gap unchanged) | accurate |
| IM-14 | - | Graph-layer gaps (L1793) | No | drifted (still present but narrowed: self-edges closed 87df1dcc, key-addressed edge admission closed 15b4073a, `--select` scoping closed acb5e66d; only key-temporal-locality-vs-admission residue remains in current wording) | accurate |
...
| IM-23 | - | `change_feed` sources never get an `UpstreamMutation` cell (Open Question) (L1822) | Yes | drifted (a cell now exists per outcome phase 28c; current wording narrows to "always re-derives from the full input" — original "never gets a cell" claim is now false) | accurate |
...
| IS-10 | The partition grain | `g_run >= g_part` auto-coarsening is not implemented (Open Question) (L1101) | Yes | drifted (OQ decided 2026-08-16 — reject with suggestion; landed, but the rejection doesn't yet name the coarsened window, so a narrower still-open gap survives) | accurate |
...
| IS-15 | The key grain | Snapshot-reconcile admits at most one unclocked source in the FROM clause (Open Question) (L1129) | Yes | drifted (moved to §Future Extensions "Multi-source snapshot-reconcile"; `KeyedSnapshotPostureUnsupported` still refuses ≥2 unclocked sources — decision recorded 2026-08-16 to defer, not fixed) | relocated |
...
| IS-18 | The key grain | The reconciliation ledger's fold is transactional on DuckDB only (Open Question) (L1137) | Yes | drifted (still present, bold lead-in reworded with an added clause about the ledger table's existence; same underlying DuckDB-only gap. This is the bullet `docs/outcomes/20260815-keyed-grain-residue` phase 3 ("Transactional ledger fold on every shipped backend") is about, and that phase is `**Status:** blocked` with its own decision log stating the criterion is "deliberately left unmet" — no outcome falsely claims closure, so this is not `residue` either. A prior phase's decision log mislabeled this bullet as `IS-24`; `IS-24` is a different bullet — see below.) | accurate |
...
| IS-22 | The key grain | The derived execution postures are internal, and one of the three is not derived at all (L1155) | No | drifted (`smelt explain` now prints `Execution postures:` per `docs/outcomes/20260815-keyed-grain-residue` phase 4 — `crates/smelt-cli/src/explain.rs:770-808`; current wording narrows to the still-unbuilt "order-independence is not yet acted on" optimization) | accurate |
| IS-23 | The key grain | The generative conformance pool cannot stage NULL payloads (Open Question) (L1160) | Yes | closed f2b412e0 | - |
| IS-24 | The key grain | Locality open questions (Open Question) (L1165) | Yes | drifted (moved to §Future Extensions "Deletion-adjacent locality relaxations"; same three sub-gaps — recurrence-bound slice pruning, granularity-equality relaxation, slice-scoped deletion — decision 2026-08-16 to defer rather than implement) | relocated |
| IS-25 | The key grain | The pattern functions (`smelt.latest`, `smelt.once`, `smelt.current`) are unshipped (L1169) | No | drifted (bold lead-in reworded to "The pattern-function template file does not exist"; same unshipped gap, still present) | accurate |
| IS-26 | The key grain | Driver granularity is `day`/`week` only (Open Question) (L1173) | Yes | drifted (moved to §Future Extensions "Wider driver granularities"; still day/week only) | relocated |
| IS-27 | The key grain | `--auto` staleness fidelity for all-invertible models is conservative in v1 (Open Question) (L1175) | Yes | drifted (moved to §Future Extensions "Exact `--auto` staleness…"; same gap) | relocated |
| IS-28 | The key grain | Self-referential keyed models are rejected (Open Question) (L1177) | Yes | drifted (moved to §Future Extensions "Self-referential keyed models"; still rejected) | relocated |
| IS-29 | The key grain | Run-pinning alignment is deferred (Open Question) (L1179) | Yes | drifted (OQ decided 2026-08-16 — both grains now run `NOW()`/`CURRENT_*` as-is, no pinning; current bullet "`NOW()`/`CURRENT_*` are still rejected in keyed models" describes the residual divergence with a decision record instead of as an open question) | accurate |
...
| MP-06 | - | `INTERSECT`/`EXCEPT` are unclassified for filter distribution (L352) | No | drifted (scope narrowed by 4f8b9c66, which built per-set-operation-arm mutation-sensitivity classification independent of this gap; the filter-distribution gap itself is still open, wording reflects the narrower scope) | accurate |
...
**Phase 3 verdict** (see the `Verdict` column above): all 11 "reworded but accurate" rows read
`accurate` — each was re-read against the current spec text and the repo, and the gap it
describes is still genuinely open. All 5 relocated rows read `relocated` — each `§Future
Extensions` destination was re-read and is still honestly framed as undecided-future, not a
shipped feature left described as future. No row needed `stale-fixed`.

## Out-of-scope spot-check (phase 3, success criterion 3)

Spot-checks every item `docs/outcomes/20260815-definition-delta-migrate` §"Out of scope" names as
needing a product decision this program declined to make (or deliberately-future per a spec's own
`§Future Extensions`). For each: does the named spec bullet/section still exist, does it still
carry `(Open Question)`/undecided framing where the out-of-scope text claims it does, and is the
behaviour it describes as missing still missing?

| Item (as named in the out-of-scope list) | Spec + section | Still present? | Still tagged/framed as claimed? | Behaviour still missing? | Verdict |
|---|---|---|---|---|---|
| Lattice v2 (retention, reconciliation points, declared indifference, per-column-group freshness) | `incremental_models.md` §Future Extensions ("Further contract-lattice points") | Yes | Yes — explicitly "not decided" | Yes | accurate |
| Proofs as product (`smelt prove`, `must_hold:`, proof-diff, guarantee-summary) | `incremental_models.md` §Future Extensions ("Proofs as product") | Yes | Yes | Yes | accurate |
| Smelt-maintained SCD2 via succession-pattern recognition | `incremental_models.md` §Future Extensions | Yes | Yes | Yes | accurate |
| Automatic watermark-diffed `--since-upstream` | `incremental_models.md` §Future Extensions | Yes | Yes | Yes | accurate |
| Observer/prefix-consistency contract for non-replayable combinations | `incremental_models.md` §Future Extensions | Yes | Yes | Yes | accurate |
| Eclipse-detection breadth (algebraic identities, join reorderings) | `definition_deltas.md` §Future Extensions | Yes | Yes | Yes | accurate |
| Row-local derivation for mid-catch-up groups | `definition_deltas.md` §Future Extensions | Yes | Yes | Yes | accurate |
| Retraction handling / change-feed consumption as a first-class delta shape | `definition_deltas.md` §"What stays data-side" (defers to `incremental_models.md` §Future Extensions) | Yes | Yes | Yes — `change_feed` sources still only get the always-full-re-derive `UpstreamMutation` cell (IM-23), not fold consumption | accurate |
| Ladder rungs 3–4 (group-rung retraction, bounded-domain multiset) | `incremental_shapes.md` §Known Divergences (IS-31) | Yes | N/A (not an Open-Question bullet) | Yes | accurate |
| Locality open questions (slice pruning, granularity-equality relaxation, slice-scoped deletion) | `incremental_shapes.md` §Future Extensions ("Deletion-adjacent locality relaxations", = IS-24 relocated) | Yes | Yes | Yes | accurate |
| Key deletion beyond retention (tombstones, opt-in hard delete) | No literal current bullet — `incremental_shapes.md` §"Departed keys and deletion" now specifies posture-derived deletion plus the opt-in `retain_departed` lattice point (closed 2e1f6d19, formerly IS-30); tombstones and an explicit hard-delete-beyond-retain opt-in are not described anywhere as planned surface | Partial | N/A | Yes, as a narrower idea than IS-30 (which is closed) | accurate but narrower than the out-of-scope text implies — see note below |
| Key temporal locality route 2 declared-FD-only admission | `incremental_shapes.md` §Known Divergences (IS-20) | Yes | Yes — tagged `(Open Question)` | Yes | accurate |
| Locality-machinery gaps (route 3 DuckDB-binder limit, granularity/recurrence-precedence underdetermined) | `incremental_shapes.md` §Known Divergences (IS-21) | Yes | N/A | Yes | accurate |
| `key_per_partition` missing plan derivation | `incremental_shapes.md` §Known Divergences (IS-32) | Yes | N/A | Yes | accurate |
| `docs/plans/20260704-model-updates.md` pending rows (C3/C4 = ladder rungs above; D1 `refresh: latest_value`, D2 `refresh: versioned`, D3 `refresh: materialized_view`) | Cross-spec (`models.md`, `materialized_view.md`) | D3 only | No — D3 (`refresh: materialized_view`) is fully specified and shipped (`docs/specs/materialized_view.md`), not "unclear whether it survived consolidation" as the out-of-scope text still says; `refresh: latest_value`/`refresh: versioned` (D1/D2) appear nowhere in the current specs, so their fate is still genuinely unclear | D1/D2 yes, D3 no | **stale for the D3 leg** — the out-of-scope text's "unclear ... individually" framing no longer holds for D3; not a spec-text divergence (no spec bullet claims D3 is undecided), so nothing to edit in `docs/specs/`, but worth flagging so a future reader of the outcome doesn't re-litigate D3 |
| `g_run >= g_part` auto-coarsening vs. reject-with-suggestion | `incremental_shapes.md` §Known Divergences (IS-10, drifted/accurate above) | Yes | No — decided 2026-08-16 (reject-with-suggestion), matches IS-10's own drifted disposition | Narrower residual gap only | accurate (consistent with IS-10) |
| Snapshot-reconcile multi-unclocked-source admission | `incremental_shapes.md` §Future Extensions (IS-15, relocated) | Yes | Yes | Yes | accurate |
| Once-write nullability route for a key-derived expression | `incremental_shapes.md` §Known Divergences (IS-13) | Yes | N/A | Yes | accurate |
| Pattern functions as built-ins vs. shipped template | `incremental_shapes.md` §Known Divergences (IS-25) + §Future Extensions ("Promoting the pattern functions to built-ins") | Yes | N/A / Yes | Yes | accurate |
| Driver granularity below `day`/`week` | `incremental_shapes.md` §Future Extensions (IS-26, relocated) | Yes | Yes | Yes | accurate |
| `--auto` staleness fidelity beyond conservative v1 | `incremental_shapes.md` §Future Extensions (IS-27, relocated) | Yes | Yes | Yes | accurate |
| Self-referential keyed models | `incremental_shapes.md` §Future Extensions (IS-28, relocated) | Yes | Yes | Yes | accurate |
| Run-pinning alignment for `NOW()`/`CURRENT_*` in keyed models | `incremental_shapes.md` §Known Divergences (IS-29, drifted/accurate above) | Yes | Yes — the out-of-scope text's own characterisation ("today a deliberate hard refusal, `KeyedForbidsNondeterministic`, not merely an unfilled gap") matches the current bullet's wording exactly | Yes | accurate |
| Broader backbuild statement-parity coverage (D2/B5/B6/E1/E2/E4/F1/F2/C1 techniques) | Not a spec bullet — an outcome-scoping note about test breadth, not a spec claim | N/A | N/A | N/A | not spec-tracked; no drift possible |
| Full contract-lattice probe listing in `smelt explain` | Not a spec bullet — `probe_plan_for_model` (`crates/smelt-runtime/src/probe_plan.rs`) still exists and is the single owner per the function-registry-style rule; the out-of-scope text itself states no success criterion asks for explain-side completeness | N/A | N/A | N/A | not spec-tracked; no drift possible |

**Finding:** one narrow staleness, not in spec text. The out-of-scope list's `docs/plans/20260704-model-updates.md` item says D1–D3's fate is "unclear ... without checking each mode's fate individually" — true for D1 (`latest_value`)/D2 (`versioned`), but D3 (`refresh: materialized_view`) has since shipped as fully-specified surface (`docs/specs/materialized_view.md`, referenced from `models.md`, `multi_backend.md`, `diagnostics.md`). This is staleness in the **outcome's own historical out-of-scope prose**, not in any of the four anchor specs' `§Known Divergences`/`§Future Extensions` text — no spec bullet claims D3 is undecided, so there is nothing to edit under this phase's spec-only-if-trivial rule (editing another outcome's historical record is out of convention; `docs/outcomes/20260815-definition-delta-migrate` is `done`). Recorded here, not edited, per the standing rule: no product decision needed (D3's fate is already decided and shipped), so this is neither a new phase row nor a `## Blocked` entry — just a closure-report footnote.

No other item's spec-side framing was found stale: every remaining named item is still accurately described as open/undecided in its owning spec section, and every relocated-to-Future-Extensions item is still honestly framed as not-decided there.
