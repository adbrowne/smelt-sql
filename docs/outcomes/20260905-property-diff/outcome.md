...
| # | Phase | Status |
|---|-------|--------|
| 1 | Diagnostics catalogue + spec cross-refs: add `PropertyDowngrade` and `PropertyDiffBaselineUnavailable` to `DiagnosticCode`, `diagnostics.md`, `map_metadata_error_to_diagnostic` exhaustiveness untouched; `cli.md`/`lsp.md` pointers to `property_diff.md` | pending |
| 2 | `PropertyProfile` in `smelt-logical` derived by the same pure functions the report uses; `ModelDiagnostics` and the CLI report render from it; `property_profile_parity` gate over both example workspaces | pending |
