...
-- The chain is:
--   `outer_call(y) AS (smelt.middle(y))`
--   `middle(z) AS (smelt.inner_unary(z))`
...
smelt.define middle(z) AS (smelt.inner_unary(z))
smelt.define outer_call(y) AS (smelt.middle(y))

SELECT smelt.outer_call(1) AS threaded
