...
static ROWS: &[LedgerRow] = &[
    ...
    //
    // Trino: `20260913-trino-emission` phase 5's live schema leg. Bulk
    // tracker #209 — most rows are a DuckDB-shaped name Trino has never
    ...
    gap("CHARACTER_LENGTH", DialectId::Trino, "#209", "no `character_length`; Trino spells it `length`"),
    ...
    gap("DAYOFWEEK", DialectId::Trino, "#209", "no `dayofweek`; Trino spells it `day_of_week`"),
    gap("EPOCH_US", DialectId::Trino, "#209", "no `epoch_us` in Trino"),
    gap("EXPLODE", DialectId::Trino, "#209", "no `explode`; Trino's array flattening is `UNNEST`, a FROM-clause table function, not a select-list call"),
    gap("FIRST", DialectId::Trino, "#209", "no `first`; Trino has no unordered pick-any aggregate under this name"),
    gap("GLOB", DialectId::Trino, "#209", "no `GLOB` operator in Trino"),
    gap("GROUP_CONCAT", DialectId::Trino, "#209", "no `group_concat`; Trino spells string aggregation `array_join(array_agg(x), sep)` or `LISTAGG`"),
    gap("IFNULL", DialectId::Trino, "#209", "no `ifnull`; Trino spells it `coalesce`"),
    ...
    gap("LAST", DialectId::Trino, "#209", "no `last`; Trino has no unordered pick-any aggregate under this name"),
    gap("LEFT", DialectId::Trino, "#209", "no `LEFT(x, n)` function form; Trino's `LEFT`/`RIGHT` are join keywords only"),
    gap("LISTAGG", DialectId::Trino, "#209", "Trino's `LISTAGG` requires a `WITHIN GROUP (ORDER BY ...)` clause; DuckDB's plain call and `OVER` window form do not parse"),
    gap("MAKE_DATE", DialectId::Trino, "#209", "no `make_date` in Trino"),
    gap("MAKE_TIME", DialectId::Trino, "#209", "no `make_time` in Trino"),
    gap("MAKE_TIMESTAMP", DialectId::Trino, "#209", "no `make_timestamp` in Trino"),
    gap("MAKE_TIMESTAMPTZ", DialectId::Trino, "#209", "no `make_timestamptz` in Trino"),
    ...
    gap("TO_JSON", DialectId::Trino, "#209", "no `to_json` in Trino"),
    gap("TO_SECONDS", DialectId::Trino, "#209", "no `to_seconds` in Trino"),
    gap("TRUNC", DialectId::Trino, "#209", "no `trunc`; Trino spells numeric truncation `truncate`"),
    gap("UNNEST", DialectId::Trino, "#209", "Trino allows `UNNEST` only in a FROM clause, never in a select list"),
    type_gap("//", DialectId::Trino, "#209", "DECIMAL `//` DECIMAL infers Double; Trino's integer-division on two DECIMALs returns a DECIMAL"),
