...
- `src/ledger.rs` — the ONE dialect dispatch point for the warehouse-resident ledger. Callers ask it, never `ddl_duckdb::generate_ledger_*` / `ddl_bigquery::generate_ledger_*` by name, so a dialect's spelling is chosen in one place; its `match` is exhaustive over `SqlDialect`, and Spark is an error (Delta has no cross-table transaction) rather than DuckDB SQL Spark cannot run. Gate: `cargo test -p smelt-state --test ledger_dialect`.
