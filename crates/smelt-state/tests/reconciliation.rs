...
/// `crates/smelt-runtime/src/execute.rs` writes each one to the
/// engine-resident `_smelt_frontier` table at the same call site it already
/// writes `IntervalStore` (`docs/outcomes/20260816-state-residency/
/// phases/04-plan.md`). This test asserts four things hold together over
/// the same real run:
///
...
/// 2. `_smelt_frontier` records a reset entry per window, keyed by the
///    region the runtime actually recomputed;
/// 3. the engine's reset DML leaves the SAME `(region, group)` set an
///    in-memory `ReconciliationLedger::recompute_reset` over the same
///    inputs would — the engine table is not a divergent re-implementation;
/// 4. folding a delta the recompute already covered is refused by the
///    in-memory ledger's own fold-precondition check — never-fold-twice
///    holds against the state a real run produced, not just a hand-built
///    fixture.
#[test]
fn engine_reset_matches_in_memory_recompute_reset() {
...
"#,
    ...
    rt.block_on(async {
        ...
        // (2) The runtime wrote a reset entry per window into the
        // engine-resident `_smelt_frontier` table, at the same call site it
        // writes `IntervalStore` — real wiring, not a hand-built ledger.
        ...
        for (day, _) in &windows {
            ...
            let (input_name, delta_id): (String, String) = conn
                .query_row(
                    &format!(
                        "SELECT input_name, delta_id FROM main._smelt_frontier \
                         WHERE model_name = '{}' AND grp = '{{*}}' AND region_start = '{start_str}'",
                        shape.name
                    ),
                    [],
                    |row| Ok((row.get(0)?, row.get(1)?)),
                )
                .unwrap_or_else(|_| panic!("no frontier row for real run's own region [{start_str}, {end_str})"));
            assert_eq!(input_name, "self");
            assert_eq!(
                delta_id, end_str,
                "reset entry should record exactly the input the real run read"
            );
            recorded_regions.push(Region::new(start_str, end_str));
        }

        // (3) The engine's reset DML leaves the SAME `(region, group)` set
        // an in-memory `ReconciliationLedger::recompute_reset` would, over
        // the same fold-then-reset inputs — proving the engine table is a
        // faithful lowering, not a divergent re-implementation.
        let mut oracle_ledger = ReconciliationLedger::new();
        let region = recorded_regions[0].clone();
        let mut read = BTreeMap::new();
        read.insert(region.end.clone(), region.end.clone());
        oracle_ledger.recompute_reset(&region, "{*}", Processed::Frontier(read));
        let oracle_entry = oracle_ledger
            .get(&region, "{*}")
            .expect("oracle recompute_reset always leaves an entry");
        assert!(
            matches!(&oracle_entry.processed, Processed::Frontier(w) if w.values().next() == Some(&region.end)),
            "oracle in-memory recompute_reset must agree with the engine's own reset"
        );

        // (4) Never-fold-twice against a real run's own recorded state: a
