...
//! this file only constructs a `SparkConformanceBackend`, decides the
...
/// `SMELT_CONFORMANCE_SPARK_CASES` env override. Smaller than the DuckDB
/// leg's default (12): each case round-trips over a real Spark Connect
/// server rather than an in-process DuckDB file.
...
/// `admission_rate_stays_above_floor_on_spark` —
/// `SMELT_CONFORMANCE_SPARK_ADMISSION_N` env override, smaller than the
/// DuckDB leg's 50.
...
fn append_only_partition_pool_upholds_equivalence_on_spark() {
    ...
    if let Some(reason) = b.skip_reason() {
        eprintln!("{reason} — skipping append_only_partition_pool_upholds_equivalence_on_spark");
        return;
    }
    let rt = tokio::runtime::Runtime::new().expect("tokio runtime");
    rt.block_on(gate::run_append_only_partition_pool(&b, case_count()))
        .expect("append-only partition pool equivalence check failed on Spark");
}
...
fn admission_rate_stays_above_floor_on_spark() {
    let b = SparkConformanceBackend;
    if let Some(reason) = b.skip_reason() {
        eprintln!("{reason} — skipping admission_rate_stays_above_floor_on_spark");
        return;
    }
    let rt = tokio::runtime::Runtime::new().expect("tokio runtime");
    ...
    .expect("admission rate check failed on Spark");
}
...
fn redelivery_of_processed_window_is_idempotent_on_spark() {
    let b = SparkConformanceBackend;
    if let Some(reason) = b.skip_reason() {
        eprintln!("{reason} — skipping redelivery_of_processed_window_is_idempotent_on_spark");
        return;
    }
    let rt = tokio::runtime::Runtime::new().expect("tokio runtime");
    rt.block_on(gate::run_redelivery_of_processed_window_is_idempotent(&b))
        .expect("redelivery idempotency check failed on Spark");
}
...
fn full_refresh_interleave_resets_state_correctly_on_spark() {
    let b = SparkConformanceBackend;
    if let Some(reason) = b.skip_reason() {
        eprintln!("{reason} — skipping full_refresh_interleave_resets_state_correctly_on_spark");
        return;
    }
    let rt = tokio::runtime::Runtime::new().expect("tokio runtime");
    rt.block_on(gate::run_full_refresh_interleave_resets_state_correctly(&b))
        .expect("full-refresh interleave check failed on Spark");
}

/// `boundary_rows_within_reach_are_reflected_on_spark`.
#[test]
fn boundary_rows_within_reach_are_reflected_on_spark() {
    let b = SparkConformanceBackend;
    if let Some(reason) = b.skip_reason() {
        eprintln!("{reason} — skipping boundary_rows_within_reach_are_reflected_on_spark");
        return;
    }
    let rt = tokio::runtime::Runtime::new().expect("tokio runtime");
    rt.block_on(gate::run_boundary_rows_within_reach_are_reflected(&b))
        .expect("boundary-row reach check failed on Spark");
}
...
fn column_add_between_runs_recovers_equivalence_on_spark() {
    let b = SparkConformanceBackend;
    if let Some(reason) = b.skip_reason() {
        eprintln!("{reason} — skipping column_add_between_runs_recovers_equivalence_on_spark");
        return;
    }
    let rt = tokio::runtime::Runtime::new().expect("tokio runtime");
    rt.block_on(gate::run_column_add_between_runs_recovers_equivalence(&b))
        .expect("column-add recovery check failed on Spark");
}
