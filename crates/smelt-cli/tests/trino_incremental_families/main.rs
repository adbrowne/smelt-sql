...
pub(crate) fn stage_int_partition_project(
...
pub(crate) fn run_smelt(project_dir: &Path, args: &[&str]) -> std::process::Output {
...
/// Runs `smelt explain <model> --json` on `project_dir` and parses the
/// output — shared by the keyed-fold and membership/column-scoped-merge
/// submodules below.
pub(crate) fn explain_json(project_dir: &Path, model_name: &str) -> serde_json::Value {
