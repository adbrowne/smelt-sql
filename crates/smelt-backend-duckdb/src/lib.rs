...
impl Backend for DuckDbBackend {
    ...
    /// Real transactional override (`docs/specs/incremental_models.md`
    /// §"The graph layer" — "Observed deltas on model edges"; extended by
    /// the merge ledger's re-run-tolerant bookkeeping record, `docs/specs/
    /// incremental_shapes.md` §"The transactional frontier write (merge
    /// ledger)"): every `pre_write_sqls` entry (e.g. the observed-delta
    /// upsert, which reads the target table's PRE-write state), then every
    /// statement in `write_group` — all inside one `duckdb::Transaction`, so
    /// either both the pre-write records and the write commit, or neither
    /// does. `ensure_sqls` (idempotent `CREATE TABLE IF NOT EXISTS`
    /// statements) run first, outside that transaction, same precedent as
    /// `fold_ledger_delta`'s own `ensure_sql` handling. `Transaction`'s
    /// default `DropBehavior::Rollback` means a failure anywhere in
    /// `pre_write_sqls` or `write_group` rolls back every statement already
    /// applied in this call — a failed write never leaves a bookkeeping row
    /// behind (the records and the write share one commit point), and a
    /// failed record never lets the write proceed.
    async fn execute_write_with_bookkeeping(
        ...
        let ensure_sqls = ensure_sqls.to_vec();
        let mut statements: Vec<String> = pre_write_sqls.to_vec();
        ...
        tokio::task::spawn_blocking(move || {
            ...
            for ensure_sql in &ensure_sqls {
                conn.execute(ensure_sql, [])
                    .map_err(|e| BackendError::execution_failed("bookkeeping", e.to_string()))?;
            }
...
mod tests {
    ...
    // ── execute_write_with_bookkeeping (generalised T5/MP12 seam) ──────

    #[tokio::test]
    async fn write_with_bookkeeping_rolls_back_on_write_failure() {
        let temp_dir = TempDir::new().unwrap();
        let db_path = temp_dir.path().join("test.duckdb");
        let backend = DuckDbBackend::new(&db_path, "main").await.unwrap();

        backend
            .execute_sql("CREATE TABLE main.bookkeeping_log (id INTEGER)")
            .await
            .unwrap();
        let ensure_sqls =
            vec!["CREATE TABLE IF NOT EXISTS main.bookkeeping_log (id INTEGER)".to_string()];
        // A write statement that fails (references a nonexistent table) —
        // the pre-write bookkeeping record must never land either.
        let write_group = StatementGroup {
            statements: vec![smelt_backend::MaintenanceStatement {
                sql: "INSERT INTO main.does_not_exist VALUES (1)".to_string(),
            }],
        ...
        let pre_write_sqls = vec!["INSERT INTO main.bookkeeping_log VALUES (1)".to_string()];
        ...
        let count = backend
            .get_row_count("main", "bookkeeping_log")
            .await
            .unwrap();
        assert_eq!(
            count, 0,
            "a failed write must leave no bookkeeping row behind"
        );
    }

    #[tokio::test]
    async fn write_with_bookkeeping_runs_pre_writes_before_the_write() {
        let temp_dir = TempDir::new().unwrap();
        let db_path = temp_dir.path().join("test.duckdb");
        let backend = DuckDbBackend::new(&db_path, "main").await.unwrap();
        ...
        backend
            ...
            .unwrap();
        backend
            .execute_sql("CREATE TABLE main.pre_write_marker (seen_before_write BOOLEAN)")
            .await
            .unwrap();

        let ensure_sqls: Vec<String> = vec![];
        let write_group = StatementGroup {
            statements: vec![smelt_backend::MaintenanceStatement {
                sql: "INSERT INTO main.device_stats \
                      SELECT 1 WHERE EXISTS (SELECT 1 FROM main.pre_write_marker \
                      WHERE seen_before_write)"
                    .to_string(),
            }],
            transactional: false,
        };
        let pre_write_sqls = vec!["INSERT INTO main.pre_write_marker VALUES (true)".to_string()];

        backend
            .execute_write_with_bookkeeping(&ensure_sqls, &pre_write_sqls, &write_group)
            .await
            .expect("pre-write then write both succeed");

        let count = backend.get_row_count("main", "device_stats").await.unwrap();
        assert_eq!(
            count, 1,
            "the write observed the pre-write record, proving pre-write ran first"
        );
    }
