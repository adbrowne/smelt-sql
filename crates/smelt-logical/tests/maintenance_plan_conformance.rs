...
const KNOWN_GAPS: &[(&str, usize, &str)] = &[
    ...
    ("additive agg (SUM/COUNT)", 0, "EX-13: plausibly covered by the pre-existing `g_01_additive_agg_append_only` property-discovery probe, not re-verified against this exact catalogue id this pass"),
    ("additive agg (SUM/COUNT)", 2, "EX-04 dagger (discussion variant)"),
    ("additive agg (SUM/COUNT)", 4, "EX-20: plausibly covered by the pre-existing `g_02_additive_agg_redelivery` property-discovery probe, not re-verified against this exact catalogue id this pass"),
    ("idempotent agg (MIN/MAX/BOOL_OR)", 0, "EX-15: plausibly covered by `g_03_idempotent_agg_append_only`, not re-verified against this catalogue id"),
    ...
    ("holistic agg (MEDIAN/COUNT DISTINCT)", 0, "EX-17: plausibly covered by `g_07_holistic_agg_append_only`, not re-verified against this catalogue id"),
    ...
    ("inner-join enrichment", 2, "EX-07: plausibly covered by `g_05_join_enrichment_mutable_dimension`, not re-verified against this catalogue id"),
    ("inner-join enrichment", 3, "EX-26 dagger (discussion variant) — headline covered under `keyed end-state fold` row instead"),
    ("inner-join enrichment", 6, "EX-10: composite-key inner-join cell — plausibly covered by `g_10_composite_key_join_fan_out`, not re-verified against this catalogue id"),
    ...
    ("join fan-out (1:N / N:1 proof)", 0, "EX-10: plausibly covered by `g_10_composite_key_join_fan_out`, not re-verified against this catalogue id"),
    ...
    ("join fan-out (1:N / N:1 proof)", 6, "EX-10: plausibly covered by `g_10_composite_key_join_fan_out`, not re-verified against this catalogue id"),
